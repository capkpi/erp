...
frappe.ui.form.on("Tally Migration", {
	...
	refresh: function (frm) {
		...
		erp.tally_migration.failed_import_log = JSON.parse(frm.doc.failed_import_log);
		erp.tally_migration.fixed_errors_log = JSON.parse(frm.doc.fixed_errors_log);
	...
	render_html_table(frm, shown_logs, hidden_logs, field) {
		...
		let rows = erp.tally_migration.get_html_rows(shown_logs, field);
	...
	show_error_summary(frm) {
		let summary = erp.tally_migration.failed_import_log.reduce((summary, row) => {
			if (row.doc) {
	...
	show_errored_import_log(frm) {
		let import_log = erp.tally_migration.failed_import_log;
	...
	show_fixed_errors_log(frm) {
		let completed_log = erp.tally_migration.fixed_errors_log;
...
erp.tally_migration.getError = (traceback) => {
...
erp.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function(key, value){
		if (value === "" || value === null){
			...
			erp.tally_migration.cleanDoc(value);
		} else if ($.isArray(value)) {
			$.each(value, function (k,v) { erp.tally_migration.cleanDoc(v); });
		}
	});
	return temp;
}

erp.tally_migration.unresolve = (document) => {
	/* Mark document migration as unresolved ie. move to failed error log */
	let frm = cur_frm;
	let failed_log = erp.tally_migration.failed_import_log;
...
erp.tally_migration.get_html_rows = (logs, field) => {
	let index = 0;
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(erp.tally_migration.cleanDoc(doc), null, 1)}</pre>
					</div>
				</div>`;

			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='erp.tally_migration.create_new_doc(${JSON.stringify(doc)})'>
					${__("Create Document")}
				</button>`

			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='erp.tally_migration.unresolve(${JSON.stringify(doc)})'>
