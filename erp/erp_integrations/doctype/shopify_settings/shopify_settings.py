...
class ShopifySettings(Document):
					...
					data=json.dumps(
						{
							"webhook": {
								...
								"address": get_webhook_address(
									connector_name="shopify_connection", method="store_request_data", force_https=True
								),
								"format": "json",
							}
						}
	...
	def update_webhook_table(self, method, res):
		self.append("webhooks", {"webhook_id": res["webhook"]["id"], "method": method})


def get_shopify_url(path, settings):
	if settings.app_type == "Private":
		return "https://{}:{}@{}/{}".format(
			settings.api_key, settings.get_password("password"), settings.shopify_url, path
		)
	else:
