...
from capkpi.utils import cint, cstr, flt, get_datetime, get_request_session, getdate, nowdate

from erp import get_company_currency
from erp.erp_integrations.doctype.shopify_log.shopify_log import (
	dump_request_data,
	make_shopify_log,
)
...
from erp.erp_integrations.doctype.shopify_settings.sync_customer import create_customer
from erp.erp_integrations.doctype.shopify_settings.sync_product import (
	sync_item_from_shopify,
)
from erp.erp_integrations.utils import validate_webhooks_request
from erp.selling.doctype.sales_order.sales_order import make_delivery_note, make_sales_invoice
...
def store_request_data(order=None, event=None):
	if capkpi.request:
		...
		event = capkpi.request.headers.get("X-Shopify-Topic")
...
def sync_sales_order(order, request_id=None, old_order_sync=False):
	...
	capkpi.flags.request_id = request_id

	if not capkpi.db.get_value("Sales Order", filters={"shopify_order_id": cstr(order["id"])}):
		try:
...
def get_sales_order(shopify_order_id):
	sales_order = capkpi.db.get_value("Sales Order", filters={"shopify_order_id": shopify_order_id})
	if sales_order:
		so = capkpi.get_doc("Sales Order", sales_order)
		return so
...
def validate_customer(order, shopify_settings):
	...
	if customer_id:
		if not capkpi.db.get_value("Customer", {"shopify_customer_id": customer_id}, "name"):
			create_customer(order.get("customer"), shopify_settings)


def validate_item(order, shopify_settings):
	for item in order.get("line_items"):
		if item.get("product_id") and not capkpi.db.get_value(
...
def create_sales_order(shopify_order, shopify_settings, company=None):
	product_not_exists = []
	customer = capkpi.db.get_value(
		"Customer", {"shopify_customer_id": shopify_order.get("customer", {}).get("id")}, "name"
	)
	so = capkpi.db.get_value("Sales Order", {"shopify_order_id": shopify_order.get("id")}, "name")
	...
	if not so:
		...
		so = capkpi.get_doc("Sales Order", so)
...
def create_sales_invoice(shopify_order, shopify_settings, so, old_order_sync=False):
	if (
		not capkpi.db.get_value("Sales Invoice", {"shopify_order_id": shopify_order.get("id")}, "name")
		and so.docstatus == 1
...
def get_item_code(shopify_item):
	...
	if not item_code:
		item_code = capkpi.db.get_value("Item", {"item_name": shopify_item.get("title")}, "item_code")
...
def get_tax_account_head(tax):
	...
	tax_account = capkpi.db.get_value(
		"Shopify Tax Account", {"parent": "Shopify Settings", "shopify_tax": tax_title}, "tax_account"
	)

	if not tax_account:
		capkpi.throw(_("Tax Account not specified for Shopify Tax {0}").format(tax.get("title")))
