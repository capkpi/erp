...
from capkpi.contacts.doctype.contact.contact import get_contact_with_phone_number
from capkpi.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from capkpi.model.document import Document

from erp.crm.doctype.lead.lead import get_lead_with_phone_number
from erp.crm.doctype.utils import get_scheduled_employees_for_popup, strip_number
...
class CallLog(Document):
	...
	def on_update(self):
		...
		if _is_call_missed(doc_before_save, self):
			capkpi.publish_realtime("call_{id}_missed".format(id=self.id), self)
			self.trigger_call_popup()

		if _is_call_ended(doc_before_save, self):
			capkpi.publish_realtime("call_{id}_ended".format(id=self.id), self)
	...
	def trigger_call_popup(self):
		if self.is_incoming_call():
			...
			for email in emails:
				capkpi.publish_realtime("show_call_popup", self, user=email)
...
def add_call_summary_and_call_type(call_log, summary, call_type):
	doc = capkpi.get_doc("Call Log", call_log)
	doc.type_of_call = call_type
	doc.save()
	doc.add_comment("Comment", capkpi.bold(_("Call Summary")) + "<br><br>" + summary)
...
def get_employees_with_number(number):
	...
	employee_doc_name_and_emails = capkpi.cache().hget("employees_with_number", number)
	if employee_doc_name_and_emails:
		return employee_doc_name_and_emails

	employee_doc_name_and_emails = capkpi.get_all(
		"Employee",
		filters={"cell_number": ["like", "%{}%".format(number)], "user_id": ["!=", ""]},
		fields=["name", "user_id"],
	)

	capkpi.cache().hset("employees_with_number", number, employee_doc_name_and_emails)
...
def link_existing_conversations(doc, state):
	...
	try:
		...
		for number in numbers:
			...
			for log in logs:
				call_log = capkpi.get_doc("Call Log", log)
				call_log.add_link(link_type=doc.doctype, link_name=doc.name)
				call_log.save(ignore_permissions=True)
			capkpi.db.commit()
	except Exception:
		capkpi.log_error(title=_("Error during caller information update"))
...
def get_linked_call_logs(doctype, docname):
	...
	logs = capkpi.get_all("Call Log", fields=["*"], filters={"name": ["in", logs]})
