...
class QualityReview(Document):
	def validate(self):
		...
		if not self.reviews:
			for d in capkpi.get_doc("Quality Goal", self.goal).objectives:
...
def review():
	day = capkpi.utils.getdate().day
	weekday = capkpi.utils.getdate().strftime("%A")
	month = capkpi.utils.getdate().strftime("%B")

	for goal in capkpi.get_list("Quality Goal", fields=["name", "frequency", "date", "weekday"]):
...
def create_review(goal):
	goal = capkpi.get_doc("Quality Goal", goal)

	review = capkpi.get_doc(
		{"doctype": "Quality Review", "goal": goal.name, "date": capkpi.utils.getdate()}
	)
