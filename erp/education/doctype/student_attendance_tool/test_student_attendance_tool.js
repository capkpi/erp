	...
	capkpi.run_serially([
		...
		() => {
			if(cur_frm.doc.based_on == 'Student Group' || cur_frm.doc.based_on == 'Course Schedule'){
				...
				cur_frm.set_value("date", capkpi.datetime.nowdate());
			}
		},
		() => capkpi.timeout(0.5),
		() => {
			assert.equal($('input.students-check').size(), 5, "Student list based on batch correctly fetched");
			assert.equal(capkpi.datetime.nowdate(), cur_frm.doc.date, 'Current date correctly set');
		...
		() => capkpi.timeout(1),
		() => capkpi.tests.click_button('Check all'), // Marking all Student as checked
		() => {
		...
		() => capkpi.timeout(1),
		() => capkpi.tests.click_button('Uncheck all'), // Marking all Student as unchecked
		() => {
			count = 0;
			for(i = 0; i < $('input.students-check').size(); i++){
		...
		() => capkpi.timeout(1),
		() => capkpi.tests.click_button('Check all'),
		() => capkpi.tests.click_button('Mark Attendance'),
		() => capkpi.timeout(1),
		...
		() => capkpi.timeout(1),
		() => capkpi.set_route('List', 'Student Attendance/List'),
		() => capkpi.timeout(1),
		() => {
