	...
	doc = capkpi.get_doc(
		{
			...
			"student_group_name": "_Test Student Group-" + capkpi.generate_hash(length=5),
			"group_based_on": "Activity",
		}
	).insert()

	student_list = capkpi.get_all("Student", limit=5)
...
class TestStudentGroup(unittest.TestCase):
	...
	def test_in_group(self):
		...
		self.assertRaises(
			erp.education.StudentNotInGroupError,
			erp.education.validate_student_belongs_to_group,
			last_student,
			doc.name,
		)

		# safe, don't throw validation
		erp.education.validate_student_belongs_to_group(doc.students[0].student, doc.name)
