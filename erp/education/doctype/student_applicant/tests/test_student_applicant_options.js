...
QUnit.test('test student applicant', function(assert){
	...
	let testing_status;
	capkpi.run_serially([
		() => capkpi.set_route('List', 'Student Applicant'),
		() => capkpi.timeout(0.5),
		...
		() => capkpi.timeout(0.5),
		() => capkpi.tests.click_button('Submit'),
		() => capkpi.tests.click_button('Yes'), // Submitting again after amend
		() => {
			testing_status = $('span.indicator.orange').text();
		...
		() => capkpi.tests.click_button('Update'),
		() => {
			testing_status = $('h1.editable-title').text();
		...
		() => capkpi.timeout(0.5),
		() => capkpi.tests.click_button('Enroll'),
		() => capkpi.timeout(0.5),
		() => {
			assert.ok(capkpi.get_route()[0] == 'Form'); // Checking if the current page is Program Enrollment page or not
			assert.ok(capkpi.get_route()[1] == 'Program Enrollment');
		},

		// Routing to Student List to check if the Applicant's entry has been made or not
		() => capkpi.timeout(0.5),
		() => capkpi.set_route('List', 'Student'),
		() => capkpi.timeout(0.5),
		() => {$(`a:contains("Fname Mname Lname"):visible`)[0].click();},
		() => capkpi.timeout(0.5),
