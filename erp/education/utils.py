...
def get_current_student():
	"""Returns current student from capkpi.session.user
...
def get_portal_programs():
	...
	"""
	published_programs = capkpi.get_all("Program", filters={"is_published": True})
	if not published_programs:
		return None

	program_list = [capkpi.get_doc("Program", program) for program in published_programs]
	portal_programs = [
...
def has_super_access():
	...
	"""
	current_user = capkpi.get_doc("User", capkpi.session.user)
...
def add_activity(course, content_type, content, program):
	...
	if not student:
		return capkpi.throw(
			_("Student with email {0} does not exist").format(capkpi.session.user), capkpi.DoesNotExistError
		)
...
def evaluate_quiz(quiz_response, quiz_name, course, program, time_taken):
	...
	quiz = capkpi.get_doc("Quiz", quiz_name)
...
def get_quiz(quiz_name, course):
	try:
		...
		capkpi.throw(_("Quiz {0} does not exist").format(quiz_name), capkpi.DoesNotExistError)
		return None
...
def get_course_progress(course, program):
	...
	for course_topic in course.topics:
		topic = capkpi.get_doc("Topic", course_topic.topic)
...
def get_program_progress(program):
	...
	for program_course in program.courses:
		course = capkpi.get_doc("Course", program_course.course)
...
def get_program_completion(program):
	...
	progress = []
	for topic in topics:
		topic_doc = capkpi.get_doc("Topic", topic[0])
...
def get_or_create_course_enrollment(course, program):
	...
	if not course_enrollment:
		...
		if not program_enrollment:
			capkpi.throw(_("You are not enrolled in program {0}").format(program))
			return
		return student.enroll_in_course(
			course_name=course, program_enrollment=get_enrollment("program", program.name, student.name)
		)
	else:
		return capkpi.get_doc("Course Enrollment", course_enrollment)
