...
def update_shortfall_status(loan, security_value, on_cancel=0):
	loan_security_shortfall = capkpi.db.get_value(
...
def add_security(loan):
	...
	loan_security_pledge = capkpi.new_doc("Loan Security Pledge")
...
def check_for_ltv_shortfall(process_loan_security_shortfall):
	...
	loan_shortfall_map = capkpi._dict(
		capkpi.get_all(
...
def create_loan_security_shortfall(
	...
	existing_shortfall = capkpi.db.get_value(
		"Loan Security Shortfall", {"loan": loan, "status": "Pending"}, "name"
	)

	if existing_shortfall:
		ltv_shortfall = capkpi.get_doc("Loan Security Shortfall", existing_shortfall)
	else:
		ltv_shortfall = capkpi.new_doc("Loan Security Shortfall")
...
def get_ltv_ratio(loan_security):
	loan_security_type = capkpi.db.get_value("Loan Security", loan_security, "loan_security_type")
	ltv_ratio = capkpi.db.get_value("Loan Security Type", loan_security_type, "loan_to_value_ratio")
	return ltv_ratio
