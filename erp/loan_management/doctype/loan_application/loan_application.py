...
from capkpi.utils import cint, flt, rounded
...
class LoanApplication(Document):
	...
	def validate_loan_type(self):
		company = capkpi.get_value("Loan Type", self.loan_type, "company")
		if company != self.company:
			capkpi.throw(_("Please select Loan Type for company {0}").format(capkpi.bold(self.company)))

	def validate_loan_amount(self):
		if not self.loan_amount:
			capkpi.throw(_("Loan Amount is mandatory"))

		maximum_loan_limit = capkpi.db.get_value("Loan Type", self.loan_type, "maximum_loan_amount")
	...
	def set_pledge_amount(self):
		for proposed_pledge in self.proposed_pledges:
			...
			if not proposed_pledge.qty and not proposed_pledge.amount:
				capkpi.throw(_("Qty or Amount is mandatroy for loan security"))
			...
			if self.repayment_method == "Repay Fixed Amount per Period":
				...
				if monthly_interest_rate:
					...
					if self.repayment_amount - min_repayment_amount <= 0:
						capkpi.throw(_("Repayment Amount must be greater than " + str(flt(min_repayment_amount, 2))))
	...
	def set_loan_amount(self):
		if self.is_secured_loan and not self.proposed_pledges:
			capkpi.throw(_("Proposed Pledges are mandatory for secured Loans"))
...
def create_loan(source_name, target_doc=None, submit=0):
	def update_accounts(source_doc, target_doc, source_parent):
		account_details = capkpi.get_all(
			"Loan Type",
			fields=[
				"mode_of_payment",
				"payment_account",
...
def create_pledge(loan_application, loan=None):
	loan_application_doc = capkpi.get_doc("Loan Application", loan_application)
...
def get_proposed_pledge(securities):
	...
	for security in securities:
		security = capkpi._dict(security)
