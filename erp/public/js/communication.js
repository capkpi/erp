capkpi.ui.form.on("Communication", {
	...
	setup_custom_buttons: (frm) => {
		...
		if(frm.doc.reference_doctype !== "Issue") {
			frm.add_custom_button(__("Issue"), () => {
				capkpi.confirm(__(confirm_msg, [__("Issue")]), () => {
		...
		if(!in_list(["Lead", "Opportunity"], frm.doc.reference_doctype)) {
			frm.add_custom_button(__("Lead"), () => {
				capkpi.confirm(__(confirm_msg, [__("Lead")]), () => {
					frm.trigger('make_lead_from_communication');
				})
			}, __('Create'));

			frm.add_custom_button(__("Opportunity"), () => {
				capkpi.confirm(__(confirm_msg, [__("Opportunity")]), () => {
	...
	make_lead_from_communication: (frm) => {
		return capkpi.call({
			method: "erp.crm.doctype.lead.lead.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return capkpi.call({
			method: "erp.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name
			},
			freeze: true,
	...
	make_opportunity_from_communication: (frm) => {
		...
		capkpi.prompt(fields, data => {
			capkpi.call({
				method: "erp.crm.doctype.opportunity.opportunity.make_opportunity_from_communication",
				args: {
					communication: frm.doc.name,
