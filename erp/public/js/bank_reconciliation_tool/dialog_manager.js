...
erp.accounts.bank_reconciliation.DialogManager = class DialogManager {
	...
	get_linked_vouchers(document_types) {
		capkpi.call({
			method:
				"erp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_linked_payments",
			args: {
	...
	get_datatable(proposals_wrapper) {
		if (!this.datatable) {
			...
			this.datatable = new capkpi.DataTable(
				...
				get_query: function () {
					return {
						filters: {
							name: [
								"in",
								Object.keys(capkpi.boot.party_account_types),
							],
						},
					};
				},
	...
	match() {
		...
		capkpi.call({
			method:
				"erp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.reconcile_vouchers",
			args: {
			...
			callback: (response) => {
				const alert_string =
					...
					" Matched";
				capkpi.show_alert(alert_string);
	...
	add_payment_entry(values) {
		capkpi.call({
			method:
				"erp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.create_payment_entry_bts",
			args: {
				bank_transaction_name: this.bank_transaction.name,
	...
	add_journal_entry(values) {
		capkpi.call({
			method:
				"erp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.create_journal_entry_bts",
			args: {
				bank_transaction_name: this.bank_transaction.name,
				reference_number: values.reference_number,
				reference_date: values.reference_date,
	...
	update_transaction(values) {
		capkpi.call({
			method:
				"erp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.update_bank_transaction",
			args: {
				bank_transaction_name: this.bank_transaction.name,
				reference_number: values.reference_number,
				party_type: values.party_type,
