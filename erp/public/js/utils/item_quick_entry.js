...
capkpi.ui.form.ItemQuickEntryForm = capkpi.ui.form.QuickEntryForm.extend({
	...
	check_naming_series_based_on: function() {
		if (capkpi.defaults.get_default("item_naming_by") === "Naming Series") {
			this.mandatory = this.mandatory.filter(d => d.fieldname !== "item_code");
		}
	},

	init_post_render_dialog_operations: function() {
		this.dialog.fields_dict.attribute_html.$wrapper.append(capkpi.render_template("item_quick_entry"));
		...
		return new Promise(resolve => {
			capkpi.call({
				...
				callback: function(r) {
					me.dialog.hide();
					// delete the old doc
					capkpi.model.clear_doc(me.dialog.doc.doctype, me.dialog.doc.name);
					me.dialog.doc = r.message;
					if (capkpi._from_link) {
						capkpi.ui.form.update_calling_link(me.dialog.doc);
					} else {
						if (me.after_insert) {
	...
	open_doc: function() {
		...
		if (this.dialog.fields_dict.create_variant.$input.prop("checked")) {
			...
			if (template)
				capkpi.set_route("Form", this.doctype, template);
		} else {
			capkpi.set_route('Form', this.doctype, this.doc.name);
		}
	},
		...
		$.each(attributes, function(index, row) {
			...
			if (!row.numeric_values) {
				...
				me[row.attribute].$input.on("change", function() {
					...
					$(this).closest(".capkpi-control").toggleClass("has-error", $(this).val() ? false : true);
				});
			}
		});
	},
	...
	init_make_control: function(fieldtype, row) {
		this[row.attribute] = capkpi.ui.form.make_control({
			df: {
				"fieldtype": fieldtype,
				"label": row.attribute,
	...
	init_awesomplete_for_attribute: function(row) {
		...
		this[row.attribute].$input.on('input', function(e) {
			...
			$(e.target).closest(".capkpi-control").toggleClass("has-error", e.target.value ? false : true);
		});
	},
	...
	get_variant_doc: function() {
		...
		if (Object.keys(attribute).length) {
			capkpi.call({
				method: "erp.controllers.item_variant.create_variant_doc_for_quick_entry",
				args: {
				...
				callback: function(r) {
					if (Object.prototype.toString.call(r.message) == "[object Object]") {
						...
						var msgprint_dialog = capkpi.msgprint(__("Item Variant {0} already exists with same attributes", [repl('<a class="strong variant-click" data-item-code="%(item)s" \
								>%(item)s</a>', {
								item: r.message
							})]));
						...
						msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							...
							me.dialog.hide();
							if (capkpi._from_link) {
								capkpi._from_link.set_value($(this).attr("data-item-code"));
							} else {
								capkpi.set_route('Form', "Item", $(this).attr("data-item-code"));
							}
						});
					}
				}
