...
erp.HierarchyChart = class {
	...
	*/
	...
	make_node_element(node) {
		let node_card = capkpi.render_template('node_card', {
			id: node.id,
			name: node.name,
			title: node.title,
			image: node.image,
		...
		let company = this.page.add_field({
			...
			change: () => {
				...
				if (company.get_value()) {
					...
					capkpi.throw(__('Please select a company first.'));
				}
			}
		});
	...
	export_chart() {
		capkpi.dom.freeze(__('Exporting...'));
		this.page.main.css({
			'min-height': '',
			'max-height': '',
			'overflow': 'visible',
	...
	get_all_nodes() {
		...
		return new Promise(resolve => {
			capkpi.call({
				method: 'erp.utilities.hierarchy_chart.get_all_nodes',
				args: {
					method: me.method,
					company: me.company
				},
	...
	setup_edit_node_action(node) {
		...
		node_element.find('.btn-edit-node').click(function() {
			capkpi.set_route('Form', me.doctype, node.id);
		});
	}
