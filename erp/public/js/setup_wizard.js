capkpi.provide("erp.setup");

capkpi.pages['setup-wizard'].on_page_load = function(wrapper) {
	if(capkpi.sys_defaults.company) {
		capkpi.set_route("desk");
		return;
	}
};

capkpi.setup.on("before_load", function () {
	erp.setup.slides_settings.map(capkpi.setup.add_slide);
});

erp.setup.slides_settings = [
	{
		// Domain
		name: 'domain',
		...
		validate: function () {
			if (this.values.domains.length === 0) {
				capkpi.msgprint(__("Please select at least one domain."));
				return false;
			}
			capkpi.setup.domains = this.values.domains;
			return true;
		},
	},

	{
		...
		fields: [
			...
			},
			{
				...
				label: capkpi.setup.domains.includes('Education') ?
		...
		bind_events: function (slide) {
			...
			}).val(capkpi.boot.sysdefaults.company_name || "").trigger("change");

			slide.get_input("company_abbr").on("change", function () {
				if (slide.get_input("company_abbr").val().length > 5) {
					capkpi.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
		...
		validate: function() {
			if ((this.values.company_name || "").toLowerCase() == "company") {
				capkpi.msgprint(__("Company Name cannot be Company"));
				return false;
			}
	...
	},
	{
		...
		icon: "fa fa-building",
		// help: capkpi.setup.domains.includes('Education') ?
		...
		fields: [
			{
				...
				fieldtype: 'Data',
				placeholder: capkpi.setup.domains.includes('Education') ?
		...
		validate: function () {
			...
			if(me.values.bank_account) {
				capkpi.call({
					async: false,
					method: "erp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
					args: {
		...
		validate_fy_dates: function() {
			...
			if (invalid || start_greater_than_end) {
				capkpi.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = capkpi.wizard.values.country;

			if (country) {
				var fy = erp.setup.fiscal_years[country];
				...
				if (year_start_date > capkpi.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date =
					capkpi.datetime.add_days(capkpi.datetime.add_months(start_date, 12), -1);
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new capkpi.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: 'erp.accounts.utils.get_coa',
				args: {
			...
			const buttons = $(form_container).find('.capkpi-control');
