...
{% include 'erp/accounts/doctype/purchase_taxes_and_charges_template/purchase_taxes_and_charges_template.js' %}

cur_frm.email_field = "contact_email";

erp.buying.BuyingController = erp.TransactionController.extend({
	setup: function() {
		this._super();
	},
	...
	onload: function(doc, cdt, cdn) {
		...
		if (this.frm.doc.__islocal
			&& capkpi.meta.has_field(this.frm.doc.doctype, "disable_rounded_total")) {

				var df = capkpi.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total");
				var disable = cint(df.default) || cint(capkpi.sys_defaults.disable_rounded_total);
		...
		if(this.frm.get_field('shipping_address')) {
			this.frm.set_query("shipping_address", function() {
				if(me.frm.doc.customer) {
					...
					return erp.queries.company_address_query(me.frm.doc)
			});
		}
		/* eslint-enable */
	},
	...
	setup_queries: function(doc, cdt, cdn) {
		...
		me.frm.set_query('supplier', erp.queries.supplier);
		me.frm.set_query('contact_person', erp.queries.contact_query);
		me.frm.set_query('supplier_address', erp.queries.address_query);

		me.frm.set_query('billing_address', erp.queries.company_address_query);
		erp.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);

		if(this.frm.fields_dict.supplier) {
			this.frm.set_query("supplier", function() {
				return{	query: "erp.controllers.queries.supplier_query" }});
		}
		...
		this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
			...
			return {
				query: "erp.controllers.queries.item_manufacturer_query",
	...
	supplier: function() {
		var me = this;
		erp.utils.get_party_details(this.frm, null, null, function(){
			me.apply_price_list();
		});
	},

	supplier_address: function() {
		erp.utils.get_address_display(this.frm);
		erp.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
	},
	...
	calculate_received_qty(doc, cdt, cdn){
		var item = capkpi.get_doc(cdt, cdn);
		capkpi.model.round_floats_in(item, ["qty", "rejected_qty"]);
		...
		capkpi.model.set_value(cdt, cdn, "received_qty", received_qty);
		capkpi.model.set_value(cdt, cdn, "received_stock_qty", received_stock_qty);
	},
	...
	validate_negative_quantity: function(cdt, cdn, item, fieldnames){
		...
		for(var i = 0; i<fieldnames.length; i++) {
			if(item[fieldnames[i]] < 0){
				capkpi.msgprint(__("Row #{0}: {1} can not be negative for item {2}", [item.idx,__(capkpi.meta.get_label(cdt, fieldnames[i], cdn)), item.item_code]));
	...
	warehouse: function(doc, cdt, cdn) {
		...
		if(item.item_code && item.warehouse) {
			return this.frm.call({
				method: "erp.stock.get_item_details.get_bin_details",
				child: item,
				args: {
	...
	set_from_product_bundle: function() {
		...
		this.frm.add_custom_button(__("Product Bundle"), function() {
			erp.buying.get_items_from_product_bundle(me.frm);
		}, __("Get Items From"));
	},

	shipping_address: function(){
		var me = this;
		erp.utils.get_address_display(this.frm, "shipping_address",
			"shipping_address_display", true);
	},

	billing_address: function() {
		erp.utils.get_address_display(this.frm, "billing_address",
	...
	update_auto_repeat_reference: function(doc) {
		if (doc.auto_repeat) {
			capkpi.call({
				method:"capkpi.automation.doctype.auto_repeat.auto_repeat.update_reference",
				args:{
				...
				callback: function(r){
					if (r.message=="success") {
						capkpi.show_alert({message:__("Auto repeat document updated"), indicator:'green'});
					} else {
						capkpi.show_alert({message:__("An error occurred during the update process"), indicator:'red'});
					}
				}
			})
		}
	...
	manufacturer: function(doc, cdt, cdn) {
		...
		if(row.manufacturer) {
			capkpi.call({
				method: "erp.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
				args: {
				...
				callback: function(r) {
					if (r.message) {
						capkpi.model.set_value(cdt, cdn, 'manufacturer_part_no', r.message);
					}
				}
			});
		}
	...
	manufacturer_part_no: function(doc, cdt, cdn) {
		...
		if (row.manufacturer_part_no) {
			capkpi.model.get_value('Item Manufacturer',
				{
					'item_code': row.item_code,
					'manufacturer': row.manufacturer,
...
erp.buying.link_to_mrs = function(frm) {
	capkpi.call({
		method: "erp.buying.utils.get_linked_material_requests",
		args:{
			...
			for (let item of frm.doc.items) {
				...
				(r.message[0] || []).forEach(function(d) {
					if (d.qty > 0 && qty > 0 && item.item_code == d.item_code && !item.material_request_item)
					{
						...
						capkpi.msgprint("Assigning " + d.mr_name + " to " + d.item_code + " (row " + item.idx + ")");
						if (qty > 0)
						{
							capkpi.msgprint("Splitting " + qty + " units of " + d.item_code);
							var newrow = capkpi.model.add_child(frm.doc, item.doctype, "items");
							item_length++;
...
erp.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return capkpi.call({
				type: "GET",
				method: "erp.stock.get_item_details.get_default_bom",
				args: {
					"item_code": d.item_code,
				},
				callback: function(r) {
					if(r) {
						capkpi.model.set_value(d.doctype, d.name, "bom", r.message);
					}
				}
			})
		}
...
erp.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new capkpi.ui.Dialog({
		...
		primary_action(args){
			...
			return capkpi.call({
				type: "GET",
				method: "erp.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
				...
				callback: function(r) {
					...
					if(!r.exc && r.message) {
						...
						for (var i=0; i< r.message.length; i++) {
							...
							if(capkpi.meta.get_docfield(d.doctype, "price_list_rate", d.name)) {
