...
// get tax rate
capkpi.provide("erp.taxes");
capkpi.provide("erp.taxes.flags");
...
capkpi.ui.form.on(cur_frm.doctype, {
	setup: function(frm) {
		...
		$(frm.wrapper).on('grid-row-render', function(e, grid_row) {
			if(in_list(['Sales Taxes and Charges', 'Purchase Taxes and Charges'], grid_row.doc.doctype)) {
				erp.taxes.set_conditional_mandatory_rate_or_amount(grid_row);
			}
		});
	},
	onload: function(frm) {
	...
	taxes_on_form_rendered: function(frm) {
		erp.taxes.set_conditional_mandatory_rate_or_amount(frm.open_grid_row());
	},
...
capkpi.ui.form.on('Sales Invoice Payment', {
	mode_of_payment: function(frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		get_payment_mode_account(frm, d.mode_of_payment, function(account){
			capkpi.model.set_value(cdt, cdn, 'account', account)
		})
	}
});
...
capkpi.ui.form.on("Payment Schedule", {
...
capkpi.ui.form.on('Salary Structure', {
	mode_of_payment: function(frm) {
		get_payment_mode_account(frm, frm.doc.mode_of_payment, function(account){
...
var get_payment_mode_account = function(frm, mode_of_payment, callback) {
	...
	return  capkpi.call({
		method: "erp.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
		args: {
...
cur_frm.cscript.account_head = function(doc, cdt, cdn) {
	...
	if(!d.charge_type && d.account_head){
		capkpi.msgprint(__("Please select Charge Type first"));
		capkpi.model.set_value(cdt, cdn, "account_head", "");
	} else if (d.account_head) {
		capkpi.call({
			type:"GET",
			method: "erp.controllers.accounts_controller.get_tax_rate",
			args: {"account_head":d.account_head},
			callback: function(r) {
				if (d.charge_type!=="Actual") {
					capkpi.model.set_value(cdt, cdn, "rate", r.message.tax_rate || 0);
				}
				capkpi.model.set_value(cdt, cdn, "description", r.message.account_name);
			}
		})
	}
}
...
cur_frm.cscript.validate_inclusive_tax = function(tax) {
	...
	if(cint(tax.included_in_print_rate)) {
		if(tax.charge_type == "Actual") {
			...
			capkpi.throw(__("Valuation type charges can not marked as Inclusive"));
		}
	}
}

if(!erp.taxes.flags[cur_frm.cscript.tax_table]) {
	erp.taxes.flags[cur_frm.cscript.tax_table] = true;

	capkpi.ui.form.on(cur_frm.cscript.tax_table, "row_id", function(frm, cdt, cdn) {
		cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
	});

	capkpi.ui.form.on(cur_frm.cscript.tax_table, "rate", function(frm, cdt, cdn) {
		cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
	});

	capkpi.ui.form.on(cur_frm.cscript.tax_table, "tax_amount", function(frm, cdt, cdn) {
		cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
	});

	capkpi.ui.form.on(cur_frm.cscript.tax_table, "charge_type", function(frm, cdt, cdn) {
		frm.cscript.validate_taxes_and_charges(cdt, cdn);
		var open_form = frm.open_grid_row();
		if(open_form) {
			erp.taxes.set_conditional_mandatory_rate_or_amount(open_form);
		} else {
			// apply in current row
			erp.taxes.set_conditional_mandatory_rate_or_amount(frm.get_field('taxes').grid.get_row(cdn));
		}
	});

	capkpi.ui.form.on(cur_frm.cscript.tax_table, "included_in_print_rate", function(frm, cdt, cdn) {
		var tax = capkpi.get_doc(cdt, cdn);
		try {
			cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
...
erp.taxes.set_conditional_mandatory_rate_or_amount = function(grid_row) {
	if(grid_row) {
