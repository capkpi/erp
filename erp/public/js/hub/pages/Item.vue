...
export default {
	...
	data() {
		return {
			page_name: capkpi.get_route()[1],
			hub_item_name: capkpi.get_route()[2],
	...
	mounted() {
		...
		// erp.hub.item_view_cache = erp.hub.item_view_cache || [];
		// if (erp.hub.item_view_cache.includes(this.hub_item_name)) {
		// 	return;
		// }
		...
		this.item_received.then(() => {
			setTimeout(() => {
				...
				// .then(() => {
				// 	erp.hub.item_view_cache.push(this.hub_item_name);
				// });
			}, 5000);
		});
	},
	methods: {
		...
		go_to_seller_profile_page(seller_name) {
			capkpi.set_route(`marketplace/seller/${seller_name}`);
		},
		build_data() {
		...
		add_to_saved_items() {
			hub.call('add_item_to_user_saved_items', {
					hub_item_name: this.hub_item_name,
					hub_user: capkpi.session.user
				})
				.then(() => {
					const saved_items_link = `<b><a href="#marketplace/saved-items">${__('Saved')}</a></b>`;
					capkpi.show_alert(saved_items_link);
					erp.hub.trigger('action:item_save');
				})
				.catch(e => {
					console.error(e);
				});
		...
		add_to_featured_items() {
			hub.call('add_item_to_seller_featured_items', {
				...
				.then(() => {
					...
					capkpi.show_alert(featured_items_link);
					erp.hub.trigger('action:item_feature');
				})
				.catch(e => {
					console.error(e);
				});
		...
		make_contact_seller_dialog() {
			this.contact_seller_dialog = new capkpi.ui.Dialog({
				...
				primary_action: ({ message }) => {
					...
					hub.call('send_message', {
						...
						.then(() => {
							...
							capkpi.set_route('marketplace', 'buying', this.item.name);
							erp.hub.trigger('action:send_message');
						});
				}
			});
		},
		...
		make_report_item_dialog() {
			this.report_item_dialog = new capkpi.ui.Dialog({
				...
				primary_action: ({ message }) => {
					hub.call('add_reported_item', {
						...
						.then(() => {
							d.hide();
							capkpi.show_alert(__('Item Reported'));
						});
				}
			});
		},
		...
		update_details(values) {
			capkpi.call('erp.hub_node.api.update_item', {
				...
				.then(() => {
					capkpi.show_alert(__('{0} Updated', [this.item.item_name]));
				});
		},
		...
		unpublish_item() {
			capkpi.confirm(__('Unpublish {0}?', [this.item.item_name]), () => {
				capkpi
					.call('erp.hub_node.api.unpublish_item', {
						item_code: this.item.item_code,
						hub_item_name: this.hub_item_name
					})
					.then(r => {
						capkpi.set_route(`marketplace/home`);
						capkpi.show_alert(__('Item listing removed'));
					});
			});
		}
	}
