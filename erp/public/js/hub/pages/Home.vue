...
export default {
	...
	mounted() {
		capkpi.route.on('change', () => {
			if (capkpi.get_route_str() === 'marketplace/home') {
				this.get_items();
			}
		})
	},
	methods: {
		get_items() {
			hub.call('get_data_for_homepage', capkpi.defaults ? {
				country: capkpi.defaults.get_user_default('country')
			} : null)
			.then((data) => {
		...
		go_to_item_details_page(hub_item_name) {
			capkpi.set_route(`marketplace/item/${hub_item_name}`);
		},

		set_search_route() {
			capkpi.set_route('marketplace', 'search', 'All', this.search_value);
		},
	}
}
</script>
