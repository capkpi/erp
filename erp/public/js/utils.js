...
$.extend(erp, {
	get_currency: function(company) {
		...
		if(company)
			return capkpi.get_doc(":Company", company).default_currency || capkpi.boot.sysdefaults.currency;
		else
			return capkpi.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = capkpi.boot.docs;
	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			} else if(erp.last_selected_company) {
				if(!cur_frm.doc.company) cur_frm.set_value("company", erp.last_selected_company);
			}
		}
	},
	...
	is_perpetual_inventory_enabled: function(company) {
		if(company) {
			return capkpi.get_doc(":Company", company).enable_perpetual_inventory
		}
	},

	stale_rate_allowed: () => {
		return cint(capkpi.boot.sysdefaults.allow_stale);
	},
	...
	setup_serial_or_batch_no: function() {
		...
		capkpi.model.get_value('Item', {'name': grid_row.doc.item_code},
	...
	route_to_adjustment_jv: (args) => {
		capkpi.model.with_doctype('Journal Entry', () => {
			// route to adjustment Journal Entry to handle Account Balance and Stock Value mismatch
			let journal_entry = capkpi.model.get_new_doc('Journal Entry');

			args.accounts.forEach((je_account) => {
				let child_row = capkpi.model.add_child(journal_entry, "accounts");
			...
			capkpi.set_route('Form','Journal Entry', journal_entry.name);
		});
	},

	route_to_pending_reposts: (args) => {
		capkpi.set_route('List', 'Repost Item Valuation', args);
	},

	proceed_save_with_reminders_frequency_change: () => {
		capkpi.ui.hide_open_dialog();

		capkpi.call({
			method: 'erp.hr.doctype.hr_settings.hr_settings.set_proceed_with_frequency_change',
			callback: () => {
				cur_frm.save();
			}
		});
...
$.extend(erp.utils, {
		...
		if(frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			...
			if(company_wise_info.length > 1) {
				company_wise_info.forEach(function(info) {
					erp.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	make_bank_account: function(doctype, docname) {
		capkpi.call({
			method: "erp.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	add_dimensions: function(report_name, index) {
		let filters = capkpi.query_reports[report_name].filters;
					...
					if (!found) {
						filters.splice(index, 0, {
							...
							get_data: function(txt) {
								return capkpi.db.get_link_options(dimension["document_type"], txt);
							},
						});
					}
				});
	...
	make_subscription: function(doctype, docname) {
		capkpi.call({
			method: "capkpi.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	make_pricing_rule: function(doctype, docname) {
		capkpi.call({
			method: "erp.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	**/
	...
	get_tree_options: function(option) {
		...
		let unscrub_option = capkpi.model.unscrub(option);
		let user_permission = capkpi.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function(option) {
		...
		if(options.includes(capkpi.defaults.get_default(option))) {
			return capkpi.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
	overrides_parent_value_in_all_rows: function(doc, dt, dn, table_fieldname, fieldname, parent_fieldname) {
		if (doc[parent_fieldname]) {
			...
			capkpi.refresh_field(table_fieldname);
		}
	},
	create_new_doc: function (doctype, update_fields) {
		capkpi.model.with_doctype(doctype, function() {
			var new_doc = capkpi.model.get_new_doc(doctype);
...
erp.utils.select_alternate_items = function(opts) {
	const frm = opts.frm;
	...
	const dialog = new capkpi.ui.Dialog({
				...
				fields: [{
					...
					onchange: function() {
						...
						if (item_code && warehouse) {
							capkpi.call({
								method: "erp.stock.utils.get_latest_stock_qty",
								args: {
									item_code: item_code,
									warehouse: warehouse
								},
					...
					get_query: (e) => {
						return {
							query: "erp.stock.doctype.item_alternative.item_alternative.get_alternative_items",
							filters: {
		...
		primary_action: function() {
			...
			alternative_items.forEach(d => {
				let row = capkpi.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				capkpi.model.set_value(row.doctype, row.name, 'qty', qty);
				capkpi.model.set_value(row.doctype, row.name, opts.original_item_field, d.item_code);
...
erp.utils.update_child_items = function(opts) {
	const frm = opts.frm;
	const cannot_add_row = (typeof opts.cannot_add_row === 'undefined') ? true : opts.cannot_add_row;
	const child_docname = (typeof opts.cannot_add_row === 'undefined') ? "items" : opts.child_docname;
	const child_meta = capkpi.get_meta(`${frm.doc.doctype} Item`);
	...
	const dialog = new capkpi.ui.Dialog({
		...
		primary_action: function() {
			...
			capkpi.call({
				method: 'erp.controllers.accounts_controller.update_child_qty_rate',
				freeze: true,
				args: {
...
erp.utils.map_current_doc = function(opts) {
	function _map() {
		if($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = capkpi.meta.get_docfield(cur_frm.doctype, 'items').options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			capkpi.get_meta(items_doctype).fields.forEach(function(d) {
			...
			if(already_set) {
				opts.source_name.forEach(function(src) {
					capkpi.model.with_doc(opts.source_doctype, src, function(r) {
						var source_doc = capkpi.model.get_doc(opts.source_doctype, src);
					...
					if(already_set) {
						capkpi.msgprint(__("You have already selected items from {0} {1}",
		...
		return capkpi.call({
			...
			type: "POST",
			method: 'capkpi.model.mapper.map_docs',
			args: {
				"method": opts.method,
	...
	if (opts.source_doctype) {
		const d = new capkpi.ui.form.MultiSelectDialog({
			...
			action: function(selections, args) {
				...
				if (values.length === 0) {
					capkpi.msgprint(__("Please select {0}", [opts.source_doctype]))
					return;
				}
...
capkpi.form.link_formatters['Item'] = function(value, doc) {
...
capkpi.form.link_formatters['Employee'] = function(value, doc) {
...
capkpi.form.link_formatters['Project'] = function(value, doc) {
