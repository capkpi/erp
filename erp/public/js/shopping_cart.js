...
// shopping cart
capkpi.provide("erp.e_commerce.shopping_cart");
var shopping_cart = erp.e_commerce.shopping_cart;
...
capkpi.ready(function() {
	var full_name = capkpi.session && capkpi.session.user_fullname;
	// update user
	if(full_name) {
	...
	referral_coupon_code=capkpi.get_cookie("referral_coupon_code");
	referral_sales_partner=capkpi.get_cookie("referral_sales_partner");
...
$.extend(shopping_cart, {
	show_shoppingcart_dropdown: function() {
		$(".shopping-cart").on('shown.bs.dropdown', function() {
			if (!$('.shopping-cart-menu .cart-container').length) {
				return capkpi.call({
					method: 'erp.e_commerce.shopping_cart.cart.get_shopping_cart_menu',
					callback: function(r) {
						if (r.message) {
	...
	update_cart: function(opts) {
		if (capkpi.session.user==="Guest") {
			...
			capkpi.call('erp.e_commerce.api.get_guest_redirect_on_action').then((res) => {
			...
			return capkpi.call({
				type: "POST",
				method: "erp.e_commerce.shopping_cart.cart.update_cart",
				args: {
	...
	set_cart_count: function(animate=false) {
		...
		var cart_count = capkpi.get_cookie("cart_count");
		if(capkpi.session.user==="Guest") {
			cart_count = 0;
		}
	...
	show_cart_navbar: function () {
		capkpi.call({
			method: "erp.e_commerce.doctype.e_commerce_settings.e_commerce_settings.is_cart_enabled",
			callback: function(r) {
	...
	bind_add_to_cart_action() {
		$('.page_content').on('click', '.btn-add-to-cart-list', (e) => {
			...
			erp.e_commerce.shopping_cart.update_cart({
				item_code,
				qty: 1
			});
