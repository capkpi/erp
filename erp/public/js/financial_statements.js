capkpi.provide("erp.financial_statements");
...
erp.financial_statements = {
	...
	"formatter": function(value, row, column, data, default_formatter) {
		if (data && column.fieldname=="account") {
			...
			column.link_onclick =
				"erp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			column.is_tree = true;
		}
	...
	"open_general_ledger": function(data) {
		...
		var project = $.grep(capkpi.query_report.filters, function(e){ return e.df.fieldname == 'project'; })
	...
	onload: function(report) {
		...
		erp.financial_statements.filters = get_filters()
		...
		report.page.add_custom_menu_item(views_menu, __("Balance Sheet"), function() {
			...
			capkpi.set_route('query-report', 'Balance Sheet', {company: filters.company});
		});

		report.page.add_custom_menu_item(views_menu, __("Profit and Loss"), function() {
			var filters = report.get_values();
			capkpi.set_route('query-report', 'Profit and Loss Statement', {company: filters.company});
		});

		report.page.add_custom_menu_item(views_menu, __("Cash Flow Statement"), function() {
			var filters = report.get_values();
			capkpi.set_route('query-report', 'Cash Flow', {company: filters.company});
		});
	}
};
