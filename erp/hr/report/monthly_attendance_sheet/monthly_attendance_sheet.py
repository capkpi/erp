...
def execute(filters=None):
	...
	default_holiday_list = capkpi.get_cached_value(
	...
	if filters.summarized_view:
		leave_types = capkpi.get_all("Leave Type", pluck="name")
...
def add_data(
	...
	for emp in employee_map:
		...
		if filters.summarized_view:
			...
			time_default_counts = capkpi.db.sql(
...
def get_attendance_list(conditions, filters):
	...
	for d in attendance_list:
		att_map.setdefault(d.employee, capkpi._dict()).setdefault(d.day_of_month, "")
...
def get_employee_details(group_by, company):
	...
	query = """select name, employee_name, designation, department, branch, company,
		holiday_list from `tabEmployee` where company = %s """ % capkpi.db.escape(
		company
	)
	...
	employee_details = capkpi.db.sql(query, as_dict=1)
