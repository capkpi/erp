...
from erp.hr.doctype.vehicle_log.test_vehicle_log import get_vehicle, make_vehicle_log
from erp.hr.doctype.vehicle_log.vehicle_log import make_expense_claim
from erp.hr.report.vehicle_expenses.vehicle_expenses import execute


class TestVehicleExpenses(unittest.TestCase):
	@classmethod
	def setUpClass(self):
		capkpi.db.sql("delete from `tabVehicle Log`")

		employee_id = capkpi.db.sql(
	...
	def test_vehicle_expenses_based_on_fiscal_year(self):
		...
		# clean up
		vehicle_log.cancel()
		capkpi.delete_doc("Expense Claim", expense_claim.name)
		capkpi.delete_doc("Vehicle Log", vehicle_log.name)

	def tearDown(self):
		capkpi.delete_doc("Vehicle", self.license_plate, force=1)
		capkpi.delete_doc("Employee", self.employee_id, force=1)
