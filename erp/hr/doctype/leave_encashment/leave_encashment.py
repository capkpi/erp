...
from erp.hr.doctype.leave_application.leave_application import get_leaves_for_period
...
class LeaveEncashment(Document):
	...
	def before_submit(self):
		if self.encashment_amount <= 0:
			capkpi.throw(_("You can only submit Leave Encashment for a valid encashment amount"))
	...
	def on_submit(self):
		...
		additional_salary.company = capkpi.get_value("Employee", self.employee, "company")
		additional_salary.employee = self.employee
		additional_salary.currency = self.currency
		earning_component = capkpi.get_value("Leave Type", self.leave_type, "earning_component")
		if not earning_component:
			capkpi.throw(_("Please set Earning Component for Leave type: {0}.").format(self.leave_type))
		...
		capkpi.db.set_value(
			...
			capkpi.db.get_value("Leave Allocation", self.leave_allocation, "total_leaves_encashed")
	...
	def get_leave_details_for_encashment(self):
		...
		if not capkpi.db.get_value("Leave Type", self.leave_type, "allow_encashment"):
			capkpi.throw(_("Leave Type {0} is not encashable").format(self.leave_type))
		...
		encashable_days = self.leave_balance - capkpi.db.get_value(
			"Leave Type", self.leave_type, "encashment_threshold_days"
		)
		self.encashable_days = encashable_days if encashable_days > 0 else 0

		per_day_encashment = capkpi.db.get_value(
