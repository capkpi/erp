...
from capkpi.utils import add_days, get_year_ending, get_year_start, getdate
...
test_records = capkpi.get_test_records("Leave Type")


class TestLeaveEncashment(CapKPITestCase):
	def setUp(self):
		capkpi.db.delete("Leave Period")
		capkpi.db.delete("Leave Policy Assignment")
		capkpi.db.delete("Leave Allocation")
		capkpi.db.delete("Leave Ledger Entry")
		capkpi.db.delete("Additional Salary")
		capkpi.db.delete("Leave Encashment")

		if not capkpi.db.exists("Leave Type", "_Test Leave Type Encashment"):
			capkpi.get_doc(test_records[2]).insert()
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee], capkpi._dict(data)
		)
	...
	def test_leave_balance_value_and_amount(self):
		leave_encashment = capkpi.get_doc(
			dict(
		...
		# assert links
		add_sal = capkpi.get_all("Additional Salary", filters={"ref_docname": leave_encashment.name})[0]
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		self.assertFalse(
			capkpi.db.exists("Leave Ledger Entry", {"transaction_name": leave_encashment.name})
		)
