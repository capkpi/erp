...
QUnit.test("Test: Leave control panel [HR]", function (assert) {
	...
	capkpi.run_serially([
		...
		() => capkpi.set_route("Form", "Leave Control Panel"),
		() => capkpi.timeout(1),
		...
		() => cur_frm.set_value("from_date", capkpi.datetime.add_months(today_date, -2)),
		() => cur_frm.set_value("to_date", capkpi.datetime.add_days(today_date, -1)),	// for two months [not today]
		() => cur_frm.set_value("no_of_days", 3),
		// allocate leaves
		() => capkpi.click_button('Allocate'),
		() => capkpi.timeout(1),
