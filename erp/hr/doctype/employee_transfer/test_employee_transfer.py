...
class TestEmployeeTransfer(unittest.TestCase):
	...
	def test_submit_before_transfer_date(self):
		...
		transfer_obj = capkpi.get_doc(
			{
				"doctype": "Employee Transfer",
				"employee": capkpi.get_value("Employee", {"user_id": "employee2@transfers.com"}, "name"),
				"transfer_details": [
					{
		...
		transfer_obj.save()
		self.assertRaises(capkpi.DocstatusTransitionError, transfer_obj.submit)
		transfer = capkpi.get_doc("Employee Transfer", transfer_obj.name)
	...
	def test_new_employee_creation(self):
		...
		transfer = capkpi.get_doc(
			{
				"doctype": "Employee Transfer",
				"employee": capkpi.get_value("Employee", {"user_id": "employee3@transfers.com"}, "name"),
		...
		self.assertEqual(capkpi.get_value("Employee", transfer.new_employee_id, "status"), "Active")
		self.assertEqual(capkpi.get_value("Employee", transfer.employee, "status"), "Left")
...
def create_company():
	if not capkpi.db.exists("Company", "Test Company"):
		capkpi.get_doc(
			{
				"doctype": "Company",
