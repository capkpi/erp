...
capkpi.ui.form.on('Interview', {
	...
	refresh: function (frm) {
		if (frm.doc.docstatus != 2 && !frm.doc.__islocal) {
			...
			if ((allowed_interviewers.includes(capkpi.session.user))) {
				capkpi.db.get_value('Interview Feedback', {'interviewer': capkpi.session.user, 'interview': frm.doc.name, 'docstatus': 1}, 'name', (r) => {
					if (Object.keys(r).length === 0) {
						frm.add_custom_button(__('Submit Feedback'), function () {
							capkpi.call({
								method: 'erp.hr.doctype.interview.interview.get_expected_skill_set',
								args: {
	...
	show_feedback_dialog: function (frm, data) {
		...
		let d = new capkpi.ui.Dialog({
			...
			primary_action: function(values) {
				capkpi.call({
					method: 'erp.hr.doctype.interview.interview.create_interview_feedback',
					args: {
						data: values,
						interview_name: frm.doc.name,
						interviewer: capkpi.session.user,
	...
	interview_round: async function (frm) {
		...
		let round_data = (await capkpi.db.get_value('Interview Round', frm.doc.interview_round, 'designation')).message;
	...
	set_interview_details: function (frm) {
		capkpi.call({
			method: 'erp.hr.doctype.interview.interview.get_interviewers',
			args: {
				interview_round: frm.doc.interview_round
			},
	...
	job_applicant: function (frm) {
		if (!frm.doc.interview_round) {
			...
			frm.refresh();
			capkpi.throw(__('Select Interview Round First'));
		}
	...
	set_designation_and_job_opening: async function (frm) {
		...
		let job_applicant_data = (await capkpi.db.get_value(
