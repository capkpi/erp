...
from capkpi.utils import add_days, get_time, getdate, nowtime

from erp.hr.doctype.designation.test_designation import create_designation
from erp.hr.doctype.interview.interview import DuplicateInterviewRoundError
...
class TestInterview(unittest.TestCase):
	...
	def test_notification_for_scheduling(self):
		from erp.hr.doctype.interview.interview import send_interview_reminder
	...
	def test_notification_for_feedback_submission(self):
		from erp.hr.doctype.interview.interview import send_daily_feedback_reminder
...
def create_interview_round(name, skill_set, interviewers=[], designation=None, save=True):
	...
	interview_round = capkpi.new_doc("Interview Round")
...
def create_skill_set(skill_set):
	for skill in skill_set:
		if not capkpi.db.exists("Skill", skill):
			doc = capkpi.new_doc("Skill")
			doc.skill_name = skill
			doc.save()


def create_interview_type(name="test_interview_type"):
	if capkpi.db.exists("Interview Type", name):
		return capkpi.get_doc("Interview Type", name).name
	else:
		doc = capkpi.new_doc("Interview Type")
		doc.name = name
