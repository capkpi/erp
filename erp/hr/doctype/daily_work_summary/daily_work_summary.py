...
from capkpi.utils import global_date_format
...
class DailyWorkSummary(Document):
	...
	def get_message_details(self):
		...
		dws_group = capkpi.get_doc("Daily Work Summary Group", self.daily_work_summary_group)
		...
		for d in replies:
			...
			try:
				if original_image:
					file_name = capkpi.get_list("File", {"file_url": original_image})

					if file_name:
						file_name = file_name[0].name
						file_doc = capkpi.get_doc("File", file_name)
		...
		did_not_reply = [
			(capkpi.db.get_value("User", {"email": email}, "full_name") or email) for email in did_not_reply
		]
...
def get_user_emails_from_group(group):
	...
	if isinstance(group_doc, string_types):
		group_doc = capkpi.get_doc("Daily Work Summary Group", group)
...
def get_users_email(doc):
	return [d.email for d in doc.users if capkpi.db.get_value("User", d.user, "enabled")]
