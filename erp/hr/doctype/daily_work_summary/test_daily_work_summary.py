...
# test_records = capkpi.get_test_records('Daily Work Summary')
...
class TestDailyWorkSummary(unittest.TestCase):
	...
	def test_email_trigger_failed(self):
		hour = "00:00"
		if capkpi.utils.nowtime().split(":")[0] == "00":
			hour = "01:00"
	...
	def test_incoming(self):
		...
		daily_work_summary = capkpi.get_doc(
			"Daily Work Summary", capkpi.get_all("Daily Work Summary")[0].name
		)
	...
	def setup_and_prepare_test(self, hour=None):
		capkpi.db.sql("delete from `tabDaily Work Summary`")
		capkpi.db.sql("delete from `tabEmail Queue`")
		capkpi.db.sql("delete from `tabEmail Queue Recipient`")
		capkpi.db.sql("delete from `tabCommunication`")
		capkpi.db.sql("delete from `tabDaily Work Summary Group`")

		self.users = capkpi.get_all(
			"User", fields=["email"], filters=dict(email=("!=", "test@example.com"))
		)
		self.setup_groups(hour)

		from erp.hr.doctype.daily_work_summary_group.daily_work_summary_group import trigger_emails

		trigger_emails()

		# check if emails are created

		self.emails = capkpi.db.sql(
	...
	def setup_groups(self, hour=None):
		...
		if not hour:
			hour = capkpi.utils.nowtime().split(":")[0]
			hour = hour + ":00"

		groups = capkpi.get_doc(
			dict(
