...
import capkpi
from capkpi.permissions import clear_user_permissions_for_doctype
...
from erp.hr.doctype.holiday_list.test_holiday_list import set_holiday_list
from erp.hr.doctype.leave_allocation.test_leave_allocation import create_leave_allocation
from erp.hr.doctype.leave_application.leave_application import (
...
from erp.hr.doctype.leave_policy_assignment.leave_policy_assignment import (
...
from erp.payroll.doctype.salary_slip.test_salary_slip import (
	make_holiday_list,
...
class TestLeaveApplication(unittest.TestCase):
	def setUp(self):
		...
		capkpi.db.delete("Attendance", {"employee": "_T-Employee-00001"})
		capkpi.db.set_value("Employee", "_T-Employee-00001", "holiday_list", "")

		from_date = get_year_start(getdate())
		to_date = get_year_ending(getdate())
		self.holiday_list = make_holiday_list(from_date=from_date, to_date=to_date)

		if not capkpi.db.exists("Leave Type", "_Test Leave Type"):
			capkpi.get_doc(
	...
	def get_application(self, doc):
		application = capkpi.copy_doc(doc)
	...
	def test_validate_application_across_allocations(self):
		...
		capkpi.delete_doc_if_exists("Leave Type", "Test Leave Validation", force=1)
		leave_type = capkpi.get_doc(
		...
		self.assertRaises(capkpi.ValidationError, leave_application.insert)
	...
	def test_insufficient_leave_balance_validation(self):
		...
		capkpi.db.set_value("Leave Type", "Test Leave Validation", "allow_negative", True)
		make_leave_application(
	...
	def test_attendance_for_include_holidays(self):
		...
		capkpi.delete_doc_if_exists("Leave Type", "Test Include Holidays", force=1)
		leave_type = capkpi.get_doc(
		...
		self.assertEqual(capkpi.db.count("Attendance", {"leave_application": leave_application.name}), 4)
	...
	def test_attendance_update_for_exclude_holidays(self):
		...
		capkpi.delete_doc_if_exists("Leave Type", "Test Do Not Include Holidays", force=1)
		leave_type = capkpi.get_doc(
			dict(
		...
		attendance_on_holiday = capkpi.get_doc(config)
		...
		attendance = capkpi.get_doc(config)
		...
		self.assertEqual(capkpi.db.count("Attendance", {"leave_application": leave_application.name}), 3)

		# attendance on holiday deleted
		self.assertFalse(capkpi.db.exists("Attendance", attendance_on_holiday.name))

		# attendance on non-holiday updated
		self.assertEqual(capkpi.db.get_value("Attendance", attendance.name, "status"), "On Leave")
	...
	def test_optional_leave(self):
		...
		capkpi.db.set_value("Leave Period", leave_period.name, "optional_holiday_list", holiday_list)
		leave_type = "Test Optional Type"
		if not capkpi.db.exists("Leave Type", leave_type):
			capkpi.get_doc(
	...
	def test_leaves_allowed(self):
		employee = get_employee()
		leave_period = get_leave_period()
		capkpi.delete_doc_if_exists("Leave Type", "Test Leave Type", force=1)
		leave_type = capkpi.get_doc(
	...
	def test_applicable_after(self):
		...
		date = add_days(nowdate(), -7)
		capkpi.db.set_value("Employee", employee.name, "date_of_joining", date)
		...
		capkpi.delete_doc_if_exists("Leave Type", "Test Leave Type 1", force=1)
		leave_type_1 = capkpi.get_doc(
		...
		capkpi.db.set_value("Employee", employee.name, "date_of_joining", "2010-01-01")
	...
	def test_earned_leaves_creation(self):
		from erp.hr.utils import allocate_earned_leaves
		...
		capkpi.db.set_value("Leave Type", leave_type, "max_leaves_allowed", 0)
	...
	def test_current_leave_on_submit(self):
		...
		if not capkpi.db.exists("Leave Type", leave_type):
			capkpi.get_doc(dict(leave_type_name=leave_type, doctype="Leave Type")).insert()

		allocation = capkpi.get_doc(
			dict(
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_application.cancel()
		self.assertFalse(
			capkpi.db.exists("Leave Ledger Entry", {"transaction_name": leave_application.name})
		)
	...
	def test_leave_approver_perms(self):
		...
		employee.save()
		self.assertTrue("Leave Approver" in capkpi.get_roles(user))
		...
		application.insert()
		self.assertTrue(application.name in capkpi.share.get_shared("Leave Application", user))

		# check shared doc revoked
		application.reload()
		application.leave_approver = "test@example.com"
		application.save()
		self.assertTrue(application.name not in capkpi.share.get_shared("Leave Application", user))
	...
	def test_get_earned_leave_details_for_dashboard(self):
		...
		allocation = capkpi.db.get_value(
			"Leave Allocation",
			{"leave_policy_assignment": leave_policy_assignments[0]},
			"name",
		)
		allocation = capkpi.get_doc("Leave Allocation", allocation)
...
def get_employee():
	return capkpi.get_doc("Employee", "_T-Employee-00001")


def set_leave_approver():
	employee = get_employee()
	dept_doc = capkpi.get_doc("Department", employee.department)
	dept_doc.append("leave_approvers", {"approver": "test@example.com"})
	dept_doc.save(ignore_permissions=True)


def get_leave_period():
	leave_period_name = capkpi.db.exists({"doctype": "Leave Period", "company": "_Test Company"})
	if leave_period_name:
		return capkpi.get_doc("Leave Period", leave_period_name[0][0])
	else:
...
def make_policy_assignment(employee, leave_type, leave_period):
	capkpi.delete_doc_if_exists("Leave Type", leave_type, force=1)
	capkpi.get_doc(
		dict(
	...
	leave_policy_assignments = create_assignment_for_multiple_employees(
		[employee.name], capkpi._dict(data)
	)
