...
capkpi.ui.form.on('Staffing Plan', {
	setup: function(frm) {
...
capkpi.ui.form.on('Staffing Plan Detail', {
	...
	vacancies: function(frm, cdt, cdn) {
		...
		if(child.vacancies < child.current_openings) {
			capkpi.throw(__("Vacancies cannot be lower than the current openings"));
		}
		set_number_of_positions(frm, cdt, cdn);
	},
...
var set_number_of_positions = function(frm, cdt, cdn) {
	let child = locals[cdt][cdn];
	if (!child.designation) capkpi.throw(__("Please enter the designation"));
	capkpi.call({
		"method": "erp.hr.doctype.staffing_plan.staffing_plan.get_designation_counts",
		args: {
		...
		callback: function (data) {
			if(data.message){
				capkpi.model.set_value(cdt, cdn, 'current_count', data.message.employee_count);
				capkpi.model.set_value(cdt, cdn, 'current_openings', data.message.job_openings);
				...
				if (cint(child.number_of_positions) < total_positions){
					capkpi.model.set_value(cdt, cdn, 'number_of_positions', total_positions);
				}
			}
			else{ // No employees for this designation
				capkpi.model.set_value(cdt, cdn, 'current_count', 0);
				capkpi.model.set_value(cdt, cdn, 'current_openings', 0);
			}
		}
	});
...
var set_total_estimated_cost = function(frm, cdt, cdn) {
	...
	if(child.vacancies > 0 && child.estimated_cost_per_position) {
		capkpi.model.set_value(cdt, cdn, 'total_estimated_cost', child.vacancies * child.estimated_cost_per_position);
	}
	else {
		capkpi.model.set_value(cdt, cdn, 'total_estimated_cost', 0);
	}
