...
capkpi.ui.form.on('Interview Feedback', {
	onload: function(frm) {
	...
	interview: function(frm) {
		capkpi.call({
			method: 'erp.hr.doctype.interview_feedback.interview_feedback.get_applicable_interviewers',
			args: {
	...
	interviewer: function(frm) {
		if (!frm.doc.interview) {
			capkpi.throw(__('Select Interview first'));
