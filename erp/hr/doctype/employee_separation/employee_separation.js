...
capkpi.ui.form.on('Employee Separation', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.employee) {
			frm.add_custom_button(__('Employee'), function() {
				capkpi.set_route("Form", "Employee", frm.doc.employee);
			},__("View"));
		}
		if (frm.doc.project) {
			frm.add_custom_button(__('Project'), function() {
				capkpi.set_route("Form", "Project", frm.doc.project);
			},__("View"));
			frm.add_custom_button(__('Task'), function() {
				capkpi.set_route('List', 'Task', {project: frm.doc.project});
			},__("View"));
		}
		if (frm.doc.docstatus === 1 && frm.doc.project) {
			capkpi.call({
				method: "erp.hr.utils.get_boarding_status",
				args: {
	...
	employee_separation_template: function(frm) {
		...
		if (frm.doc.employee_separation_template) {
			capkpi.call({
				method: "erp.hr.utils.get_onboarding_details",
				args: {
				...
				callback: function(r) {
					if (r.message) {
						$.each(r.message, function(i, d) {
							var row = capkpi.model.add_child(frm.doc, "Employee Boarding Activity", "activities");
							$.extend(row, d);
						});
					}
