...
from capkpi.utils import DATE_FORMAT, flt, getdate, today


class LeaveLedgerEntry(Document):
	def validate(self):
		if getdate(self.from_date) > getdate(self.to_date):
			capkpi.throw(_("To date needs to be before from date"))

	def on_cancel(self):
		# allow cancellation of expiry leaves
		if self.is_expired:
			capkpi.db.set_value("Leave Allocation", self.transaction_name, "expired", 0)
		else:
			capkpi.throw(_("Only expired allocation can be cancelled"))
...
def validate_leave_allocation_against_leave_application(ledger):
	...
	leave_application_records = capkpi.db.sql_list(
		"""
...
def get_previous_expiry_ledger_entry(ledger):
	...
	creation_date = capkpi.db.get_value(
		"Leave Ledger Entry",
		filters={
...
def process_expired_allocation():
	...
	leave_type_records = capkpi.db.get_values(
	...
	expire_allocation = capkpi.db.sql(
		"""
		SELECT
...
def expire_allocation(allocation, expiry_date=None):
	...
	capkpi.db.set_value("Leave Allocation", allocation.name, "expired", 1)
