...
from capkpi.utils import add_days, cstr, date_diff, getdate
...
from erp.hr.utils import get_holiday_dates_for_employee
...
@capkpi.whitelist()
def get_template():
	if not capkpi.has_permission("Attendance", "create"):
		raise capkpi.PermissionError

	args = capkpi.local.form_dict

	if getdate(args.from_date) > getdate(args.to_date):
		capkpi.throw(_("To Date should be greater than From Date"))
	...
	try:
		...
		capkpi.respond_as_web_page("Holiday List Missing", html=e)
		return

	# write out response as a type csv
	capkpi.response["result"] = cstr(w.getvalue())
	capkpi.response["type"] = "csv"
	capkpi.response["doctype"] = "Attendance"


def add_header(w):
	status = ", ".join(
		(capkpi.get_meta("Attendance").get_field("status").options or "").strip().split("\n")
	)
	w.writerow(["Notes:"])
...
def get_naming_series():
	series = capkpi.get_meta("Attendance").get_field("naming_series").options.strip().split("\n")
	if not series:
		capkpi.throw(_("Please setup numbering series for Attendance via Setup > Numbering Series"))
	return series[0]
...
def upload():
	...
	rows = read_csv_content(capkpi.local.uploaded_file)
	if not rows:
		capkpi.throw(_("Please select a csv file"))
	capkpi.enqueue(import_attendances, rows=rows, now=True if len(rows) < 200 else False)
...
def import_attendances(rows):
	...
	from capkpi.utils.csvutils import check_record, import_doc

	for i, row in enumerate(rows):
		if not row:
			continue
		row_idx = i + 5
		d = capkpi._dict(zip(columns, row))

		d["doctype"] = "Attendance"
		if d.name:
			d["docstatus"] = capkpi.db.get_value("Attendance", d.name, "docstatus")

		try:
			check_record(d)
			ret.append(import_doc(d, "Attendance", 1, row_idx, submit=True))
			capkpi.publish_realtime("import_attendance", dict(progress=i, total=len(rows)))
		except AttributeError:
			pass
		except Exception as e:
			error = True
			ret.append("Error for row (#%d) %s : %s" % (row_idx, len(row) > 1 and row[1] or "", cstr(e)))
			capkpi.errprint(capkpi.get_traceback())
	...
	capkpi.publish_realtime("import_attendance", dict(messages=ret, error=error))
