...
erp.hr.AttendanceControlPanel = capkpi.ui.form.Controller.extend({
	onload: function() {
		this.frm.set_value("att_fr_date", capkpi.datetime.get_today());
		this.frm.set_value("att_to_date", capkpi.datetime.get_today());
	},
	...
	get_template:function() {
		if(!this.frm.doc.att_fr_date || !this.frm.doc.att_to_date) {
			capkpi.msgprint(__("Attendance From Date and Attendance To Date is mandatory"));
			return;
		}
		window.location.href = repl(capkpi.request.url +
			'?cmd=%(cmd)s&from_date=%(from_date)s&to_date=%(to_date)s', {
			cmd: "erp.hr.doctype.upload_attendance.upload_attendance.get_template",
	...
	show_upload() {
		...
		new capkpi.ui.FileUploader({
			wrapper: $wrapper,
			method: 'erp.hr.doctype.upload_attendance.upload_attendance.upload'
		});
	},
	...
	setup_import_progress() {
		...
		capkpi.realtime.on('import_attendance', (data) => {
			if (data.progress) {
...
cur_frm.cscript = new erp.hr.AttendanceControlPanel({frm: cur_frm});
