...
class TestEmployeePromotion(unittest.TestCase):
	def setUp(self):
		...
		capkpi.db.sql("""delete from `tabEmployee Promotion`""")

	def test_submit_before_promotion_date(self):
		promotion_obj = capkpi.get_doc(
			{
		...
		promotion_obj.save()
		self.assertRaises(capkpi.DocstatusTransitionError, promotion_obj.submit)
		promotion = capkpi.get_doc("Employee Promotion", promotion_obj.name)
