...
erp.hr.EmployeeController = capkpi.ui.form.Controller.extend({
	setup: function() {
		this.frm.fields_dict.user_id.get_query = function() {
			return {
				query: "capkpi.core.doctype.user.user.user_query",
				filters: {
					ignore_user_type: 1
				}
			};
...
capkpi.ui.form.on('Employee', {
	...
	update_contact: function(frm) {
		var prefered_email_fieldname = capkpi.model.scrub(frm.doc.prefered_contact_email) || 'user_id';
	...
	create_user: function(frm) {
		if (!frm.doc.prefered_email) {
			capkpi.throw(__("Please enter Preferred Contact Email"));
		}
		capkpi.call({
			method: "erp.hr.doctype.employee.employee.create_user",
			args: {
...
cur_frm.cscript = new erp.hr.EmployeeController({
	frm: cur_frm
});


capkpi.tour['Employee'] = [
	{
