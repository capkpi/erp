...
class EmployeeCheckin(Document):
	...
	def validate_duplicate_log(self):
		doc = capkpi.db.exists(
			"Employee Checkin", {"employee": self.employee, "time": self.time, "name": ["!=", self.name]}
		)
		if doc:
			doc_link = capkpi.get_desk_link("Employee Checkin", doc)
			capkpi.throw(
...
def add_log_based_on_employee_field(
	...
	if not employee_field_value or not timestamp:
		capkpi.throw(_("'employee_field_value' and 'timestamp' are required."))

	employee = capkpi.db.get_values(
		"Employee",
	...
	doc = capkpi.new_doc("Employee Checkin")
...
def mark_attendance_and_link_log(
	...
	if attendance_status == "Skip":
		...
		company = capkpi.get_cached_value("Employee", employee, "company")
		duplicate = capkpi.db.exists(
			"Attendance",
		...
		capkpi.throw(_("{} is an invalid Attendance Status.").format(attendance_status))
