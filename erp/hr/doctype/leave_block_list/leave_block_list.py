	...
	def validate(self):
		...
		for d in self.get("leave_block_list_dates"):
			...
			if d.block_date in dates:
				capkpi.msgprint(_("Date is repeated") + ":" + d.block_date, raise_exception=1)
...
def get_applicable_block_lists(employee=None, company=None, all_lists=False):
	...
	if department:
		block_list = capkpi.db.get_value("Department", department, "leave_block_list")
		add_block_list(block_list)

	# global
	for block_list in capkpi.db.sql_list(
...
def is_user_in_allow_list(block_list):
	return capkpi.session.user in capkpi.db.sql_list(
		"""select allow_user
