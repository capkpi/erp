...
from capkpi.utils import cint, cstr, formatdate, get_datetime, getdate, nowdate
...
class Attendance(Document):
	...
	def validate_attendance_date(self):
		...
		if (
			...
			capkpi.throw(_("Attendance can not be marked for future dates"))
		elif date_of_joining and getdate(self.attendance_date) < getdate(date_of_joining):
			capkpi.throw(_("Attendance date can not be less than employee's joining date"))
		...
		if res:
			capkpi.throw(
				_("Attendance for employee {0} is already marked for the date {1}").format(
					capkpi.bold(self.employee), capkpi.bold(self.attendance_date)
				)
			)

	def validate_employee_status(self):
		if capkpi.db.get_value("Employee", self.employee, "status") == "Inactive":
			capkpi.throw(_("Cannot mark attendance for an Inactive employee {0}").format(self.employee))
	...
	def validate_employee(self):
		...
		if not emp:
			capkpi.throw(_("Employee {0} is not active or does not exist").format(self.employee))

	def unlink_attendance_from_checkins(self):
		from capkpi.utils import get_link_to_form

		EmployeeCheckin = capkpi.qb.DocType("Employee Checkin")
		linked_logs = (
			capkpi.qb.from_(EmployeeCheckin)
		...
		if linked_logs:
			(
				capkpi.qb.update(EmployeeCheckin)
				.set("attendance", "")
				.where(EmployeeCheckin.attendance == self.name)
			).run()
...
def get_events(start, end, filters=None):
	...
	employee = capkpi.db.get_value("Employee", {"user_id": capkpi.session.user})
...
def add_attendance(events, start, end, conditions=None):
	...
	for d in capkpi.db.sql(query, {"from_date": start, "to_date": end}, as_dict=True):
		e = {
			"name": d.name,
			"doctype": "Attendance",
...
def mark_attendance(
	...
	if not capkpi.db.exists(
		...
		company = capkpi.db.get_value("Employee", employee, "company")
...
def mark_bulk_attendance(data):
	...
	company = capkpi.get_value("Employee", data.employee, "company")
	if not data.unmarked_days:
		capkpi.throw(_("Please select a date."))
		return
	...
	for date in data.unmarked_days:
		...
		attendance = capkpi.get_doc(doc_dict).insert()
		attendance.submit()
