...
from capkpi.utils import add_days, add_months, get_first_day, get_last_day, getdate

from erp.hr.doctype.leave_application.test_leave_application import (
	get_employee,
	get_leave_period,
)
from erp.hr.doctype.leave_policy.test_leave_policy import create_leave_policy
...
class TestLeavePolicyAssignment(CapKPITestCase):
	def setUp(self):
		for doctype in [
			"Leave Period",
			"Leave Application",
	...
	def test_grant_leaves(self):
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee.name], capkpi._dict(data)
		)
		self.assertEqual(
			capkpi.db.get_value("Leave Policy Assignment", leave_policy_assignments[0], "leaves_allocated"),
			1,
		)

		leave_allocation = capkpi.get_list(
			"Leave Allocation",
			filters={
		...
		leave_alloc_doc = capkpi.get_doc("Leave Allocation", leave_allocation)
	...
	def test_earned_leave_allocation(self):
		...
		capkpi.flags.current_date = add_days(get_last_day(getdate()), -1)
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee.name], capkpi._dict(data)
		)
	...
	def test_earned_leave_alloc_for_passed_months_based_on_leave_period(self):
		...
		capkpi.flags.current_date = get_first_day(getdate())
		data = {
			"assignment_based_on": "Leave Period",
			"leave_policy": leave_policy.name,
			"leave_period": leave_period.name,
	...
	def test_earned_leave_alloc_for_passed_months_on_month_end_based_on_leave_period(self):
		...
		capkpi.flags.current_date = get_last_day(getdate())
	...
	def test_earned_leave_alloc_for_passed_months_with_cf_leaves_based_on_leave_period(self):
		...
		capkpi.flags.current_date = get_last_day(add_months(getdate(), -1))
		data = {
			"assignment_based_on": "Leave Period",
			"leave_policy": leave_policy.name,
			"leave_period": leave_period.name,
	...
	def test_earned_leave_alloc_for_passed_months_based_on_joining_date(self):
		...
		effective_from = capkpi.db.get_value(
	...
	def tearDown(self):
		capkpi.db.set_value("Employee", self.employee.name, "date_of_joining", self.original_doj)
		capkpi.flags.current_date = None
...
def create_leave_period(name, start_date=None):
	capkpi.delete_doc_if_exists("Leave Period", name, force=1)
	if not start_date:
