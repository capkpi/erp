...
from capkpi.utils.data import get_link_to_form


class JobOffer(Document):
	def onload(self):
		employee = capkpi.db.get_value("Employee", {"job_applicant": self.job_applicant}, "name") or ""
		self.set_onload("employee", employee)

	def validate(self):
		self.validate_vacancies()
		job_offer = capkpi.db.exists(
		...
		if job_offer and job_offer != self.name:
			capkpi.throw(
				_("Job Offer: {0} is already for Job Applicant: {1}").format(
					capkpi.bold(job_offer), capkpi.bold(self.job_applicant)
				)
			)

	def validate_vacancies(self):
		staffing_plan = get_staffing_plan_detail(self.designation, self.company, self.offer_date)
		check_vacancies = capkpi.get_single("HR Settings").check_vacancies
		if staffing_plan and check_vacancies:
			...
			if not staffing_plan.get("vacancies") or cint(staffing_plan.vacancies) - len(job_offers) <= 0:
				error_variable = "for " + capkpi.bold(self.designation)
				if staffing_plan.get("parent"):
					error_variable = capkpi.bold(get_link_to_form("Staffing Plan", staffing_plan.parent))

				capkpi.throw(_("There are no vacancies under staffing plan {0}").format(error_variable))
...
def update_job_applicant(status, job_applicant):
	if status in ("Accepted", "Rejected"):
		capkpi.set_value("Job Applicant", job_applicant, "status", status)


def get_staffing_plan_detail(designation, company, offer_date):
	detail = capkpi.db.sql(
		"""
	...
	return capkpi._dict(detail[0]) if (detail and detail[0].parent) else None


@capkpi.whitelist()
def make_employee(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.personal_email, target.first_name = capkpi.db.get_value(
