...
capkpi.ui.form.on("Job Applicant", {
		...
		if (!frm.doc.__islocal) {
			if (frm.doc.__onload && frm.doc.__onload.job_offer) {
				...
				frm.add_custom_button(__("Job Offer"), function() {
					capkpi.set_route("Form", "Job Offer", frm.doc.__onload.job_offer);
				}, __("View"));
			} else {
	...
	make_dashboard: function(frm) {
		capkpi.call({
			method: "erp.hr.doctype.job_applicant.job_applicant.get_interview_details",
			args: {
				job_applicant: frm.doc.name
			},
			callback: function(r) {
				...
				frm.dashboard.add_section(
					capkpi.render_template('job_applicant_dashboard', {
						data: r.message
					}),
	...
	create_interview: function (frm, values) {
		capkpi.call({
			method: "erp.hr.doctype.job_applicant.job_applicant.create_interview",
			args: {
				doc: frm.doc,
