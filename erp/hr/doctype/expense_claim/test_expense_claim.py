...
from capkpi.utils import flt, nowdate, random_string
...
class TestExpenseClaim(CapKPITestCase):
	def setUp(self):
		if not capkpi.db.get_value("Cost Center", {"company": company_name}):
			capkpi.get_doc(
				{
					"doctype": "Cost Center",
	...
	def test_total_expense_claim_for_project(self):
		...
		capkpi.db.sql("update `tabExpense Claim` set project = '', task = ''")

		project = capkpi.get_doc({"project_name": "_Test Project 1", "doctype": "Project"})
		project.save()
		...
		self.assertEqual(capkpi.db.get_value("Task", task_name, "total_expense_claim"), 200)
		self.assertEqual(capkpi.db.get_value("Project", project.name, "total_expense_claim"), 200)

		expense_claim2 = make_expense_claim(
			payable_account, 600, 500, company_name, "Travel Expenses - _TC3", project.name, task_name
		)

		self.assertEqual(capkpi.db.get_value("Task", task_name, "total_expense_claim"), 700)
		self.assertEqual(capkpi.db.get_value("Project", project.name, "total_expense_claim"), 700)
	...
	def test_expense_claim_status(self):
		...
		expense_claim = capkpi.get_doc("Expense Claim", expense_claim.name)
	...
	def test_expense_claim_against_fully_paid_advances(self):
		from erp.hr.doctype.employee_advance.test_employee_advance import (
			get_advances_for_claim,
			make_employee_advance,
			make_payment_entry,
		)

		capkpi.db.delete("Employee Advance")
	...
	def test_expense_approver_perms(self):
		...
		expense_claim.save()
		self.assertTrue(expense_claim.name in capkpi.share.get_shared("Expense Claim", user))

		# check shared doc revoked
		expense_claim.reload()
		expense_claim.expense_approver = "test@example.com"
		expense_claim.save()
		self.assertTrue(expense_claim.name not in capkpi.share.get_shared("Expense Claim", user))
...
def get_payable_account(company):
	return capkpi.get_cached_value("Company", company, "default_payable_account")
...
def make_expense_claim(
	...
	currency, cost_center = capkpi.db.get_value(
	...
	expense_claim = capkpi.get_doc(expense_claim)
...
def get_outstanding_and_total_reimbursed_amounts(expense_claim):
	outstanding_amount = flt(
		capkpi.db.get_value("Expense Claim", expense_claim.name, "total_sanctioned_amount")
	) - flt(capkpi.db.get_value("Expense Claim", expense_claim.name, "total_amount_reimbursed"))
	total_amount_reimbursed = flt(
		capkpi.db.get_value("Expense Claim", expense_claim.name, "total_amount_reimbursed")
	)
