...
import erp
from erp.hr.doctype.employee.employee import (
	InactiveEmployeeStatusError,
	get_holiday_list_for_employee,
)


class DuplicateDeclarationError(capkpi.ValidationError):
	pass
...
class EmployeeBoardingController(Document):
	...
	def on_cancel(self):
		# delete task project
		for task in capkpi.get_all("Task", filters={"project": self.project}):
			capkpi.delete_doc("Task", task.name, force=1)
		capkpi.delete_doc("Project", self.project, force=1)
		self.db_set("project", "")
		for activity in self.activities:
...
def set_employee_name(doc):
	if doc.employee and not doc.employee_name:
		doc.employee_name = capkpi.db.get_value("Employee", doc.employee, "employee_name")
...
def update_employee_work_history(employee, details, date=None, cancel=False):
	...
	internal_work_history = {}
	for item in details:
		field = capkpi.get_meta("Employee").get_field(item.fieldname)
		if not field:
			continue
...
def delete_employee_work_history(details, employee, date):
	...
	if filters:
		capkpi.db.delete("Employee Internal Work History", filters)
		employee.save()
...
def get_employee_fields_label():
	fields = []
	for df in capkpi.get_meta("Employee").get("fields"):
		if df.fieldname in [
			"salutation",
			"user_id",
			"employee_number",
...
def get_employee_field_property(employee, fieldname):
	if employee and fieldname:
		field = capkpi.get_meta("Employee").get_field(fieldname)
		value = capkpi.db.get_value("Employee", employee, fieldname)
...
def validate_dates(doc, from_date, to_date):
	date_of_joining, relieving_date = capkpi.db.get_value(
		"Employee", doc.employee, ["date_of_joining", "relieving_date"]
	)
	if getdate(from_date) > getdate(to_date):
		capkpi.throw(_("To date can not be less than from date"))
	elif getdate(from_date) > getdate(nowdate()):
		capkpi.throw(_("Future dates not allowed"))
	elif date_of_joining and getdate(from_date) < getdate(date_of_joining):
		capkpi.throw(_("From date can not be less than employee's joining date"))
	elif relieving_date and getdate(to_date) > getdate(relieving_date):
		capkpi.throw(_("To date can not greater than employee's relieving date"))
...
def validate_duplicate_exemption_for_payroll_period(doctype, docname, payroll_period, employee):
	existing_record = capkpi.db.exists(
		doctype,
		{
...
def validate_tax_declaration(declarations):
	...
	for d in declarations:
		if d.exemption_sub_category in subcategories:
			capkpi.throw(_("More than one selection for {0} not allowed").format(d.exemption_sub_category))
		subcategories.append(d.exemption_sub_category)


def get_total_exemption_amount(declarations):
	exemptions = capkpi._dict()
	for d in declarations:
		exemptions.setdefault(d.exemption_category, capkpi._dict())
...
def generate_leave_encashment():
	...
	from erp.hr.doctype.leave_encashment.leave_encashment import create_leave_encashment

	if capkpi.db.get_single_value("HR Settings", "auto_leave_encashment"):
		leave_type = capkpi.get_all("Leave Type", filters={"allow_encashment": 1}, fields=["name"])
		leave_type = [l["name"] for l in leave_type]

		leave_allocation = capkpi.get_all(
			"Leave Allocation",
...
def allocate_earned_leaves():
	...
	for e_leave_type in e_leave_types:
		...
		for allocation in leave_allocations:
			...
			annual_allocation = capkpi.db.get_value(
				"Leave Policy Detail",
			...
			if e_leave_type.based_on_date_of_joining:
				from_date = capkpi.db.get_value("Employee", allocation.employee, "date_of_joining")
...
def update_previous_leave_allocation(allocation, annual_allocation, e_leave_type):
	...
	allocation = capkpi.get_doc("Leave Allocation", allocation.name)
	...
	if new_allocation != allocation.total_leaves_allocated:
		...
		if e_leave_type.based_on_date_of_joining:
			text = _("allocated {0} leave(s) via scheduler on {1} based on the date of joining").format(
				capkpi.bold(earned_leaves), capkpi.bold(formatdate(today_date))
			)
		else:
...
def is_earned_leave_already_allocated(allocation, annual_allocation):
	...
	date_of_joining = capkpi.db.get_value("Employee", allocation.employee, "date_of_joining")

	assignment = capkpi.get_doc("Leave Policy Assignment", allocation.leave_policy_assignment)
...
def get_sal_slip_total_benefit_given(employee, payroll_period, component=False):
	...
	sum_of_given_benefit = capkpi.db.sql(
		query,
		{
			"employee": employee,
...
def get_holidays_for_employee(
	...
	holidays = capkpi.get_all("Holiday", fields=["description", "holiday_date"], filters=filters)
...
def get_previous_claimed_amount(employee, payroll_period, non_pro_rata=False, component=False):
	...
	sum_of_claimed_amount = capkpi.db.sql(
		query,
		{
			"employee": employee,
			"start_date": payroll_period.start_date,
...
def share_doc_with_approver(doc, user):
	...
	if not capkpi.has_permission(doc=doc, ptype="submit", user=user):
		capkpi.share.add(doc.doctype, doc.name, user, submit=1, flags={"ignore_share_permission": True})

		capkpi.msgprint(
			_("Shared with the user {0} with {1} access").format(user, capkpi.bold("submit"), alert=True)
		)
	...
	if doc_before_save:
		...
		if doc_before_save.get(approver) != doc.get(approver):
			capkpi.share.remove(doc.doctype, doc.name, doc_before_save.get(approver))


def validate_active_employee(employee):
	if capkpi.db.get_value("Employee", employee, "status") == "Inactive":
		capkpi.throw(
