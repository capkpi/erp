...
from erp.projects.doctype.task.task import CircularReferenceError
...
class TestTask(unittest.TestCase):
	...
	def test_reschedule_dependent_task(self):
		...
		self.assertEqual(
			capkpi.db.get_value("Task", task2.name, "exp_start_date"), getdate(add_days(nowdate(), 21))
		)
		self.assertEqual(
			capkpi.db.get_value("Task", task2.name, "exp_end_date"), getdate(add_days(nowdate(), 25))
		)

		self.assertEqual(
			capkpi.db.get_value("Task", task3.name, "exp_start_date"), getdate(add_days(nowdate(), 26))
		)
		self.assertEqual(
			capkpi.db.get_value("Task", task3.name, "exp_end_date"), getdate(add_days(nowdate(), 30))
		)

	def test_close_assignment(self):
		if not capkpi.db.exists("Task", "Test Close Assignment"):
			task = capkpi.new_doc("Task")
	...
	def test_overdue(self):
		...
		from erp.projects.doctype.task.task import set_tasks_as_overdue

		set_tasks_as_overdue()

		self.assertEqual(capkpi.db.get_value("Task", task.name, "status"), "Overdue")
...
def create_task(
	...
	if not capkpi.db.exists("Task", subject):
		task = capkpi.new_doc("Task")
		task.status = "Open"
		...
		task.project = (
			project or None
			if is_template
			else capkpi.get_value("Project", {"project_name": "_Test Project"})
		)
		...
		task = capkpi.get_doc("Task", subject)
