...
capkpi.treeview_settings['Task'] = {
	get_tree_nodes: "erp.projects.doctype.task.task.get_children",
	add_tree_node: "erp.projects.doctype.task.task.add_node",
	filters: [
		{
			fieldname: "project",
			fieldtype:"Link",
		...
		},
		{
			...
			label: __("Task"),
			get_query: function() {
				var me = capkpi.treeview_settings['Task'];
	...
	onload: function(me) {
		capkpi.treeview_settings['Task'].page = {};
		$.extend(capkpi.treeview_settings['Task'].page, me.page);
		me.make_tree();
	},
	toolbar: [
		{
			...
			click: function(node) {
				...
				const dialog = new capkpi.ui.Dialog({
					...
					primary_action: function() {
						dialog.hide();
						return capkpi.call({
							method: "erp.projects.doctype.task.task.add_multiple_tasks",
							args: {
