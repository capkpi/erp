...
from capkpi.desk.form.assign_to import clear, close_all_assignments
from capkpi.model.mapper import get_mapped_doc
from capkpi.utils import add_days, cstr, date_diff, flt, get_link_to_form, getdate, today
from capkpi.utils.nestedset import NestedSet


class CircularReferenceError(capkpi.ValidationError):
	pass


class EndDateCannotBeGreaterThanProjectEndDateError(capkpi.ValidationError):
	pass
...
class Task(NestedSet):
	...
	def get_customer_details(self):
		cust = capkpi.db.sql("select customer_name from `tabCustomer` where name=%s", self.customer)
		if cust:
	...
	def validate_dates(self):
		if (
			...
			capkpi.throw(
				_("{0} can not be greater than {1}").format(
					capkpi.bold("Expected Start Date"), capkpi.bold("Expected End Date")
				)
			)
		...
		if (
			...
			capkpi.throw(
				_("{0} can not be greater than {1}").format(
					capkpi.bold("Actual Start Date"), capkpi.bold("Actual End Date")
				)
			)

	def validate_parent_expected_end_date(self):
		if self.parent_task:
			parent_exp_end_date = capkpi.db.get_value("Task", self.parent_task, "exp_end_date")
	...
	def validate_parent_project_dates(self):
		if not self.project or capkpi.flags.in_test:
			return

		expected_end_date = capkpi.db.get_value("Project", self.project, "expected_end_date")
	...
	def validate_status(self):
		...
		if self.status != self.get_db_value("status") and self.status == "Completed":
			for d in self.depends_on:
				if capkpi.db.get_value("Task", d.task, "status") not in ("Completed", "Cancelled"):
					capkpi.throw(
						_(
							"Cannot complete task {0} as its dependant task {1} are not ccompleted / cancelled."
						).format(capkpi.bold(self.name), capkpi.bold(d.task))
					)
	...
	def validate_progress(self):
		if flt(self.progress or 0) > 100:
			capkpi.throw(_("Progress % for a task cannot be more than 100."))
	...
	def validate_parent_template_task(self):
		if self.parent_task:
			if not capkpi.db.get_value("Task", self.parent_task, "is_template"):
				parent_task_format = """<a href="#Form/Task/{0}">{0}</a>""".format(self.parent_task)
				capkpi.throw(_("Parent Task {0} is not a Template Task").format(parent_task_format))

	def validate_depends_on_tasks(self):
		if self.depends_on:
			for task in self.depends_on:
				if not capkpi.db.get_value("Task", task.task, "is_template"):
					dependent_task_format = """<a href="#Form/Task/{0}">{0}</a>""".format(task.task)
					capkpi.throw(_("Dependent Task {0} is not a Template Task").format(dependent_task_format))

	def validate_completed_on(self):
		if self.completed_on and getdate(self.completed_on) > getdate():
			capkpi.throw(_("Completed On cannot be greater than Today"))
	...
	def update_total_expense_claim(self):
		self.total_expense_claim = capkpi.db.sql(
	...
	def update_project(self):
		if self.project and not self.flags.from_project:
			capkpi.get_cached_doc("Project", self.project).update_project()
	...
	def check_recursion(self):
		...
		for d in check_list:
			...
			while len(task_list) > count:
				...
				for b in tasks:
					if b[0] == self.name:
						capkpi.throw(_("Circular Reference Error"), CircularReferenceError)
					if b[0]:
	...
	def reschedule_dependent_tasks(self):
		...
		if end_date:
			for task_name in capkpi.db.sql(
				"""
			...
			""",
				...
				task = capkpi.get_doc("Task", task_name.name)
				if (
					task.exp_start_date
	...
	def has_webform_permission(self):
		project_user = capkpi.db.get_value(
			"Project User", {"parent": self.project, "user": capkpi.session.user}, "user"
		)
		if project_user:
			return True
	...
	def populate_depends_on(self):
		if self.parent_task:
			parent = capkpi.get_doc("Task", self.parent_task)
...
def check_if_child_exists(name):
	child_tasks = capkpi.get_all("Task", filters={"parent_task": name})
...
def set_multiple_status(names, status):
	...
	for name in names:
		task = capkpi.get_doc("Task", name)
		task.status = status
		task.save()


def set_tasks_as_overdue():
	tasks = capkpi.get_all(
		"Task",
	...
	for task in tasks:
		if task.status == "Pending Review":
			if getdate(task.review_date) > getdate(today()):
				continue
		capkpi.get_doc("Task", task.name).update_status()
...
def add_multiple_tasks(data, parent):
	...
	new_doc["project"] = capkpi.db.get_value("Task", {"name": parent}, "project") or ""

	for d in data:
		if not d.get("subject"):
			continue
		new_doc["subject"] = d.get("subject")
		new_task = capkpi.get_doc(new_doc)
		new_task.insert()


def on_doctype_update():
	capkpi.db.add_index("Task", ["lft", "rgt"])
