...
import capkpi
from capkpi.utils import add_months, add_to_date, now_datetime, nowdate
...
class TestTimesheet(unittest.TestCase):
	...
	def test_salary_slip_from_timesheet(self):
		...
		timesheet = capkpi.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_billing_based_on_project(self):
		...
		ts = capkpi.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_time_overlap(self):
		...
		timesheet = capkpi.new_doc("Timesheet")
		...
		self.assertRaises(capkpi.ValidationError, timesheet.save)
...
def make_salary_structure_for_timesheet(employee, company=None):
	...
	if not capkpi.db.exists("Salary Component", "Timesheet Component"):
		capkpi.get_doc(
	...
	if not capkpi.db.get_value("Salary Structure Assignment", {"employee": employee, "docstatus": 1}):
		capkpi.db.set_value("Employee", employee, "date_of_joining", add_months(nowdate(), -5))
...
def update_activity_type(activity_type):
	activity_type = capkpi.get_doc("Activity Type", activity_type)
