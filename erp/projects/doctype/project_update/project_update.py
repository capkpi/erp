...
def email_sending(
	...
	msg += "</table>"
	if len(holiday) == 0:
		email = capkpi.db.sql("""SELECT user from `tabProject User` WHERE parent = %s;""", project_name)
		for emails in email:
			capkpi.sendmail(
				recipients=emails, subject=capkpi._(project_name + " " + "Summary"), message=msg
			)
	else:
		pass
