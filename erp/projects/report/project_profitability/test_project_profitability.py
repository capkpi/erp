...
from erp.projects.doctype.timesheet.timesheet import make_salary_slip, make_sales_invoice
from erp.projects.report.project_profitability.project_profitability import execute


class TestProjectProfitability(CapKPITestCase):
	def setUp(self):
		capkpi.db.sql("delete from `tabTimesheet`")
		...
		if holidays:
			capkpi.db.set_value("Payroll Settings", None, "include_holidays_in_total_working_days", 1)
		...
		capkpi.db.set_value("HR Settings", None, "standard_working_hours", 8)
	...
	def test_project_profitability(self):
		...
		row = report[1][0]
		timesheet = capkpi.get_doc("Timesheet", self.timesheet.name)
		...
		standard_working_hours = capkpi.db.get_single_value("HR Settings", "standard_working_hours")
