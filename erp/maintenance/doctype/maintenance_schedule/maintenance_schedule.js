...
capkpi.ui.form.on('Maintenance Schedule', {
	setup: function (frm) {
	...
	generate_schedule: function (frm) {
		if (frm.is_new()) {
			capkpi.msgprint(__('Please save first'));
		} else {
...
erp.maintenance.MaintenanceSchedule = capkpi.ui.form.Controller.extend({
	refresh: function () {
...
$.extend(cur_frm.cscript, new erp.maintenance.MaintenanceSchedule({ frm: cur_frm }));
