...
capkpi.provide("erp.maintenance");
capkpi.ui.form.on('Maintenance Visit', {
	setup: function (frm) {
	...
	onload: function (frm) {
		...
		if (frm.doc.maintenance_type === "Scheduled") {
			...
			capkpi.call({
				method: "erp.maintenance.doctype.maintenance_schedule.maintenance_schedule.get_serial_nos_from_schedule",
				args: {
		...
		if (frm.doc.__islocal) {
			frm.set_value({ mntc_date: capkpi.datetime.get_today() });
		}
	},
	...
	customer_address: function (frm) {
		erp.utils.get_address_display(frm, 'customer_address', 'address_display');
	},
...
erp.maintenance.MaintenanceVisit = capkpi.ui.form.Controller.extend({
	refresh: function () {
			...
			this.frm.add_custom_button(__('Maintenance Schedule'),
				function () {
					if (!me.frm.doc.customer) {
						capkpi.msgprint(__('Please select Customer first'));
						return;
					}
					erp.utils.map_current_doc({
						method: "erp.maintenance.doctype.maintenance_schedule.maintenance_schedule.make_maintenance_visit",
...
$.extend(cur_frm.cscript, new erp.maintenance.MaintenanceVisit({ frm: cur_frm }));
