...
from erp.utilities.activation import get_level
...
def update_doctypes():
	for d in capkpi.db.sql(
		...
		dt = capkpi.get_doc("DocType", d.parent)
...
def get_site_info(site_info):
	...
	if not company:
		company = capkpi.db.sql("select name from `tabCompany` order by creation asc")
		company = company[0][0] if company else None

	if company:
		domain = capkpi.get_cached_value("Company", cstr(company), "domain")
