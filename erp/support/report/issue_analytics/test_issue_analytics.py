...
import capkpi
from capkpi.desk.form.assign_to import add as add_assignment
from capkpi.utils import add_months, getdate

from erp.support.doctype.issue.test_issue import create_customer, make_issue
from erp.support.doctype.service_level_agreement.test_service_level_agreement import (
	create_service_level_agreements_for_issues,
)
from erp.support.report.issue_analytics.issue_analytics import execute
...
class TestIssueAnalytics(unittest.TestCase):
	@classmethod
	def setUpClass(self):
		capkpi.db.sql("delete from `tabIssue` where company='_Test Company'")
...
def create_issue_types():
	for entry in ["Bug", "Service Request", "Discomfort"]:
		if not capkpi.db.exists("Issue Type", entry):
			capkpi.get_doc({"doctype": "Issue Type", "__newname": entry}).insert()
