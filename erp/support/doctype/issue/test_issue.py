...
from erp.support.doctype.service_level_agreement.test_service_level_agreement import (
...
class TestIssue(TestSetUp):
	def test_response_time_and_resolution_time_based_on_different_sla(self):
		...
		capkpi.flags.current_time = get_datetime("2019-03-04 15:00")
	...
	def test_issue_metrics(self):
		...
		capkpi.flags.current_time = get_datetime("2020-03-04 5:05")
		issue.reload()
		issue.status = "Closed"
		issue.save()
	...
	def test_hold_time_on_replied(self):
		...
		capkpi.flags.current_time = get_datetime("2020-03-04 4:15")
		issue.reload()
		issue.status = "Replied"
		issue.save()

		self.assertEqual(issue.on_hold_since, capkpi.flags.current_time)

		creation = get_datetime("2020-03-04 5:00")
		capkpi.flags.current_time = get_datetime("2020-03-04 5:00")
		create_communication(issue.name, "test@example.com", "Received", creation)
...
def create_customer(name, customer_group, territory):
	...
	if not capkpi.db.exists("Customer", {"customer_name": name}):
		capkpi.get_doc(
			{
				"doctype": "Customer",
				"customer_name": name,
...
def create_customer_group(customer_group):
	...
	if not capkpi.db.exists("Customer Group", {"customer_group_name": customer_group}):
		capkpi.get_doc({"doctype": "Customer Group", "customer_group_name": customer_group}).insert(
...
def create_territory(territory):
	...
	if not capkpi.db.exists("Territory", {"territory_name": territory}):
		capkpi.get_doc(
			{
				"doctype": "Territory",
