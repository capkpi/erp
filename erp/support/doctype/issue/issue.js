capkpi.ui.form.on("Issue", {
	onload: function(frm) {
		...
		capkpi.db.get_value("Support Settings", {name: "Support Settings"},
		...
		if (frm.doc.service_level_agreement) {
			capkpi.call({
				method: "erp.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
				args: {
		...
		if (frm.doc.status !== "Closed") {
			...
			frm.add_custom_button(__("Task"), function() {
				capkpi.model.open_mapped_doc({
					method: "erp.support.doctype.issue.issue.make_task",
					frm: frm
				});
			}, __("Create"));
	...
	reset_service_level_agreement: function(frm) {
		let reset_sla = new capkpi.ui.Dialog({
			...
			primary_action: (values) => {
				...
				frm.call("reset_service_level_agreement", {
					...
					user: capkpi.session.user_email
				}, () => {
					reset_sla.enable_primary_action();
					frm.refresh();
					capkpi.msgprint(__("Service Level Agreement was reset."));
				});
			}
		});
	...
	timeline_refresh: function(frm) {
					...
					var dialog = new capkpi.ui.Dialog({
						...
						primary_action: () => {
							frm.call("split_issue", {
								...
								capkpi.msgprint(`New issue created: <a href="/app/issue/${r.message}">${r.message}</a>`);
								frm.reload_doc();
								dialog.hide();
							});
						}
		...
		// if (capkpi.model.can_create("Help Article")) {
		...
		// 			${capkpi.utils.icon('solid-info', 'sm')}
		// 		</a>
		// 	`);
		...
		// 			const doc = capkpi.model.get_new_doc("Help Article");
		// 			doc.title = frm.doc.subject;
		// 			doc.content = content;
		// 			capkpi.set_route("Form", "Help Article", doc.name);
		// 		});
		// 	}
