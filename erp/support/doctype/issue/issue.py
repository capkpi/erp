...
from erp.support.doctype.service_level_agreement.service_level_agreement import (
...
class Issue(Document):
	...
	def validate(self):
		...
		if not self.raised_by:
			self.raised_by = capkpi.session.user
	...
	def set_lead_contact(self, email_id):
		...
		if email_id:
			if not self.lead:
				self.lead = capkpi.db.get_value("Lead", {"email_id": email_id})

			if not self.contact and not self.customer:
				self.contact = capkpi.db.get_value("Contact", {"email_id": email_id})

				if self.contact:
					contact = capkpi.get_doc("Contact", self.contact)
					self.customer = contact.get_link_for("Customer")

			if not self.company:
				self.company = capkpi.db.get_value("Lead", self.lead, "company") or capkpi.db.get_default(
					"Company"
				)
	...
	def update_status(self):
		status = capkpi.db.get_value("Issue", self.name, "status")
		if self.status != "Open" and status == "Open" and not self.first_responded_on:
			self.first_responded_on = capkpi.flags.current_time or now_datetime()

		if self.status in ["Closed", "Resolved"] and status not in ["Resolved", "Closed"]:
			self.resolution_date = capkpi.flags.current_time or now_datetime()
			if capkpi.db.get_value("Issue", self.name, "agreement_status") == "Ongoing":
	...
	def handle_hold_time(self, status):
		if self.service_level_agreement:
			...
			pause_sla_on = capkpi.db.get_all(
			...
			if hold_statuses:
				if self.status in hold_statuses and status not in hold_statuses:
					update_values["on_hold_since"] = capkpi.flags.current_time or now_datetime()
				...
				if self.status not in hold_statuses and status in hold_statuses:
					...
					now_time = capkpi.flags.current_time or now_datetime()
					last_hold_time = 0
	...
	def update_agreement_status(self):
		if self.service_level_agreement and self.agreement_status == "Ongoing":
			if (
				cint(capkpi.db.get_value("Issue", self.name, "response_by_variance")) < 0
				or cint(capkpi.db.get_value("Issue", self.name, "resolution_by_variance")) < 0
			):
	...
	def split_issue(self, subject, communication_id):
		...
		capkpi.get_doc(replicated_issue).insert()

		# Replicate linked Communications
		# TODO: get all communications in timeline before this, and modify them to append them to new doc
		comm_to_split_from = capkpi.get_doc("Communication", communication_id)
		...
		for communication in communications:
			doc = capkpi.get_doc("Communication", communication.name)
		...
		capkpi.get_doc(
			{
				...
				"content": " - Split the Issue from <a href='/app/Form/Issue/{0}'>{1}</a>".format(
					self.name, capkpi.bold(self.name)
				),
			}
	...
	def before_insert(self):
		if capkpi.db.get_single_value("Support Settings", "track_service_level_agreement"):
	...
	def set_response_and_resolution_time(self, priority=None, service_level_agreement=None):
		...
		if not service_level_agreement:
			if capkpi.db.get_value("Issue", self.name, "service_level_agreement"):
				capkpi.throw(
	...
	def change_service_level_agreement_and_priority(self):
		if (
			...
			and capkpi.db.exists("Issue", self.name)
			and capkpi.db.get_single_value("Support Settings", "track_service_level_agreement")
		):

			if not self.priority == capkpi.db.get_value("Issue", self.name, "priority"):
				self.set_response_and_resolution_time(
					priority=self.priority, service_level_agreement=self.service_level_agreement
				)
				capkpi.msgprint(_("Priority has been changed to {0}.").format(self.priority))

			if not self.service_level_agreement == capkpi.db.get_value(
	...
	def reset_service_level_agreement(self, reason, user):
		if not capkpi.db.get_single_value("Support Settings", "allow_resetting_service_level_agreement"):
			capkpi.throw(_("Allow Resetting Service Level Agreement from Support Settings."))
...
def get_priority(issue):
	service_level_agreement = capkpi.get_doc("Service Level Agreement", issue.service_level_agreement)
...
def get_expected_time_for(parameter, service_level, start_date_time):
	...
	if parameter == "response":
		...
		capkpi.throw(_("{0} parameter is invalid").format(parameter))

	expected_time_is_set = 0

	support_days = {}
	for service in service_level.get("support_and_resolution"):
		support_days[service.workday] = capkpi._dict(
			{
...
def set_service_level_agreement_variance(issue=None):
	current_time = capkpi.flags.current_time or now_datetime()

	filters = {"status": "Open", "agreement_status": "Ongoing"}
	if issue:
		filters = {"name": issue}

	for issue in capkpi.get_list("Issue", filters=filters):
		doc = capkpi.get_doc("Issue", issue.name)
...
def get_issue_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by=None):
	...
	contact = capkpi.db.get_value("Contact", {"user": user}, "name")
	customer = None

	if contact:
		contact_doc = capkpi.get_doc("Contact", contact)
...
def auto_close_tickets():
	...
	auto_close_after_days = (
		capkpi.db.get_value("Support Settings", "Support Settings", "close_issue_after_days") or 7
	)

	issues = capkpi.db.sql(
	...
	for issue in issues:
		doc = capkpi.get_doc("Issue", issue.get("name"))
		doc.status = "Closed"
...
def has_website_permission(doc, ptype, user, verbose=False):
	from erp.controllers.website_list_for_contact import has_website_permission
...
def update_issue(contact, method):
	...
	capkpi.db.sql("""UPDATE `tabIssue` set contact='' where contact=%s""", contact.name)


def get_holidays(holiday_list_name):
	holiday_list = capkpi.get_cached_doc("Holiday List", holiday_list_name)
...
def calculate_first_response_time(issue, first_responded_on):
	...
	support_hours = capkpi.get_cached_doc(
...
def get_working_hours(date, support_hours):
	if is_work_day(date, support_hours):
		weekday = capkpi.utils.get_weekday(date)
