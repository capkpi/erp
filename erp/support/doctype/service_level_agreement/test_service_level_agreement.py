...
from erp.hr.doctype.employee_group.test_employee_group import make_employee_group
from erp.support.doctype.issue_priority.test_issue_priority import make_priorities


class TestServiceLevelAgreement(unittest.TestCase):
	def setUp(self):
		capkpi.db.sql("delete from `tabService Level Agreement`")
		capkpi.db.set_value("Support Settings", None, "track_service_level_agreement", 1)
...
def get_service_level_agreement(
	...
	service_level_agreement = capkpi.get_doc("Service Level Agreement", filters)
...
def create_service_level_agreement(
	...
	service_level_agreement = capkpi.get_doc(
		{
			...
			"entity": entity,
			"start_date": capkpi.utils.getdate(),
			"end_date": capkpi.utils.add_to_date(capkpi.utils.getdate(), days=100),
			"priorities": [
				{
					"priority": "Low",
	...
	service_level_agreement_exists = capkpi.db.exists("Service Level Agreement", filters)

	if not service_level_agreement_exists:
		service_level_agreement.insert(ignore_permissions=True)
		return service_level_agreement
	else:
		return capkpi.get_doc("Service Level Agreement", service_level_agreement_exists)
...
def create_customer():
	...
	if not capkpi.db.exists("Customer", "_Test Customer"):
		customer.insert(ignore_permissions=True)
		return customer.name
	else:
		return capkpi.db.exists("Customer", "_Test Customer")


def create_customer_group():
	customer_group = capkpi.get_doc(
		{"doctype": "Customer Group", "customer_group_name": "_Test SLA Customer Group"}
	)

	if not capkpi.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"}):
		customer_group.insert()
		return customer_group.name
	else:
		return capkpi.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"})


def create_territory():
	territory = capkpi.get_doc(
		{
			"doctype": "Territory",
	...
	if not capkpi.db.exists("Territory", {"territory_name": "_Test SLA Territory"}):
		territory.insert()
		return territory.name
	else:
		return capkpi.db.exists("Territory", {"territory_name": "_Test SLA Territory"})
...
def make_holiday_list():
	holiday_list = capkpi.db.exists("Holiday List", "__Test Holiday List")
	if not holiday_list:
