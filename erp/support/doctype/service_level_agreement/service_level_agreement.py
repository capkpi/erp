...
from capkpi.utils import get_link_to_form, get_weekdays, getdate, nowdate
...
class ServiceLevelAgreement(Document):
	...
	def check_support_and_resolution(self):
		...
		if not len(set(support_days)) == len(support_days):
			...
			capkpi.throw(_("Workday {0} has been repeated.").format(repeated_days))

	def validate_doc(self):
		if (
			not capkpi.db.get_single_value("Support Settings", "track_service_level_agreement")
			and self.enable
		):
			capkpi.throw(
				_("{0} is not enabled in {1}").format(
					capkpi.bold("Track Service Level Agreement"),
		...
		if self.default_service_level_agreement:
			if capkpi.db.exists(
				...
				capkpi.throw(_("A Default Service Level Agreement already exists."))
		else:
			if self.start_date and self.end_date:
				if getdate(self.start_date) >= getdate(self.end_date):
					capkpi.throw(_("Start Date of Agreement can't be greater than or equal to End Date."))

				if getdate(self.end_date) < getdate(capkpi.utils.getdate()):
					capkpi.throw(_("End Date of Agreement can't be less than today."))
	...
	def validate_condition(self):
		temp_doc = capkpi.new_doc("Issue")
		if self.condition:
			try:
				capkpi.safe_eval(self.condition, None, get_context(temp_doc))
			except Exception:
				capkpi.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def get_service_level_agreement_priority(self, priority):
		priority = capkpi.get_doc("Service Level Priority", {"priority": priority, "parent": self.name})
...
def check_agreement_status():
	service_level_agreements = capkpi.get_list(
		"Service Level Agreement",
	...
	for service_level_agreement in service_level_agreements:
		doc = capkpi.get_doc("Service Level Agreement", service_level_agreement.name)
		if doc.end_date and getdate(doc.end_date) < getdate(capkpi.utils.getdate()):
			capkpi.db.set_value("Service Level Agreement", service_level_agreement.name, "active", 0)


def get_active_service_level_agreement_for(doc):
	if not capkpi.db.get_single_value("Support Settings", "track_service_level_agreement"):
		return
	...
	for agreement in agreements:
		...
		if not condition or (condition and capkpi.safe_eval(condition, None, get_context(doc))):
...
def get_customer_group(customer):
	if customer:
		return capkpi.db.get_value("Customer", customer, "customer_group")


def get_customer_territory(customer):
	if customer:
		return capkpi.db.get_value("Customer", customer, "territory")
...
def get_service_level_agreement_filters(name, customer=None):
	...
	return {
		"priority": [
			priority.priority
			for priority in capkpi.get_list(
				"Service Level Priority", filters={"parent": name}, fields=["priority"]
			)
		],
		"service_level_agreements": [
			d.name
			for d in capkpi.get_list("Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
