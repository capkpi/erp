...
from erp.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from erp.accounts.party import (
	get_party_account,
	get_party_account_currency,
	get_party_gle_currency,
	validate_party_frozen_disabled,
)
from erp.accounts.utils import get_account_currency, get_fiscal_years, validate_fiscal_year
from erp.buying.utils import update_last_purchase_rate
from erp.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from erp.controllers.sales_and_purchase_return import validate_return
from erp.exceptions import InvalidCurrency
from erp.setup.utils import get_exchange_rate
from erp.stock.doctype.item.item import get_uom_conv_factor
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = erp.get_company_currency(self.company)
	...
	def calculate_taxes_and_totals(self):
		from erp.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from erp.accounts.party import validate_due_date
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center", self.get("cost_center") or erp.get_default_cost_center(self.company)
						)
						...
						self.get_gl_dict(
							{
								...
								"cost_center": self.cost_center or erp.get_default_cost_center(self.company),
	...
	def update_against_document_in_jv(self):
		...
		if lst:
			from erp.accounts.utils import reconcile_against_document
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on, parentfield):
		from erp.controllers.status_updater import get_allowance_for
	...
	def check_conversion_rate(self):
		default_currency = erp.get_company_currency(self.company)
...
def update_bin_on_delete(row, doctype):
	...
	from erp.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
