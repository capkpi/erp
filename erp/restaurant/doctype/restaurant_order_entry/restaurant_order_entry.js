...
capkpi.ui.form.on('Restaurant Order Entry', {
	setup: function(frm) {
		let get_item_query = () => {
			return {
				query: 'erp.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				filters: {
	...
	onload_post_render: function(frm) {
		if(!frm.item_selector) {
			frm.item_selector = new erp.ItemSelector({
				frm: frm,
				item_field: 'item',
				item_query: 'erp.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				get_filters: () => {
	...
	restaurant_table: function(frm) {
		...
		return capkpi.call({
			method: 'erp.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.get_invoice',
			args: {
				table: frm.doc.restaurant_table
			},
			callback: (r) => {
	...
	sync: function(frm) {
		return capkpi.call({
			method: 'erp.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.sync',
			args: {
				table: frm.doc.restaurant_table,
				items: frm.doc.items
			},
			callback: (r) => {
				frm.events.set_invoice_items(frm, r);
				capkpi.show_alert({message: __('Saved'), indicator: 'green'});
			}
		});
	...
	make_invoice: function(frm) {
		frm.trigger('sync').then(() => {
			capkpi.prompt([
				...
				return capkpi.call({
					method: 'erp.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.make_invoice',
					args: {
						table: frm.doc.restaurant_table,
