...
class RestaurantMenu(Document):
	def validate(self):
		for d in self.items:
			if not d.rate:
				d.rate = capkpi.db.get_value("Item", d.item, "standard_rate")
	...
	def clear_item_price(self, price_list=None):
		...
		capkpi.db.sql("delete from `tabItem Price` where price_list = %s", price_list)
	...
	def make_price_list(self):
		...
		for d in self.items:
			capkpi.get_doc(
				dict(
					doctype="Item Price", price_list=price_list.name, item_code=d.item, price_list_rate=d.rate
				)
			).insert()

	def get_price_list(self):
		"""Create price list for menu if missing"""
		price_list_name = capkpi.db.get_value("Price List", dict(restaurant_menu=self.name))
		if price_list_name:
			price_list = capkpi.get_doc("Price List", price_list_name)
		else:
			price_list = capkpi.new_doc("Price List")
