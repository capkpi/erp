capkpi.ready(function() {
	...
	$(".file-size").each(function() {
		$(this).text(capkpi.form.formatters.FileSize($(this).text()));
	});
	...
	var reload_items = function(item_status, item, $btn) {
		$.ajax({
			...
			dataType: "json",
			data: {
				cmd: "erp.templates.pages.projects.get_"+ item +"_html",
				project: '{{ doc.name }}',
	...
	var close_item = function(item, item_name){
		...
		capkpi.call({
			btn: this,
			type: "POST",
			method: "erp.templates.pages.projects.set_"+ item +"_status",
			args: args,
			callback: function(r) {
				if(r.exc) {
