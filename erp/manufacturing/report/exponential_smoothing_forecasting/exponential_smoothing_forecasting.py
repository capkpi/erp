...
from capkpi.utils import add_years, cint, flt, getdate
...
class ForecastingReport(ExponentialSmoothingForecast):
	def __init__(self, filters=None):
		...
		self.company_currency = erp.get_company_currency(self.filters.company)
	...
	def get_data_for_forecast(self):
		parent = capkpi.qb.DocType(self.doctype)
		child = capkpi.qb.DocType(self.child_doctype)

		date_field = "posting_date" if self.doctype == "Delivery Note" else "transaction_date"

		query = (
			capkpi.qb.from_(parent)
			.from_(child)
			.select(
	...
	def add_total(self):
		...
		total_row = {"item_code": _(capkpi.bold("Total Quantity"))}
