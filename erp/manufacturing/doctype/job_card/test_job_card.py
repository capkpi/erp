...
from capkpi.utils.data import add_to_date, now

from erp.manufacturing.doctype.job_card.job_card import (
...
from erp.manufacturing.doctype.work_order.work_order import WorkOrder
from erp.manufacturing.doctype.workstation.test_workstation import make_workstation
from erp.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry


class TestJobCard(CapKPITestCase):
	def setUp(self):
	...
	def test_job_card_operations(self):
		...
		if job_cards:
			...
			capkpi.db.set_value("Job Card", job_card.name, "operation_row_number", job_card.operation_id)

			doc = capkpi.get_doc("Job Card", job_card.name)
	...
	def test_job_card_with_different_work_station(self):
		...
		if job_card:
			workstation = capkpi.db.get_value(
			...
			completed_qty = capkpi.db.get_value(
	...
	def test_job_card_overlap(self):
		...
		jc1 = capkpi.get_last_doc("Job Card", {"work_order": self.work_order.name})
		jc2 = capkpi.get_last_doc("Job Card", {"work_order": wo2.name})
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		job_card_name = capkpi.db.get_value("Job Card", {"work_order": self.work_order.name})
		job_card = capkpi.get_doc("Job Card", job_card_name)
	...
	def test_job_card_excess_material_transfer(self):
		...
		job_card = capkpi.get_last_doc("Job Card", {"work_order": self.work_order.name})
	...
	def test_job_card_material_transfer_correctness(self):
		...
		job_card_name = capkpi.db.get_value(
	...
	def test_corrective_costing(self):
		...
		corrective_action = capkpi.get_doc(
			doctype="Operation", is_corrective_operation=1, name=capkpi.generate_hash()
		).insert()
...
def create_bom_with_multiple_operations():
	...
	test_record = capkpi.get_test_records("BOM")[2]
	bom_doc = capkpi.get_doc(test_record)
