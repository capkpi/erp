...
capkpi.ui.form.on('Job Card', {
	setup: function(frm) {
		frm.set_query('operation', function() {
			return {
				query: 'erp.manufacturing.doctype.job_card.job_card.get_operations',
				filters: {
	...
	refresh: function(frm) {
		capkpi.flags.pause_job = 0;
		capkpi.flags.resume_job = 0;
		...
		if (frm.doc.docstatus == 0 && !frm.is_new() &&
			...
			if (frm.doc.work_order) {
				capkpi.db.get_value('Work Order', frm.doc.work_order, ['skip_transfer', 'status'], (result) => {
		...
		if (frm.doc.work_order) {
			capkpi.db.get_value('Work Order', frm.doc.work_order,
	...
	make_corrective_job_card: function(frm, operation, for_operation) {
		capkpi.call({
			method: 'erp.manufacturing.doctype.job_card.job_card.make_corrective_job_card',
			args: {
	...
	operation: function(frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			capkpi.call({
				method: "erp.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
					"work_order":frm.doc.work_order,
		...
		if (!frm.doc.started_time && !frm.doc.current_time) {
			frm.add_custom_button(__("Start Job"), () => {
				if ((frm.doc.employee && !frm.doc.employee.length) || !frm.doc.employee) {
					capkpi.prompt({fieldtype: 'Table MultiSelect', label: __('Select Employees'),
			...
			frm.add_custom_button(__("Complete Job"), () => {
				...
				if (set_qty) {
					capkpi.prompt({fieldtype: 'Float', label: __('Completed Quantity'),
	...
	start_job: function(frm, status, employee) {
		const args = {
			...
			start_time: capkpi.datetime.now_datetime(),
			employees: employee,
			status: status
		};
	...
	complete_job: function(frm, status, completed_qty) {
		const args = {
			job_card_id: frm.doc.name,
			complete_time: capkpi.datetime.now_datetime(),
			status: status,
	...
	make_time_log: function(frm, args) {
		...
		capkpi.call({
			method: "erp.manufacturing.doctype.job_card.job_card.make_time_log",
			args: {
				args: args
			},
			freeze: true,
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.started_time || frm.doc.current_time) {
			if (frm.doc.status == "On Hold") {
				...
				currentIncrement += moment(capkpi.datetime.now_datetime()).diff(moment(frm.doc.started_time),"seconds");
				initialiseTimer();
			}
	...
	make_material_request: function(frm) {
		capkpi.model.open_mapped_doc({
			method: "erp.manufacturing.doctype.job_card.job_card.make_material_request",
			frm: frm,
	...
	make_stock_entry: function(frm) {
		capkpi.model.open_mapped_doc({
			method: "erp.manufacturing.doctype.job_card.job_card.make_stock_entry",
			frm: frm,
			run_link_triggers: true
		});
	},
...
capkpi.ui.form.on('Job Card Time Log', {
