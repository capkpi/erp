...
capkpi.provide("erp.bom");

capkpi.ui.form.on("BOM", {
	setup: function(frm) {
		...
		frm.set_query("item", function() {
			return {
				query: "erp.manufacturing.doctype.bom.bom.item_query",
				filters: {
					"is_stock_item": 1
				}
			};
	...
	refresh: function(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus<2) {
			...
			frm.add_custom_button(__("Browse BOM"), function() {
				...
				capkpi.set_route("Tree", "BOM");
			});
		}

		if (!frm.is_new() && !frm.doc.docstatus == 0) {
			frm.add_custom_button(__("New Version"), function() {
				let new_bom = capkpi.model.copy_doc(frm.doc);
				capkpi.set_route("Form", "BOM", new_bom.name);
			});
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.$wrapper.find(".variants-intro").on("click", () => {
				capkpi.set_route("List", "Item", {"variant_of": frm.doc.item});
			});
		}
	},
	...
	make_work_order: function(frm) {
		frm.events.setup_variant_prompt(frm, "Work Order", (frm, item, data, variant_items) => {
			capkpi.call({
				method: "erp.manufacturing.doctype.work_order.work_order.make_work_order",
				args: {
					bom_no: frm.doc.name,
					item: item,
					qty: data.qty || 0.0,
	...
	make_variant_bom: function(frm) {
		frm.events.setup_variant_prompt(frm, "Variant BOM", (frm, item, data, variant_items) => {
			capkpi.call({
				method: "erp.manufacturing.doctype.bom.bom.make_variant_bom",
				args: {
	...
	setup_variant_prompt: function(frm, title, callback, skip_qty_field) {
				...
				fields: [
					...
					},
					{
						...
						get_query: function(data) {
							if (!data.item_code) {
								capkpi.throw(__("Select template item"));
							}
		...
		let dialog = capkpi.prompt(fields, data => {
			let item = data.item || frm.doc.item;
			let variant_items = data.items || [];

			variant_items.forEach(d => {
				if (!d.variant_item_code) {
					capkpi.throw(__("Select variant item code for the template item {0}", [d.item_code]));
				}
			})
	...
	make_quality_inspection: function(frm) {
		capkpi.model.open_mapped_doc({
			method: "erp.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm
		})
	},
		...
		if (frm.doc.routing) {
			capkpi.call({
				...
				callback: function(r) {
					if (!r.exc) {
						frm.refresh_fields();
						erp.bom.calculate_op_cost(frm.doc);
						erp.bom.calculate_total(frm.doc);
					}
				}
			});
		}
...
erp.bom.BomController = erp.TransactionController.extend({
	conversion_rate: function(doc) {
		if(this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			erp.bom.update_cost(doc);
		}
	},
...
$.extend(cur_frm.cscript, new erp.bom.BomController({frm: cur_frm}));

cur_frm.cscript.hour_rate = function(doc) {
	erp.bom.calculate_op_cost(doc);
	erp.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return capkpi.call({
			...
			callback: function(r) {
				...
				erp.bom.calculate_rm_cost(doc);
				erp.bom.calculate_scrap_materials_cost(doc);
				erp.bom.calculate_total(doc);
			},
			freeze: true
		});
...
cur_frm.cscript.rate = function(doc, cdt, cdn) {
	...
	if (d.bom_no) {
		capkpi.msgprint(__("You cannot change the rate if BOM is mentioned against any Item."));
		get_bom_material_detail(doc, cdt, cdn, scrap_items);
	} else {
		erp.bom.calculate_rm_cost(doc);
		erp.bom.calculate_scrap_materials_cost(doc);
...
erp.bom.update_cost = function(doc) {
	erp.bom.calculate_op_cost(doc);
	erp.bom.calculate_rm_cost(doc);
	erp.bom.calculate_scrap_materials_cost(doc);
	erp.bom.calculate_total(doc);
...
erp.bom.calculate_op_cost = function(doc) {
	...
	for(var i=0;i<op.length;i++) {
		...
		capkpi.model.set_value('BOM Operation',op[i].name, "operating_cost", operating_cost);
		capkpi.model.set_value('BOM Operation',op[i].name, "base_operating_cost", base_operating_cost);
...
// rm : raw material
erp.bom.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
	...
	for(var i=0;i<rm.length;i++) {
		...
		capkpi.model.set_value('BOM Item', rm[i].name, 'base_rate',
			flt(rm[i].rate) * flt(doc.conversion_rate));
		capkpi.model.set_value('BOM Item', rm[i].name, 'amount', amount);
		capkpi.model.set_value('BOM Item', rm[i].name, 'base_amount', base_amount);
		capkpi.model.set_value('BOM Item', rm[i].name,
...
// sm : scrap material
erp.bom.calculate_scrap_materials_cost = function(doc) {
	...
	for(var i=0;i<sm.length;i++) {
		...
		capkpi.model.set_value('BOM Scrap Item',sm[i].name, 'base_rate', base_rate);
		capkpi.model.set_value('BOM Scrap Item',sm[i].name, 'amount', amount);
		capkpi.model.set_value('BOM Scrap Item',sm[i].name, 'base_amount', base_amount);
...
erp.bom.calculate_total = function(doc) {
...
capkpi.ui.form.on("BOM Operation", "operation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
capkpi.ui.form.on("BOM Operation", "workstation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	capkpi.call({
		...
		callback: function (data) {
			capkpi.model.set_value(d.doctype, d.name, "base_hour_rate", data.message.hour_rate);
			capkpi.model.set_value(d.doctype, d.name, "hour_rate",
...
capkpi.ui.form.on("BOM Item", "qty", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	d.stock_qty = d.qty * d.conversion_factor;
	refresh_field("stock_qty", d.name, d.parentfield);
});

capkpi.ui.form.on("BOM Item", "item_code", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	capkpi.db.get_value('Item', {name: d.item_code}, 'allow_alternative_item', (r) => {
		d.allow_alternative_item = r.allow_alternative_item
	})
	refresh_field("allow_alternative_item", d.name, d.parentfield);
});

capkpi.ui.form.on("BOM Item", "sourced_by_supplier", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
capkpi.ui.form.on("BOM Item", "rate", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.sourced_by_supplier) {
		d.rate = 0;
		refresh_field("rate", d.name, d.parentfield);
...
capkpi.ui.form.on("BOM Operation", "operations_remove", function(frm) {
	erp.bom.calculate_op_cost(frm.doc);
	erp.bom.calculate_total(frm.doc);
});

capkpi.ui.form.on("BOM Item", "items_remove", function(frm) {
	erp.bom.calculate_rm_cost(frm.doc);
	erp.bom.calculate_total(frm.doc);
});


capkpi.ui.form.on("BOM Scrap Item", {
