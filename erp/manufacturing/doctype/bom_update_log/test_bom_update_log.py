...
from erp.manufacturing.doctype.bom_update_log.bom_update_log import (
	BOMMissingError,
	resume_bom_cost_update_jobs,
)
from erp.manufacturing.doctype.bom_update_tool.bom_update_tool import (
	enqueue_replace_bom,
	enqueue_update_cost,
)
...
class TestBOMUpdateLog(CapKPITestCase):
	...
	def test_bom_update_log_validate(self):
		...
		with self.assertRaises(capkpi.ValidationError):
			enqueue_replace_bom(boms=capkpi._dict(current_bom=self.boms.new_bom, new_bom=self.boms.new_bom))

		with self.assertRaises(capkpi.ValidationError):
			enqueue_replace_bom(boms=capkpi._dict(current_bom=self.boms.new_bom, new_bom="Dummy BOM"))
