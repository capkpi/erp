...
from erp.manufacturing.doctype.bom_update_log.bom_updation_utils import (
	get_leaf_boms,
...
class BOMMissingError(capkpi.ValidationError):
	pass
...
class BOMUpdateLog(Document):
	...
	def validate_same_bom(self):
		if cstr(self.current_bom) == cstr(self.new_bom):
			capkpi.throw(_("Current BOM and New BOM can not be same"))

	def validate_bom_items(self):
		current_bom_item = capkpi.db.get_value("BOM", self.current_bom, "item")
		new_bom_item = capkpi.db.get_value("BOM", self.new_bom, "item")

		if current_bom_item != new_bom_item:
			capkpi.throw(_("The selected BOMs are not for the same item"))

	def validate_bom_cost_update_in_progress(self):
		"If another Cost Updation Log is still in progress, dont make new ones."

		wip_log = capkpi.get_all(
			"BOM Update Log",
			{"update_type": "Update Cost", "status": ["in", ["Queued", "In Progress"]]},
			limit_page_length=1,
		)
		if wip_log:
			log_link = capkpi.utils.get_link_to_form("BOM Update Log", wip_log[0].name)
			capkpi.throw(
	...
	def on_submit(self):
		if self.update_type == "Replace BOM":
			...
			capkpi.enqueue(
				method="erp.manufacturing.doctype.bom_update_log.bom_update_log.run_replace_bom_job",
				doc=self,
				boms=boms,
				timeout=40000,
			...
			capkpi.enqueue(
				method="erp.manufacturing.doctype.bom_update_log.bom_update_log.process_boms_cost_level_wise",
				update_doc=self,
				now=capkpi.flags.in_test,
			)
...
def run_replace_bom_job(
	...
	try:
		...
		capkpi.db.auto_commit_on_many_writes = 1
		boms = capkpi._dict(boms or {})
		...
		capkpi.db.auto_commit_on_many_writes = 0
...
def queue_bom_cost_jobs(
	...
	while current_boms_list:
		...
		capkpi.enqueue(
			method="erp.manufacturing.doctype.bom_update_log.bom_updation_utils.update_cost_in_level",
			doc=update_doc,
...
def resume_bom_cost_update_jobs():
	...
	in_progress_logs = capkpi.db.get_all(
		"BOM Update Log",
	...
	for log in in_progress_logs:
		...
		if status == "Completed":
			capkpi.db.delete("BOM Update Batch", {"parent": log.name})

		if parent_boms:  # there is a next level to process
			process_boms_cost_level_wise(
				update_doc=capkpi.get_doc("BOM Update Log", log.name), parent_boms=parent_boms
			)
