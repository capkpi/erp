...
from erp.controllers.taxes_and_totals import get_itemised_tax
from erp.regional.italy import state_codes
...
def export_invoices(filters=None):
	capkpi.has_permission("Sales Invoice", throw=True)

	invoices = capkpi.get_all(
		"Sales Invoice", filters=get_conditions(filters), fields=["name", "company_tax_id"]
	)

	attachments = get_e_invoice_attachments(invoices)

	zip_filename = "{0}-einvoices.zip".format(capkpi.utils.get_datetime().strftime("%Y%m%d_%H%M%S"))
...
def prepare_invoice(invoice, progressive_number):
	...
	company = capkpi.get_doc("Company", invoice.company)
	...
	company_address = capkpi.get_doc("Address", invoice.company_address)
	...
	if not invoice.type_of_document:
		if invoice.is_return and invoice.return_against:
			...
			invoice.return_against_unamended = get_unamended_name(
				capkpi.get_doc("Sales Invoice", invoice.return_against)
			)
		else:
			invoice.type_of_document = "TD01"  # Sales Invoice (Fattura)

	# set customer information
	invoice.customer_data = capkpi.get_doc("Customer", invoice.customer)
	customer_address = capkpi.get_doc("Address", invoice.customer_address)
	invoice.customer_address_data = customer_address

	if invoice.shipping_address_name:
		invoice.shipping_address_data = capkpi.get_doc("Address", invoice.shipping_address_name)
...
def download_zip(files, output_filename):
	...
	with zipfile.ZipFile(zip_stream, "w", zipfile.ZIP_DEFLATED) as zip_file:
		for file in files:
			file_path = capkpi.utils.get_files_path(file.file_name, is_private=file.is_private)

			zip_file.write(file_path, arcname=file.file_name)

	capkpi.local.response.filename = output_filename
	capkpi.local.response.filecontent = zip_stream.getvalue()
	capkpi.local.response.type = "download"
	zip_stream.close()


def get_invoice_summary(items, taxes):
	summary_data = capkpi._dict()
	for tax in taxes:
			...
			if reference_row:
				items.append(
					capkpi._dict(
						...
						stock_uom=capkpi.db.get_single_value("Stock Settings", "stock_uom") or _("Nos"),
						tax_rate=tax.rate,
...
def sales_invoice_validate(doc):
	...
	company_fiscal_regime = capkpi.get_cached_value("Company", doc.company, "fiscal_regime")
	...
	doc.company_tax_id = capkpi.get_cached_value("Company", doc.company, "tax_id")
	doc.company_fiscal_code = capkpi.get_cached_value("Company", doc.company, "fiscal_code")
	...
	customer = capkpi.get_doc("Customer", doc.customer)
	...
	if not doc.customer_address:
		capkpi.throw(_("Please set the Customer Address"), title=_("E-Invoicing Information Missing"))
	else:
	...
	for schedule in doc.payment_schedule:
		if schedule.mode_of_payment and not schedule.mode_of_payment_code:
			schedule.mode_of_payment_code = capkpi.get_cached_value(
...
def sales_invoice_on_submit(doc, method):
	...
	if not len(doc.payment_schedule):
		capkpi.throw(_("Please set the Payment Schedule"), title=_("E-Invoicing Information Missing"))
	else:
		for schedule in doc.payment_schedule:
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	item_meta = capkpi.get_meta("Sales Invoice Item")

	invoice_xml = capkpi.render_template(
		"erp/regional/italy/e-invoice.xml",
...
def generate_single_invoice(docname):
	doc = capkpi.get_doc("Sales Invoice", docname)
	capkpi.has_permission("Sales Invoice", doc=doc, throw=True)
...
def get_company_country(company):
	return capkpi.get_cached_value("Company", company, "country")
...
def validate_address(address_name):
	...
	data = capkpi.get_cached_value("Address", address_name, fields, as_dict=1) or {}
...
def get_progressive_name_and_number(doc, replace=False):
	...
	progressive_name = capkpi.model.naming.make_autoname(company_tax_id + "_.#####")
