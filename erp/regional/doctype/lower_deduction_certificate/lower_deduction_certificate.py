...
class LowerDeductionCertificate(Document):
	...
	def validate_dates(self):
		if getdate(self.valid_upto) < getdate(self.valid_from):
			capkpi.throw(_("Valid Upto date cannot be before Valid From date"))

		fiscal_year = get_fiscal_year(fiscal_year=self.fiscal_year, as_dict=True)

		if not (fiscal_year.year_start_date <= getdate(self.valid_from) <= fiscal_year.year_end_date):
			capkpi.throw(_("Valid From date not in Fiscal Year {0}").format(capkpi.bold(self.fiscal_year)))

		if not (fiscal_year.year_start_date <= getdate(self.valid_upto) <= fiscal_year.year_end_date):
			capkpi.throw(_("Valid Upto date not in Fiscal Year {0}").format(capkpi.bold(self.fiscal_year)))

	def validate_supplier_against_tax_category(self):
		duplicate_certificate = capkpi.db.get_value(
		...
		if duplicate_certificate and self.are_dates_overlapping(duplicate_certificate):
			...
			capkpi.throw(
				_(
					...
					certificate_link, capkpi.bold(self.supplier), capkpi.bold(self.tax_withholding_category)
				)
			)
