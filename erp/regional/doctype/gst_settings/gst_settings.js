...
capkpi.ui.form.on('GST Settings', {
	refresh: function(frm) {
		frm.add_custom_button(__('Send GST Update Reminder'), () => {
			return new Promise((resolve) => {
				return capkpi.call({
					method: 'erp.regional.doctype.gst_settings.gst_settings.send_reminder'
				}).always(() => { resolve(); });
			});
		});
		...
		frm.add_custom_button(__('Sync HSN Codes'), () => {
			capkpi.call({
				"method": "erp.regional.doctype.gst_settings.gst_settings.update_hsn_codes"
			});
		});
