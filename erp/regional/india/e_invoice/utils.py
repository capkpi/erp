...
from capkpi.integrations.utils import make_get_request, make_post_request
...
from erp.regional.india.utils import get_gst_accounts, get_place_of_supply
...
def validate_eligibility(doc):
	...
	einvoicing_enabled = cint(capkpi.db.get_single_value("E Invoice Settings", "enable"))
	if not einvoicing_enabled:
		return False

	einvoicing_eligible_from = (
		capkpi.db.get_single_value("E Invoice Settings", "applicable_from") or "2021-04-01"
	)
	if getdate(doc.get("posting_date")) < getdate(einvoicing_eligible_from):
		return False

	invalid_company = not capkpi.db.get_value("E Invoice User", {"company": doc.get("company")})
	invalid_company_gstin = not capkpi.db.get_value(
...
def validate_einvoice_fields(doc):
	...
	if doc.docstatus == 0 and doc._action == "save":
		if doc.irn:
			capkpi.throw(_("You cannot edit the invoice after generating IRN"), title=_("Edit Not Allowed"))
		if len(doc.name) > 16:
		...
		capkpi.throw(_("You must generate IRN before submitting the document."), title=_("Missing IRN"))
...
def get_party_details(address_name, skip_gstin_validation=False):
	addr = capkpi.get_doc("Address", address_name)
	...
	party_address_details = capkpi._dict(
		dict(
...
def get_overseas_address_details(address_name):
	address_title, address_line1, address_line2, city = capkpi.db.get_value(
...
def get_item_list(invoice):
	...
	hide_discount_in_einvoice = cint(
		capkpi.db.get_single_value("E Invoice Settings", "dont_show_discounts_in_e_invoice")
	)
...
def get_invoice_value_details(invoice):
	invoice_value_details = capkpi._dict(dict())
...
def get_return_doc_reference(invoice):
	invoice_date = capkpi.db.get_value("Sales Invoice", invoice.return_against, "posting_date")
	return capkpi._dict(
...
def validate_mandatory_fields(invoice):
	...
	if not capkpi.db.get_value("Address", invoice.company_address, "gstin"):
		capkpi.throw(
			_(
	...
	if invoice.gst_category != "Overseas" and not capkpi.db.get_value(
...
def make_einvoice(invoice):
	...
	shipping_details = payment_details = prev_doc_details = eway_bill_details = capkpi._dict({})
	...
	# not yet implemented
	period_details = export_details = capkpi._dict({})
...
def log_error(data=None):
	...
	return capkpi.log_error(title=_("E Invoice Request Failed"), message=message)
...
class GSPConnector:
	...
	def set_invoice(self):
		...
		if self.doctype and self.docname:
			self.invoice = capkpi.get_cached_doc(self.doctype, self.docname)

	def set_credentials(self):
		self.e_invoice_settings = capkpi.get_cached_doc("E Invoice Settings")
	...
	def get_seller_gstin(self):
		gstin = capkpi.db.get_value("Address", self.invoice.company_address, "gstin")
		if not gstin:
			capkpi.throw(
	...
	def get_client_credentials(self):
		...
		return capkpi.conf.einvoice_client_id, capkpi.conf.einvoice_client_secret
	...
	def get_gstin_details(gstin):
		...
		if not hasattr(capkpi.local, "gstin_cache"):
			capkpi.local.gstin_cache = {}

		key = gstin
		gsp_connector = GSPConnector()
		details = gsp_connector.fetch_gstin_details(gstin)

		capkpi.local.gstin_cache[key] = details
		capkpi.cache().hset("gstin_cache", key, details)
		return details
			...
			if res.get("success"):
				...
				if irn_details:
					...
					raise RequestFailed(
						...
						Contact ERP support to resolve the issue."
					)
	...
	def fetch_and_attach_qrcode_from_irn(self):
		is_qrcode_file_attached = self.invoice.qrcode_image and capkpi.db.exists(
			"File",
			{
		...
		if not is_qrcode_file_attached:
			if self.invoice.signed_qr_code:
				...
				capkpi.msgprint(_("QR Code attached to the invoice."), alert=True)
			else:
				qrcode = self.get_qrcode_from_irn(self.invoice.irn)
				if qrcode:
					qrcode_file = self.create_qr_code_file(qrcode)
					capkpi.db.set_value("Sales Invoice", self.invoice.name, "qrcode_image", qrcode_file.file_url)
					capkpi.msgprint(_("QR Code attached to the invoice."), alert=True)
				else:
					capkpi.msgprint(_("QR Code not found for the IRN"), alert=True)
		else:
			capkpi.msgprint(_("QR Code is already Attached"), indicator="green", alert=True)
	...
	def get_ewb_details(self):
		...
		if not irn:
			capkpi.throw(_("IRN is mandatory to get E-Waybill Details. Please generate IRN first."))
	...
	def update_ewb_details(self, ewb_details=None):
		...
		if not self.invoice.irn:
			capkpi.throw(_("IRN is mandatory to update E-Waybill Details. Please generate IRN first."))
		if not ewb_details:
	...
	def cancel_eway_bill(self):
		...
		if ewb_details:
			...
			if ewb_status == "CNL":
				...
				msg += "<br><br>"
				msg += _(
					"You must first use the portal to cancel the e-way bill and then update the cancelled status in the ERP system."
				)
				capkpi.msgprint(msg)
			elif ewb_status == "ACT" and self.invoice.ewaybill != ewb_no:
			...
			capkpi.msgprint(_("E-Way Bill Details not found for this IRN."))
	...
	def raise_error(self, raise_exception=False, errors=None):
		...
		if errors:
			capkpi.throw(errors, title=title, as_list=1)
		else:
...
def get_einvoice(doctype, docname):
	invoice = capkpi.get_doc(doctype, docname)
...
def schedule_bulk_generate_irn(docnames):
	...
	capkpi.publish_realtime(
		...
		{"user": capkpi.session.user, "failures": failures, "invoices": docnames},
	)
...
def show_bulk_action_failure_message(failures):
	for doc in failures:
		...
		capkpi.msgprint(message, title=_("Bulk E-Invoice Generation Complete"), indicator="red")
