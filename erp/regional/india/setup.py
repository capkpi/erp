...
from erp.accounts.utils import FiscalYearError, get_fiscal_year
from erp.regional.india import states


def setup(company=None, patch=True):
	# Company independent fixtures should be called only once at the first company setup
	if patch or capkpi.db.count("Company", {"country": "India"}) <= 1:
...
def setup_company_independent_fixtures(patch=False):
	...
	capkpi.enqueue("erp.regional.india.setup.add_hsn_sac_codes", now=capkpi.flags.in_test)
...
def add_hsn_sac_codes():
	if capkpi.flags.in_test and capkpi.flags.created_hsn_codes:
		return
	...
	if capkpi.flags.in_test:
		capkpi.flags.created_hsn_codes = True


def create_hsn_codes(data, code_field):
	for d in data:
		hsn_code = capkpi.new_doc("GST HSN Code")
...
def add_custom_roles_for_reports():
	for report_name in (
		...
		if not capkpi.db.get_value("Custom Role", dict(report=report_name)):
			capkpi.get_doc(
				dict(
					doctype="Custom Role",
					report=report_name,
...
def add_print_formats():
	capkpi.reload_doc("regional", "print_format", "gst_tax_invoice")
	capkpi.reload_doc("accounts", "print_format", "gst_pos_invoice")
	capkpi.reload_doc("accounts", "print_format", "GST E-Invoice")

	capkpi.db.set_value("Print Format", "GST POS Invoice", "disabled", 0)
	capkpi.db.set_value("Print Format", "GST Tax Invoice", "disabled", 0)
	capkpi.db.set_value("Print Format", "GST E-Invoice", "disabled", 0)


def make_property_setters(patch=False):
	# GST rules do not allow for an invoice no. bigger than 16 characters
	journal_entry_types = capkpi.get_meta("Journal Entry").get_options("voucher_type").split("\n") + [
		"Reversal Of ITC"
	]
	sales_invoice_series = ["SINV-.YY.-", "SRET-.YY.-", ""] + capkpi.get_meta(
		"Sales Invoice"
	).get_options("naming_series").split("\n")
	purchase_invoice_series = ["PINV-.YY.-", "PRET-.YY.-", ""] + capkpi.get_meta(
		"Purchase Invoice"
	).get_options("naming_series").split("\n")
...
def make_fixtures(company=None):
	docs = []
	company = company or capkpi.db.get_value("Global Defaults", None, "default_company")
...
def update_regional_tax_settings(country, company):
	...
	gst_settings = capkpi.get_single("GST Settings")
...
def set_tax_withholding_category(company):
	...
	abbr = capkpi.get_value("Company", company, "abbr")
	tds_account = capkpi.get_value("Account", "TDS Payable - {0}".format(abbr), "name")
	...
	for d in docs:
		if not capkpi.db.exists("Tax Withholding Category", d.get("name")):
			doc = capkpi.get_doc(d)
			...
			doc = capkpi.get_doc("Tax Withholding Category", d.get("name"), for_update=True)
...
def create_gratuity_rule():
	...
	if not capkpi.db.exists("Gratuity Rule", "Indian Standard Gratuity Rule"):
...
def update_accounts_settings_for_taxes():
	if capkpi.db.count("Company") == 1:
		capkpi.db.set_value("Accounts Settings", None, "add_taxes_from_item_tax_template", 0)
