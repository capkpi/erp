...
from capkpi.utils import cstr, nowdate
from capkpi.utils.data import fmt_money
from capkpi.utils.jinja import render_template
from capkpi.utils.pdf import get_pdf
from capkpi.utils.print_format import read_multi_pdf
...
def execute(filters=None):
	filters = filters if isinstance(filters, capkpi._dict) else capkpi._dict(filters)
	if not filters:
		filters.setdefault("fiscal_year", get_fiscal_year(nowdate())[0])
		filters.setdefault("company", capkpi.db.get_default("company"))

	region = capkpi.db.get_value("Company", filters={"name": filters.company}, fieldname=["country"])
	...
	if filters.supplier_group:
		conditions += "AND s.supplier_group = %s" % capkpi.db.escape(filters.get("supplier_group"))
...
def irs_1099_print(filters):
	if not filters:
		capkpi._dict(
			{
				"company": capkpi.db.get_default("Company"),
				"fiscal_year": capkpi.db.get_default("Fiscal Year"),
			}
		)
	else:
	...
	company_tin = capkpi.db.get_value("Company", filters.company, "tax_id")

	columns, data = execute(filters)
	template = capkpi.get_doc("Print Format", "IRS 1099 Form").html
	...
	capkpi.local.response.filename = (
...
def get_payer_address_html(company):
	...
	if address_list:
		...
		address_display = capkpi.get_doc("Address", company_address).get_display()
...
def get_street_address_html(party_type, party):
	...
	if address_list:
		...
		doc = capkpi.get_doc("Address", supplier_address)
