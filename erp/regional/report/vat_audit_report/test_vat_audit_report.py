...
from erp.regional.report.vat_audit_report.vat_audit_report import execute
...
class TestVATAuditReport(TestCase):
	...
	def tearDown(self):
		capkpi.db.sql("delete from `tabSales Invoice` where company='_Test Company SA VAT'")
		capkpi.db.sql("delete from `tabPurchase Invoice` where company='_Test Company SA VAT'")
...
def set_sa_vat_accounts():
	if not capkpi.db.exists("South Africa VAT Settings", "_Test Company SA VAT"):
...
def make_customer():
	if not capkpi.db.exists("Customer", "_Test SA Customer"):
		capkpi.get_doc(
			{
				"doctype": "Customer",
...
def make_supplier():
	if not capkpi.db.exists("Supplier", "_Test SA Supplier"):
		capkpi.get_doc(
			{
				"doctype": "Supplier",
