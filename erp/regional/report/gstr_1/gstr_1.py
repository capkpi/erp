...
from capkpi.utils import flt, formatdate, getdate
from six import iteritems

from erp.regional.india.utils import get_gst_accounts
...
class Gstr1Report(object):
	...
	def get_invoice_data(self):
		self.invoices = capkpi._dict()
		conditions = self.get_conditions()

		invoice_data = capkpi.db.sql(
			"""
			select
				{select_columns}
			from `tab{doctype}`
	...
	def get_conditions(self):
		...
		if self.filters.get("type_of_business") in ("B2C Large", "B2C Small"):
			b2c_limit = capkpi.db.get_single_value("GST Settings", "b2c_limit")
			if not b2c_limit:
				capkpi.throw(_("Please set B2C Limit in GST Settings."))
	...
	def get_invoice_items(self):
		self.invoice_items = capkpi._dict()
		self.item_tax_rate = capkpi._dict()
		self.item_hsn_map = capkpi._dict()
	...
	def get_items_based_on_tax_rate(self):
		...
		self.tax_details = capkpi.db.sql(
			"""
			select
		...
		self.invoice_cess = capkpi._dict()
		...
		for item in report_data[:-1]:
			if not item.get("place_of_supply"):
				capkpi.throw(
					...
					).format(capkpi.bold("Place Of Supply"))
				)
			...
			if not invoice[0]["place_of_supply"]:
				capkpi.throw(
					...
					Please update and try again"""
						...
						capkpi.bold("Place Of Supply"), capkpi.bold(invoice[0]["invoice_number"])
					)
				)
...
def get_company_gstin_number(company, address=None, all_gstins=False):
	gstin = ""
	if address:
		gstin = capkpi.db.get_value("Address", address, "gstin")
	...
	if not gstin:
		address = capkpi.bold(address) if address else ""
		capkpi.throw(
			_("Please set valid GSTIN No. in Company Address {} for company {}").format(
				address, capkpi.bold(company)
			)
		)
...
def download_json_file():
	...
	capkpi.response["filename"] = (
		capkpi.scrub("{0} {1}".format(data["report_name"], data["report_type"])) + ".json"
	)
	capkpi.response["filecontent"] = data["data"]
	capkpi.response["content_type"] = "application/json"
...
def get_company_gstins(company):
	address = capkpi.qb.DocType("Address")
	links = capkpi.qb.DocType("Dynamic Link")

	addresses = (
		capkpi.qb.from_(address)
		.inner_join(links)
...
def get_hsn_wise_tax_rates():
	hsn_wise_tax_rate = {}
	gst_hsn_code = capkpi.qb.DocType("GST HSN Code")
	hsn_tax_rates = capkpi.qb.DocType("HSN Tax Rate")

	hsn_code_data = (
		capkpi.qb.from_(gst_hsn_code)
