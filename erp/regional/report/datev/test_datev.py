...
import capkpi
from capkpi.utils import cstr, now_datetime, today
...
from erp.regional.germany.utils.datev.datev_csv import get_datev_csv, get_header
from erp.regional.report.datev.datev import (
	download_datev_csv,
	get_account_names,
	get_customers,
	get_suppliers,
...
def setup_fiscal_year():
	...
	if not capkpi.db.get_value("Fiscal Year", {"year": year}, "name"):
		try:
...
def make_customer_with_account(customer_name, company):
	...
	if not capkpi.db.exists("Customer", customer_name):
		...
		customer = capkpi.get_doc("Customer", customer_name)

	return customer


def make_item(item_code, company):
	warehouse_name = capkpi.db.get_value(
...
def make_datev_settings(company):
	if not capkpi.db.exists("DATEV Settings", company.name):
		capkpi.get_doc(
			{
...
class TestDatev(TestCase):
	...
	def test_columns(self):
		def is_subset(get_data, allowed_keys):
			...
			allowed_set = set({capkpi.as_unicode(key) for key in allowed_keys})
	...
	def test_download(self):
		...
		zip_buffer = BytesIO()
		zip_buffer.write(capkpi.response["filecontent"])
