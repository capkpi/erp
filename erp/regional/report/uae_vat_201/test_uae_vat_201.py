...
from erp.regional.report.uae_vat_201.uae_vat_201 import (
	get_exempt_total,
...
from erp.stock.doctype.warehouse.test_warehouse import get_warehouse_account
...
class TestUaeVat201(TestCase):
	def setUp(self):
		...
		capkpi.db.sql("delete from `tabSales Invoice` where company='_Test Company UAE VAT'")
		capkpi.db.sql("delete from `tabPurchase Invoice` where company='_Test Company UAE VAT'")
...
def make_company(company_name, abbr):
	if not capkpi.db.exists("Company", company_name):
		...
		company = capkpi.get_doc("Company", company_name)

	company.create_default_warehouses()

	if not capkpi.db.get_value("Cost Center", {"is_group": 0, "company": company.name}):
...
def set_vat_accounts():
	if not capkpi.db.exists("UAE VAT Settings", "_Test Company UAE VAT"):
		vat_accounts = capkpi.get_all(
			"Account",
			fields=["name"],
...
def make_customer():
	if not capkpi.db.exists("Customer", "_Test UAE Customer"):
		...
		customer = capkpi.get_doc("Customer", "_Test UAE Customer")


def make_supplier():
	if not capkpi.db.exists("Supplier", "_Test UAE Supplier"):
		capkpi.get_doc(
			{
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	if not capkpi.db.exists("Warehouse", warehouse_id):
		warehouse = capkpi.new_doc("Warehouse")
