...
from erp.regional.report.gstr_1.gstr_1 import get_company_gstin_number
...
def _execute(filters=None):
	...
	company_currency = erp.get_company_currency(filters.company)
...
def get_output_gst_accounts(company):
	...
	accounts = capkpi.qb.DocType("Account")
	gst_accounts = capkpi.qb.DocType("GST Account")

	accounts_query = (
		capkpi.qb.from_(accounts)
		.select(accounts.name)
		.where((accounts.account_type == "Tax") & (accounts.root_type == "Liability"))
	)

	gst_accounts_query = (
		capkpi.qb.from_(gst_accounts)
		.select(
...
def get_items(filters):
	...
	match_conditions = capkpi.build_match_conditions("Sales Invoice")
	if match_conditions:
		...
		if item_wise_tax_detail:
			try:
				...
				for item_code, tax_data in item_wise_tax_detail.items():
					if not capkpi.db.get_value("Item", item_code, "gst_hsn_code"):
						continue
					...
					for d in item_row_map.get(parent, {}).get(item_code, []):
						...
						if item_tax_amount:
							itemised_tax.setdefault((parent, item_code), {})[account_head] = capkpi._dict(
								{
	...
	for account_head in tax_columns:
		if account_head in output_gst_accounts:
			columns.append(
				{
					"label": account_head,
					"fieldname": capkpi.scrub(account_head),
					"fieldtype": "Float",
					"width": 110,
				}
			)
...
def get_json(filters, report_name, data):
	...
	if not filters.get("from_date") or not filters.get("to_date"):
		capkpi.throw(_("Please enter From Date and To Date to generate JSON"))
...
def download_json_file():
	...
	capkpi.response["filename"] = capkpi.scrub("{0}".format(data["report_name"])) + ".json"
...
def get_hsn_wise_json_data(filters, report_data):
	...
	for hsn in report_data:
		...
		for account in gst_accounts.get("igst_account"):
			row["iamt"] += flt(hsn.get(capkpi.scrub(cstr(account)), 0.0), 2)

		for account in gst_accounts.get("cgst_account"):
			row["camt"] += flt(hsn.get(capkpi.scrub(cstr(account)), 0.0), 2)

		for account in gst_accounts.get("sgst_account"):
			row["samt"] += flt(hsn.get(capkpi.scrub(cstr(account)), 0.0), 2)

		for account in gst_accounts.get("cess_account"):
			row["csamt"] += flt(hsn.get(capkpi.scrub(cstr(account)), 0.0), 2)
