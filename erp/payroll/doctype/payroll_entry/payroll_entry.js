...
capkpi.ui.form.on('Payroll Entry', {
	onload: function (frm) {
		if (!frm.doc.posting_date) {
			frm.doc.posting_date = capkpi.datetime.nowdate();
		}
	...
	refresh: function (frm) {
		if (frm.doc.docstatus == 0) {
			...
			if ((frm.doc.employees || []).length && !capkpi.model.has_workflow(frm.doctype)) {
				frm.page.clear_primary_action();
	...
	add_bank_entry_button: function (frm) {
		capkpi.call({
			method: 'erp.payroll.doctype.payroll_entry.payroll_entry.payroll_entry_has_bank_entries',
			args: {
				'name': frm.doc.name
			},
			callback: function (r) {
	...
	setup: function (frm) {
		...
		frm.set_query('employee', 'employees', () => {
			...
			mandatory_fields.forEach(field => {
				if (!frm.doc[field]) {
					error_fields.push(capkpi.unscrub(field));
				}
			});
			...
			return {
				query: "erp.payroll.doctype.payroll_entry.payroll_entry.employee_query",
	...
	set_payable_account_and_currency: function (frm) {
		capkpi.db.get_value("Company", {"name": frm.doc.company}, "default_currency", (r) => {
			frm.set_value('currency', r.default_currency);
		});
		capkpi.db.get_value("Company", {"name": frm.doc.company}, "default_payroll_payable_account", (r) => {
	...
	set_start_end_dates: function (frm) {
		if (!frm.doc.salary_slip_based_on_timesheet) {
			capkpi.call({
				method: 'erp.payroll.doctype.payroll_entry.payroll_entry.get_start_end_dates',
				args: {
...
const submit_salary_slip = function (frm) {
	capkpi.confirm(__('This will submit Salary Slips and create accrual Journal Entry. Do you want to proceed?'),
		function () {
			capkpi.call({
...
let make_bank_entry = function (frm) {
	...
	if (doc.payment_account) {
		...
		capkpi.msgprint(__("Payment Account is mandatory"));
...
let render_employee_attendance = function (frm, data) {
	frm.fields_dict.attendance_detail_html.html(
		capkpi.render_template('employees_to_mark_attendance', {
			data: data
		})
	);
};
