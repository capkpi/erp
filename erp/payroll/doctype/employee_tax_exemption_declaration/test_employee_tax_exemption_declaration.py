...
from erp.hr.utils import DuplicateDeclarationError


class TestEmployeeTaxExemptionDeclaration(CapKPITestCase):
	def setUp(self):
		...
		capkpi.db.delete("Employee Tax Exemption Declaration")
	...
	def test_duplicate_category_in_declaration(self):
		declaration = capkpi.get_doc(
			{
				...
				"employee": capkpi.get_value("Employee", {"user_id": "employee@taxexemption.com"}, "name"),
		...
		self.assertRaises(capkpi.ValidationError, declaration.save)
	...
	def test_duplicate_entry_for_payroll_period(self):
		...
		duplicate_declaration = capkpi.get_doc(
			{
				"doctype": "Employee Tax Exemption Declaration",
				"employee": capkpi.get_value("Employee", {"user_id": "employee@taxexemption.com"}, "name"),
				"company": erp.get_default_company(),
		...
		duplicate_declaration.employee = capkpi.get_value(
	...
	def test_india_hra_exemption_with_multiple_salary_structure_assignments(self):
		from erp.payroll.doctype.salary_slip.test_salary_slip import create_tax_slab
...
def create_payroll_period(**args):
	...
	if not capkpi.db.exists("Payroll Period", name):
		from datetime import date

		payroll_period = capkpi.get_doc(
			dict(
				doctype="Payroll Period",
				name=name,
				company=args.company or erp.get_default_company(),
		...
		return capkpi.get_doc("Payroll Period", name)


def create_exemption_category():
	if not capkpi.db.exists("Employee Tax Exemption Category", "_Test Category"):
		category = capkpi.get_doc(
			{
	...
	if not capkpi.db.exists("Employee Tax Exemption Sub Category", "_Test Sub Category"):
		capkpi.get_doc(
			{
	...
	if not capkpi.db.exists("Employee Tax Exemption Sub Category", "_Test1 Sub Category"):
		capkpi.get_doc(
			{
				"doctype": "Employee Tax Exemption Sub Category",
