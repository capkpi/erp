...
from capkpi.utils import add_days, add_months, floor, flt, get_datetime, get_first_day, getdate

from erp.hr.doctype.employee.test_employee import make_employee
from erp.hr.doctype.expense_claim.test_expense_claim import get_payable_account
from erp.hr.doctype.holiday_list.test_holiday_list import set_holiday_list
from erp.payroll.doctype.gratuity.gratuity import get_last_salary_slip
...
class TestGratuity(CapKPITestCase):
	def setUp(self):
		capkpi.db.delete("Gratuity")
		capkpi.db.delete("Salary Slip")
		capkpi.db.delete("Additional Salary", {"ref_doctype": "Gratuity"})
	...
	def test_gratuity_based_on_current_slab_via_additional_salary(self):
		...
		# amount calculation
		component_amount = capkpi.get_all(
			"Salary Detail",
			filters={
				"docstatus": 1,
				"parent": sal_slip,
		...
		self.assertTrue(capkpi.db.exists("Additional Salary", {"ref_docname": gratuity.name}))
...
def get_gratuity_rule(name):
	rule = capkpi.db.exists("Gratuity Rule", name)
	if not rule:
		create_gratuity_rule()
	rule = capkpi.get_doc("Gratuity Rule", name)
...
def create_gratuity(**args):
	...
	gratuity = capkpi.new_doc("Gratuity")
...
def set_mode_of_payment_account():
	...
	mode_of_payment = capkpi.get_doc("Mode of Payment", "Cash")
...
def create_salary_slip(employee):
	if not capkpi.db.exists("Salary Slip", {"employee": employee}):
