...
import capkpi
from capkpi.utils import add_years, date_diff, get_first_day, nowdate
...
class TestSalaryStructure(unittest.TestCase):
	...
	def make_holiday_list(self):
		if not capkpi.db.get_value("Holiday List", "Salary Structure Test Holiday List"):
	...
	def test_amount_totals(self):
		...
		sal_slip = capkpi.get_value("Salary Slip", {"employee_name": "test_employee_2@salary.com"})
		if not sal_slip:
	...
	def test_salary_structures_assignment(self):
		company_currency = erp.get_default_currency()
		...
		salary_structure_assignment = capkpi.get_doc(
...
def make_salary_structure(
	...
	if test_tax:
		capkpi.db.sql("""delete from `tabSalary Structure` where name=%s""", (salary_structure))
	...
	if (
		employee
		and not capkpi.db.get_value("Salary Structure Assignment", filters)
...
def create_salary_structure_assignment(
	...
	if not allow_duplicate and capkpi.db.exists(
		"Salary Structure Assignment", {"employee": employee}
	):
		capkpi.db.sql("""delete from `tabSalary Structure Assignment` where employee=%s""", (employee))

	if not payroll_period:
		payroll_period = create_payroll_period()

	income_tax_slab = capkpi.db.get_value("Income Tax Slab", {"currency": currency})

	if not income_tax_slab:
		income_tax_slab = create_tax_slab(payroll_period, allow_tax_exemption=True, currency=currency)

	salary_structure_assignment = capkpi.new_doc("Salary Structure Assignment")
	...
	if not from_date:
		...
		joining_date = capkpi.get_cached_value("Employee", employee, "date_of_joining")
	...
	salary_structure_assignment.company = company or erp.get_default_company()
...
def get_payable_account(company=None):
	...
	return capkpi.db.get_value("Company", company, "default_payroll_payable_account")
