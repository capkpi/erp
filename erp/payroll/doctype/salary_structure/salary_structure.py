...
class SalaryStructure(Document):
	...
	def set_missing_values(self):
		...
		for table in ["earnings", "deductions"]:
			for d in self.get(table):
				component_default_value = capkpi.db.get_value(
					"Salary Component",
	...
	def validate_amount(self):
		if flt(self.net_pay) < 0 and self.salary_slip_based_on_timesheet:
			capkpi.throw(_("Net pay cannot be negative"))
			...
			for row in self.get(component_type):
				if (
					...
					message = _("Row #{0}: The {1} Component has the options {2} and {3} enabled.").format(
						row.idx,
						capkpi.bold(row.salary_component),
						capkpi.bold("Amount based on formula"),
						capkpi.bold("Depends On Payment Days"),
					)
					message += "<br><br>" + _(
						...
						capkpi.bold("Depends On Payment Days"), capkpi.bold(row.salary_component)
					)
					capkpi.throw(message, title=_("Payment Days Dependency"))
		...
		for component in self.earnings:
			if component.salary_component == self.salary_component:
				capkpi.msgprint(
					...
					).format(self.idx, capkpi.bold(self.salary_component)),
					title=_("Warning"),
					indicator="orange",
				)
				break
		...
		if self.earnings:
			...
			for earning_component in self.earnings:
				if earning_component.is_flexible_benefit == 1:
					have_a_flexi = True
					max_of_component = capkpi.db.get_value(
			...
			if have_a_flexi and flt(self.max_benefits) == 0:
				capkpi.throw(_("Max benefits should be greater than zero to dispense benefits"))
	...
	def assign_salary_structure(
		...
		if employees:
			...
			capkpi.msgprint(_("No Employee Found"))
...
def assign_salary_structure_for_employees(
	...
	if salary_structures_assignments:
		capkpi.msgprint(_("Structures have been assigned successfully"))
...
def create_salary_structures_assignment(
	...
	if not payroll_payable_account:
		...
		if not payroll_payable_account:
			capkpi.throw(_('Please set "Default Payroll Payable Account" in Company Defaults'))
	payroll_payable_account_currency = capkpi.db.get_value(
		"Account", payroll_payable_account, "account_currency"
	)
	company_curency = erp.get_company_currency(salary_structure.company)
	if (
	...
	assignment = capkpi.new_doc("Salary Structure Assignment")
...
def get_existing_assignments(employees, salary_structure, from_date):
	salary_structures_assignments = capkpi.db.sql_list(
		"""
...
def make_salary_slip(
	...
	def postprocess(source, target):
		if employee:
			employee_details = capkpi.db.get_value(
				"Employee",
				employee,
				["employee_name", "branch", "designation", "department", "payroll_cost_center"],
				as_dict=1,
			)
			...
			if not target.payroll_cost_center and target.department:
				target.payroll_cost_center = capkpi.db.get_value(
	...
	if cint(as_print):
		...
		return capkpi.get_print(doc.doctype, doc.name, doc=doc, print_format=print_format)
	else:
		return doc
