...
class DuplicateAssignment(capkpi.ValidationError):
	pass
...
class SalaryStructureAssignment(Document):
	...
	def validate_dates(self):
		...
		if self.from_date:
			if capkpi.db.exists(
				...
				capkpi.throw(_("Salary Structure Assignment for Employee already exists"), DuplicateAssignment)
	...
	def validate_income_tax_slab(self):
		...
		income_tax_slab_currency = capkpi.db.get_value(
			...
			if not payroll_payable_account:
				payroll_payable_account = capkpi.db.get_value(
					"Account",
					{
						...
						"account_currency": capkpi.db.get_value("Company", self.company, "default_currency"),
						"is_group": 0,
					},
				)
...
def get_assigned_salary_structure(employee, on_date):
	if not employee or not on_date:
		return None
	salary_structure = capkpi.db.sql(
		"""
...
def get_employee_currency(employee):
	employee_currency = capkpi.db.get_value(
