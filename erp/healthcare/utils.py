...
from capkpi.utils import cstr, rounded, time_diff_in_hours
from capkpi.utils.formatters import format_value

from erp.healthcare.doctype.fee_validity.fee_validity import create_fee_validity
from erp.healthcare.doctype.healthcare_settings.healthcare_settings import get_income_account
from erp.healthcare.doctype.lab_test.lab_test import create_multiple
...
def validate_customer_created(patient):
	if not capkpi.db.get_value("Patient", patient.name, "customer"):
...
def get_appointments_to_invoice(patient, company):
	...
	patient_appointments = capkpi.get_list(
		"Patient Appointment",
		fields="*",
		filters={
	...
	for appointment in patient_appointments:
		...
		# Consultation Appointments, should check fee validity
			...
			) and capkpi.db.exists("Fee Validity Reference", {"appointment": appointment.name}):
		...
		for encounter in encounters:
			if not encounter.appointment:
				...
				if encounter.practitioner:
					if encounter.inpatient_record and capkpi.db.get_single_value(
...
def get_lab_tests_to_invoice(patient, company):
	...
	lab_prescriptions = capkpi.db.sql(
		"""
			SELECT
...
def get_clinical_procedures_to_invoice(patient, company):
	...
	procedure_prescriptions = capkpi.db.sql(
		"""
			SELECT
...
def get_inpatient_services_to_invoice(patient, company):
	services_to_invoice = []
	inpatient_services = capkpi.db.sql(
		"""
			SELECT
				io.*
			FROM
	...
	for inpatient_occupancy in inpatient_services:
		service_unit_type = capkpi.db.get_value(
			"Healthcare Service Unit", inpatient_occupancy.service_unit, "service_unit_type"
		)
		service_unit_type = capkpi.get_cached_doc("Healthcare Service Unit Type", service_unit_type)
...
def get_therapy_sessions_to_invoice(patient, company):
	...
	for therapy in therapy_sessions:
		if not therapy.appointment:
			if therapy.therapy_type and capkpi.db.get_value(
				...
				therapy_sessions_to_invoice.append(
					{
						...
						"service": capkpi.db.get_value("Therapy Type", therapy.therapy_type, "item"),
					}
				)
...
def get_appointment_type_service_item(appointment_type, department, is_inpatient):
	from erp.healthcare.doctype.appointment_type.appointment_type import (
...
def throw_config_service_item(is_inpatient):
	...
	capkpi.throw(msg, title=_("Missing Configuration"))
...
def get_practitioner_service_item(practitioner, is_inpatient):
	...
	if is_inpatient:
		service_item, practitioner_charge = capkpi.db.get_value(
...
def get_healthcare_service_item(is_inpatient):
	...
	if is_inpatient:
		service_item = capkpi.db.get_single_value("Healthcare Settings", "inpatient_visit_charge_item")
	else:
		service_item = capkpi.db.get_single_value("Healthcare Settings", "op_consulting_charge_item")
...
def get_practitioner_charge(practitioner, is_inpatient):
	if is_inpatient:
		practitioner_charge = capkpi.db.get_value(
	...
	if doc.items:
		for item in doc.items:
			if item.get("reference_dt") and item.get("reference_dn"):
				if capkpi.get_meta(item.reference_dt).has_field("invoiced"):
					set_invoiced(item, method, doc.name)

	if method == "on_submit" and capkpi.db.get_single_value(
...
def set_invoiced(item, method, ref_invoice=None):
	...
	if item.reference_dt == "Clinical Procedure":
		...
		if service_item == item.item_code:
			capkpi.db.set_value(item.reference_dt, item.reference_dn, "consumption_invoiced", invoiced)
		else:
			capkpi.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)
	else:
		capkpi.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)

	if item.reference_dt == "Patient Appointment":
		if capkpi.db.get_value("Patient Appointment", item.reference_dn, "procedure_template"):
...
def validate_invoiced_on_submit(item):
	if (
		...
		and capkpi.db.get_single_value("Healthcare Settings", "clinical_procedure_consumable_item")
		== item.item_code
	):
		is_invoiced = capkpi.db.get_value(item.reference_dt, item.reference_dn, "consumption_invoiced")
	else:
		is_invoiced = capkpi.db.get_value(item.reference_dt, item.reference_dn, "invoiced")
	if is_invoiced:
		capkpi.throw(
...
def manage_prescriptions(invoiced, ref_dt, ref_dn, dt, created_check_field):
	created = capkpi.db.get_value(ref_dt, ref_dn, created_check_field)
	if created:
		# Fetch the doc created for the prescription
		doc_created = capkpi.db.get_value(dt, {"prescription": ref_dn})
		capkpi.db.set_value(dt, doc_created, "invoiced", invoiced)
...
def check_fee_validity(appointment):
	...
	validity = capkpi.get_doc("Fee Validity", validity)
	return validity
...
def manage_fee_validity(appointment):
	...
	if fee_validity:
		if appointment.status == "Cancelled" and fee_validity.visited > 0:
			...
			capkpi.db.delete("Fee Validity Reference", {"appointment": appointment.name})
...
def manage_doc_for_appointment(dt_from_appointment, appointment, invoiced):
	dn_from_appointment = capkpi.db.get_value(
		dt_from_appointment, filters={"appointment": appointment}
	)
	if dn_from_appointment:
		capkpi.db.set_value(dt_from_appointment, dn_from_appointment, "invoiced", invoiced)


@capkpi.whitelist()
def get_drugs_to_invoice(encounter):
	encounter = capkpi.get_doc("Patient Encounter", encounter)
	if encounter:
			...
			if patient.customer:
				...
				for drug_line in encounter.drug_prescription:
					if drug_line.drug_code:
						qty = 1
						if capkpi.db.get_value("Item", drug_line.drug_code, "stock_uom") == "Nos":
...
def render_doc_as_html(doctype, docname, exclude_fields=None):
	...
	for df in meta.fields:
		...
		if df.fieldtype == "Table":
			...
			if not items:
				continue
			child_meta = capkpi.get_meta(df.options)
...
def update_address_links(address, method):
	...
	for link in patient_links:
		customer = capkpi.db.get_value("Patient", link.get("link_name"), "customer")
...
def update_patient_email_and_phone_numbers(contact, method):
	...
	"""
	if "Healthcare" not in capkpi.get_active_domains() or contact.flags.skip_patient_update:
		return
	...
	if contact.is_primary_contact and (contact.email_id or contact.mobile_no or contact.phone):
		...
		for link in patient_links:
			contact_details = capkpi.db.get_value(
			...
			if new_contact_details:
				capkpi.db.set_value("Patient", link.get("link_name"), new_contact_details)
