...
import capkpi
from capkpi.utils import add_days, now_datetime, nowdate

from erp.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from erp.healthcare.doctype.patient_appointment.patient_appointment import (
...
class TestPatientAppointment(unittest.TestCase):
	def setUp(self):
		capkpi.db.sql("""delete from `tabPatient Appointment`""")
		capkpi.db.sql("""delete from `tabFee Validity`""")
		capkpi.db.sql("""delete from `tabPatient Encounter`""")
		make_pos_profile()
		capkpi.db.sql("""delete from `tabHealthcare Service Unit` where name like '_Test %'""")
		capkpi.db.sql(
	...
	def test_status(self):
		...
		capkpi.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 0)
	...
	def test_start_encounter(self):
		patient, practitioner = create_healthcare_docs()
		capkpi.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 1)
		...
		self.assertEqual(
			encounter.invoiced, capkpi.db.get_value("Patient Appointment", appointment.name, "invoiced")
		)

	def test_auto_invoicing(self):
		patient, practitioner = create_healthcare_docs()
		capkpi.db.set_value("Healthcare Settings", None, "enable_free_follow_ups", 0)
		capkpi.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 0)
		appointment = create_appointment(patient, practitioner, nowdate())
		self.assertEqual(capkpi.db.get_value("Patient Appointment", appointment.name, "invoiced"), 0)

		capkpi.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 1)
		appointment = create_appointment(patient, practitioner, add_days(nowdate(), 2), invoice=1)
		self.assertEqual(capkpi.db.get_value("Patient Appointment", appointment.name, "invoiced"), 1)
		sales_invoice_name = capkpi.db.get_value(
			"Sales Invoice Item", {"reference_dn": appointment.name}, "parent"
		)
		self.assertTrue(sales_invoice_name)
		self.assertEqual(
			capkpi.db.get_value("Sales Invoice", sales_invoice_name, "company"), appointment.company
		)
		self.assertEqual(
			capkpi.db.get_value("Sales Invoice", sales_invoice_name, "patient"), appointment.patient
		)
		self.assertEqual(
			capkpi.db.get_value("Sales Invoice", sales_invoice_name, "paid_amount"), appointment.paid_amount
		)
	...
	def test_appointment_cancel(self):
		patient, practitioner = create_healthcare_docs()
		capkpi.db.set_value("Healthcare Settings", None, "enable_free_follow_ups", 1)
		appointment = create_appointment(patient, practitioner, nowdate())
		fee_validity = capkpi.db.get_value(
		...
		self.assertEqual(capkpi.db.get_value("Fee Validity", fee_validity, "visited"), 1)

		update_status(appointment.name, "Cancelled")
		# check fee validity updated
		self.assertEqual(capkpi.db.get_value("Fee Validity", fee_validity, "visited"), 0)
		...
		self.assertEqual(capkpi.db.get_value("Sales Invoice", sales_invoice_name, "status"), "Cancelled")

	def test_appointment_booking_for_admission_service_unit(self):
		from erp.healthcare.doctype.inpatient_record.inpatient_record import (
			admit_patient,
			discharge_patient,
			schedule_discharge,
		)
		from erp.healthcare.doctype.inpatient_record.test_inpatient_record import (
			create_inpatient,
			get_healthcare_service_unit,
			mark_invoiced_inpatient_occupancy,
		)

		capkpi.db.sql("""delete from `tabInpatient Record`""")
		patient, practitioner = create_healthcare_docs()
		...
		# Discharge
		schedule_discharge(
			capkpi.as_json({"patient": patient, "discharge_ordered_datetime": now_datetime()})
		)
		ip_record1 = capkpi.get_doc("Inpatient Record", ip_record.name)
	...
	def test_invalid_healthcare_service_unit_validation(self):
		...
		self.assertRaises(capkpi.exceptions.ValidationError, appointment.save)
	...
	def test_payment_should_be_mandatory_for_new_patient_appointment(self):
		capkpi.db.set_value("Healthcare Settings", None, "enable_free_follow_ups", 1)
		capkpi.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 1)
		capkpi.db.set_value("Healthcare Settings", None, "max_visits", 3)
		capkpi.db.set_value("Healthcare Settings", None, "valid_days", 30)
	...
	def test_sales_invoice_should_be_generated_for_new_patient_appointment(self):
		patient, practitioner = create_healthcare_docs()
		capkpi.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 1)
		invoice_count = capkpi.db.count("Sales Invoice")

		assert check_is_new_patient(patient)
		create_appointment(patient, practitioner, nowdate())
		new_invoice_count = capkpi.db.count("Sales Invoice")

		assert new_invoice_count == invoice_count + 1

	def test_overlap_appointment(self):
		from erp.healthcare.doctype.patient_appointment.patient_appointment import OverlapError
	...
	def test_patient_appointment_should_consider_permissions_while_fetching_appointments(self):
		...
		new_practitioner = capkpi.get_doc("Healthcare Practitioner", new_practitioner)
		new_practitioner.user_id = user.email
		new_practitioner.save()

		capkpi.set_user(user.name)
		appointments = capkpi.get_list("Patient Appointment")
...
def create_patient(
	...
	if capkpi.db.exists("Patient", {"firstname": f"_Test Patient {str(id)}"}):
		patient = capkpi.db.get_value("Patient", {"first_name": f"_Test Patient {str(id)}"}, ["name"])
		return patient
...
def create_medical_department(id=0):
	if capkpi.db.exists("Medical Department", f"_Test Medical Department {str(id)}"):
		return f"_Test Medical Department {str(id)}"

	medical_department = capkpi.new_doc("Medical Department")
...
def create_practitioner(id=0, medical_department=None):
	if capkpi.db.exists(
		...
		practitioner = capkpi.db.get_value(
			"Healthcare Practitioner", {"firstname": f"_Test Healthcare Practitioner {str(id)}"}, ["name"]
		)
		return practitioner

	practitioner = capkpi.new_doc("Healthcare Practitioner")
...
def create_appointment(
	...
	item = create_healthcare_service_items()
	capkpi.db.set_value("Healthcare Settings", None, "inpatient_visit_charge_item", item)
	capkpi.db.set_value("Healthcare Settings", None, "op_consulting_charge_item", item)
...
def create_healthcare_service_items():
	if capkpi.db.exists("Item", "HLC-SI-001"):
		return "HLC-SI-001"
...
def create_clinical_procedure_template():
	if capkpi.db.exists("Clinical Procedure Template", "Knee Surgery and Rehab"):
		return capkpi.get_doc("Clinical Procedure Template", "Knee Surgery and Rehab")

	template = capkpi.new_doc("Clinical Procedure Template")
...
def create_appointment_type(args=None):
	...
	if capkpi.db.exists("Appointment Type", name):
		return capkpi.get_doc("Appointment Type", name)
		...
		return capkpi.get_doc(
			{
				...
				"price_list": args.get("price_list") or capkpi.db.get_value("Price List", {"selling": 1}),
...
def create_service_unit_type(id=0, allow_appointments=1, overlap_appointments=0):
	if capkpi.db.exists("Healthcare Service Unit Type", f"_Test Service Unit Type {str(id)}"):
		return f"_Test Service Unit Type {str(id)}"

	service_unit_type = capkpi.new_doc("Healthcare Service Unit Type")
...
def create_service_unit(id=0, service_unit_type=None, service_unit_capacity=0):
	if capkpi.db.exists("Healthcare Service Unit", f"_Test Service Unit {str(id)}"):
		return f"_Test service_unit {str(id)}"

	service_unit = capkpi.new_doc("Healthcare Service Unit")
...
def create_user(email=None, roles=None):
	if not email:
		email = "{}@capkpi.com".format(capkpi.utils.random_string(10))
	user = capkpi.db.exists("User", email)
	if not user:
		user = capkpi.get_doc(
			{
				"doctype": "User",
