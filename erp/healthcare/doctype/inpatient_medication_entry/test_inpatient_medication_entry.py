...
import capkpi
from capkpi.utils import add_days, getdate, now_datetime
...
from erp.healthcare.doctype.inpatient_medication_order.test_inpatient_medication_order import (
	create_ipme,
	create_ipmo,
)
...
class TestInpatientMedicationEntry(unittest.TestCase):
	def setUp(self):
		...
		capkpi.db.sql("""delete from `tabInpatient Medication Order`""")
		capkpi.db.sql("""delete from `tabInpatient Medication Entry`""")
	...
	def test_ipme_with_stock_update(self):
		...
		is_order_completed = capkpi.db.get_value(
			"Inpatient Medication Order Entry", ipme.medication_orders[0].against_imoe, "is_completed"
		)
		self.assertEqual(is_order_completed, 1)

		# test stock entry
		stock_entry = capkpi.db.exists("Stock Entry", {"inpatient_medication_entry": ipme.name})
		self.assertTrue(stock_entry)

		# check references
		stock_entry = capkpi.get_doc("Stock Entry", stock_entry)
	...
	def tearDown(self):
		# cleanup - Discharge
		schedule_discharge(
			capkpi.as_json({"patient": self.patient, "discharge_ordered_datetime": now_datetime()})
		)
		...
		for entry in capkpi.get_all("Inpatient Medication Entry"):
			doc = capkpi.get_doc("Inpatient Medication Entry", entry.name)
			doc.cancel()

		for entry in capkpi.get_all("Inpatient Medication Order"):
			doc = capkpi.get_doc("Inpatient Medication Order", entry.name)
			doc.cancel()
