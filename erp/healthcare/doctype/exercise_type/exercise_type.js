...
capkpi.ui.form.on('Exercise Type', {
	refresh: function(frm) {
		let wrapper = frm.fields_dict.steps_html.wrapper;

		frm.ExerciseEditor = new erp.ExerciseEditor(frm, wrapper);
	}
});

erp.ExerciseEditor = Class.extend({
	...
	show_add_card_dialog: function(frm) {
		...
		let d = new capkpi.ui.Dialog({
			...
			primary_action: function() {
				...
				let step = capkpi.model.add_child(frm.doc, 'Exercise Type Step', 'steps_table');
	...
	show_edit_card_dialog: function(frm, id) {
		let new_dialog = new capkpi.ui.Dialog({
