...
from erp.healthcare.page.patient_history.patient_history import get_patient_history_doctypes
...
class PatientHistorySettings(Document):
	...
	def validate_submittable_doctypes(self):
		for entry in self.custom_doctypes:
			if not cint(capkpi.db.get_value("DocType", entry.document_type, "is_submittable")):
				msg = _("Row #{0}: Document Type {1} is not submittable.").format(
					entry.idx, capkpi.bold(entry.document_type)
				)
	...
	def validate_date_fieldnames(self):
		for entry in self.custom_doctypes:
			field = capkpi.get_meta(entry.document_type).get_field(entry.date_fieldname)
			if not field:
				capkpi.throw(
					_("Row #{0}: No such Field named {1} found in the Document Type {2}.").format(
						entry.idx, capkpi.bold(entry.date_fieldname), capkpi.bold(entry.document_type)
					)
				)
	...
	def get_doctype_fields(self, document_type, fields):
		multicheck_fields = []
		doc_fields = capkpi.get_meta(document_type).fields

		for field in doc_fields:
			if (
				field.fieldtype not in capkpi.model.no_value_fields
				or field.fieldtype in capkpi.model.table_fields
	...
	def get_date_field_for_dt(self, document_type):
		meta = capkpi.get_meta(document_type)
...
def create_medical_record(doc, method=None):
	...
	if capkpi.db.exists("Patient Medical Record", {"reference_name": doc.name}):
		return

	subject = set_subject_field(doc)
	date_field = get_date_field(doc.doctype)
	medical_record = capkpi.new_doc("Patient Medical Record")
...
def update_medical_record(doc, method=None):
	...
	medical_record_id = capkpi.db.exists("Patient Medical Record", {"reference_name": doc.name})

	if medical_record_id:
		subject = set_subject_field(doc)
		capkpi.db.set_value("Patient Medical Record", medical_record_id[0][0], "subject", subject)
	else:
...
def delete_medical_record(doc, method=None):
	...
	record = capkpi.db.exists("Patient Medical Record", {"reference_name": doc.name})
	if record:
		capkpi.delete_doc("Patient Medical Record", record, force=1)
...
def set_subject_field(doc):
	...
	for entry in patient_history_fields:
		...
		if entry.get("fieldtype") == "Table" and doc.get(fieldname):
			...
			subject += capkpi.bold(_(entry.get("label")) + ":") + "<br>" + cstr(formatted_value) + "<br>"

		else:
			if doc.get(fieldname):
				formatted_value = format_value(doc.get(fieldname), meta.get_field(fieldname), doc)
				subject += capkpi.bold(_(entry.get("label")) + ":") + cstr(formatted_value) + "<br>"
...
def get_date_field(doctype):
	...
	return capkpi.db.get_value(dt, {"document_type": doctype}, "date_fieldname")


def get_patient_history_fields(doc):
	dt = get_patient_history_config_dt(doc.doctype)
	patient_history_fields = capkpi.db.get_value(
...
def get_patient_history_config_dt(doctype):
	if capkpi.db.get_value("DocType", doctype, "custom"):
