...
capkpi.ui.form.on('Clinical Procedure', {
	setup: function(frm) {
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			...
			if (!item.item_code) {
				...
				if (frm.doc.status == 'In Progress') {
					filters['posting_date'] = frm.doc.start_date || capkpi.datetime.nowdate();
										...
										function() {
											capkpi.call({
												...
												callback: function(r) {
													if (!r.exc) {
														frm.reload_doc();
														let doclist = capkpi.model.sync(r.message);
	...
	practitioner: function(frm) {
		if (frm.doc.practitioner) {
			capkpi.call({
				...
				callback: function (data) {
					capkpi.model.set_value(frm.doctype,frm.docname, 'practitioner_name', data.message.practitioner_name);
				}
			});
		} else {
			capkpi.model.set_value(frm.doctype,frm.docname, 'practitioner_name', '');
		}
	},
	...
	set_procedure_consumables: function(frm) {
		capkpi.call({
			method: 'erp.healthcare.doctype.clinical_procedure.clinical_procedure.get_procedure_consumables',
			args: {
...
capkpi.ui.form.on('Clinical Procedure Item', {
	qty: function(frm, cdt, cdn) {
		let d = locals[cdt][cdn];
		capkpi.model.set_value(cdt, cdn, 'transfer_qty', d.qty*d.conversion_factor);
	},
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (d.item_code) {
			...
			return capkpi.call({
				method: 'erp.healthcare.doctype.clinical_procedure_template.clinical_procedure_template.get_item_details',
				args: {args: args},
				callback: function(r) {
					if (r.message) {
						let d = locals[cdt][cdn];
