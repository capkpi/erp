...
from capkpi.utils import flt, nowdate, nowtime

from erp.healthcare.doctype.healthcare_settings.healthcare_settings import get_account
from erp.healthcare.doctype.lab_test.lab_test import create_sample_doc
...
class ClinicalProcedure(Document):
	...
	def after_insert(self):
		if self.prescription:
			capkpi.db.set_value("Procedure Prescription", self.prescription, "procedure_created", 1)
		if self.appointment:
			capkpi.db.set_value("Patient Appointment", self.appointment, "status", "Closed")
		template = capkpi.get_doc("Clinical Procedure Template", self.procedure_template)
		if template.sample:
			patient = capkpi.get_doc("Patient", self.patient)
			sample_collection = create_sample_doc(template, patient, None, self.company)
			capkpi.db.set_value("Clinical Procedure", self.name, "sample", sample_collection.name)
		self.reload()
	...
	def complete_procedure(self):
		...
		if self.items:
			...
			if customer:
				...
				capkpi.throw(
					_("Please set Customer in Patient {0}").format(capkpi.bold(self.patient)),
...
def make_procedure(source_name, target_doc=None):
	def set_missing_values(source, target):
		consume_stock = capkpi.db.get_value(
		...
		if consume_stock:
			...
			if source.service_unit:
				warehouse = capkpi.db.get_value("Healthcare Service Unit", source.service_unit, "warehouse")
			if not warehouse:
				warehouse = capkpi.db.get_value("Stock Settings", None, "default_warehouse")
			if warehouse:
