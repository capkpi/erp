...
class TestClinicalProcedure(unittest.TestCase):
	def test_procedure_template_item(self):
		...
		self.assertTrue(capkpi.db.exists("Item", procedure_template.item))

		procedure_template.disabled = 1
		procedure_template.save()
		self.assertEqual(capkpi.db.get_value("Item", procedure_template.item, "disabled"), 1)
	...
	def test_consumables(self):
		...
		# check consumption
		self.assertTrue(capkpi.db.exists("Stock Entry", result))


def create_consumable():
	if capkpi.db.exists("Item", "Syringe"):
		return capkpi.get_doc("Item", "Syringe")
	consumable = capkpi.new_doc("Item")
