...
class TestInpatientMedicationOrder(unittest.TestCase):
	...
	def test_order_creation(self):
		...
		prescription_dosage = capkpi.get_doc("Prescription Dosage", "1-1-1")
	...
	def test_inpatient_validation(self):
		...
		self.assertRaises(capkpi.ValidationError, ipmo.insert)
...
def create_dosage_form():
	if not capkpi.db.exists("Dosage Form", "Tablet"):
		capkpi.get_doc({"doctype": "Dosage Form", "dosage_form": "Tablet"}).insert()


def create_drug(item=None):
	if not item:
		item = "Dextromethorphan"
	drug = capkpi.db.exists("Item", {"item_code": "Dextromethorphan"})
	if not drug:
		drug = capkpi.get_doc(
			{
				"doctype": "Item",
...
def create_ipmo(patient):
	orders = get_orders()
	ipmo = capkpi.new_doc("Inpatient Medication Order")
...
def create_ipme(filters, update_stock=0):
	ipme = capkpi.new_doc("Inpatient Medication Entry")
