...
import capkpi
from capkpi.utils import getdate, nowtime
...
from erp.healthcare.doctype.patient_medical_record.test_patient_medical_record import (
...
class TestLabTest(unittest.TestCase):
	def test_lab_test_item(self):
		...
		self.assertTrue(capkpi.db.exists("Item", lab_template.item))
		self.assertEqual(
			capkpi.db.get_value("Item Price", {"item_code": lab_template.item}, "price_list_rate"),
			lab_template.lab_test_rate,
		)

		lab_template.disabled = 1
		lab_template.save()
		self.assertEqual(capkpi.db.get_value("Item", lab_template.item, "disabled"), 1)
	...
	def test_descriptive_lab_test(self):
		...
		lab_test.save()
		self.assertRaises(capkpi.ValidationError, lab_test.submit)
	...
	def test_sample_collection(self):
		...
		self.assertTrue(capkpi.db.exists("Sample Collection", {"sample": lab_template.sample}))
...
def create_lab_test_template(test_sensitivity=0, sample_collection=1):
	...
	if capkpi.db.exists("Lab Test Template", "Insulin Resistance"):
		return capkpi.get_doc("Lab Test Template", "Insulin Resistance")
	template = capkpi.new_doc("Lab Test Template")
...
def create_medical_department():
	medical_department = capkpi.db.exists("Medical Department", "_Test Medical Department")
...
def create_lab_test_sample():
	blood_sample = capkpi.db.exists("Lab Test Sample", "Blood Sample")
	if blood_sample:
		return blood_sample

	sample = capkpi.new_doc("Lab Test Sample")
...
def create_patient_encounter():
	...
	patient_encounter = capkpi.new_doc("Patient Encounter")
...
def create_practitioner():
	practitioner = capkpi.db.exists("Healthcare Practitioner", "_Test Healthcare Practitioner")
