...
capkpi.ui.form.on('Patient', {
	refresh: function (frm) {
		...
		if (capkpi.defaults.get_default('patient_name_by') != 'Naming Series') {
			frm.toggle_display('naming_series', false);
		} else {
			erp.toggle_naming_series();
		}

		if (capkpi.defaults.get_default('collect_registration_fee') && frm.doc.status == 'Disabled') {
			frm.add_custom_button(__('Invoice Patient Registration'), function () {
				invoice_registration(frm);
			});
		}

		if (frm.doc.patient_name && capkpi.user.has_role('Physician')) {
			frm.add_custom_button(__('Patient Progress'), function() {
				capkpi.route_options = {'patient': frm.doc.name};
				capkpi.set_route('patient-progress');
			}, __('View'));
		...
		capkpi.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Patient'};
		frm.toggle_display(['address_html', 'contact_html'], !frm.is_new());

		if (!frm.is_new()) {
			if ((capkpi.user.has_role('Nursing User') || capkpi.user.has_role('Physician'))) {
...
capkpi.ui.form.on('Patient', 'dob', function(frm) {
	if (frm.doc.dob) {
		let today = new Date();
		let birthDate = new Date(frm.doc.dob);
		if (today < birthDate) {
			capkpi.msgprint(__('Please select a valid Date'));
			capkpi.model.set_value(frm.doctype,frm.docname, 'dob', '');
		} else {
...
capkpi.ui.form.on('Patient Relation', {
...
let create_vital_signs = function (frm) {
	if (!frm.doc.name) {
		capkpi.throw(__('Please save the patient first'));
	}
	capkpi.route_options = {
		'patient': frm.doc.name,
	};
...
let create_encounter = function (frm) {
	...
	capkpi.new_doc('Patient Encounter');
};
		...
		callback: function(data) {
			if (!data.exc) {
				if (data.message.invoice) {
					capkpi.set_route('Form', 'Sales Invoice', data.message.invoice);
				}
				cur_frm.reload_doc();
			}
		}
