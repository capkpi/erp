...
from capkpi.utils import cint, cstr, getdate
from capkpi.utils.nestedset import get_root_of

from erp import get_default_currency
from erp.accounts.party import get_dashboard_info
...
class Patient(Document):
	...
	def after_insert(self):
		if capkpi.db.get_single_value("Healthcare Settings", "collect_registration_fee"):
			capkpi.db.set_value("Patient", self.name, "status", "Disabled")
		else:
			send_registration_sms(self)
		self.reload()

	def on_update(self):
		if capkpi.db.get_single_value("Healthcare Settings", "link_customer_to_patient"):
			if self.customer:
				customer = capkpi.get_doc("Customer", self.customer)
	...
	def set_missing_customer_details(self):
		if not self.customer_group:
			self.customer_group = capkpi.db.get_single_value(
				"Selling Settings", "customer_group"
			) or get_root_of("Customer Group")
		if not self.territory:
			self.territory = capkpi.db.get_single_value("Selling Settings", "territory") or get_root_of(
				"Territory"
			)
		if not self.default_price_list:
			self.default_price_list = capkpi.db.get_single_value("Selling Settings", "selling_price_list")
		...
		if not self.language:
			self.language = capkpi.db.get_single_value("System Settings", "language")

	def create_website_user(self):
		users = capkpi.db.get_all(
			"User",
		...
		if users and users[0]:
			capkpi.throw(
				...
				).format(capkpi.bold(users[0].email), capkpi.bold(users[0].mobile_no)),
	...
	def autoname(self):
		patient_name_by = capkpi.db.get_single_value("Healthcare Settings", "patient_name_by")
	...
	def get_patient_name(self):
		...
		if capkpi.db.get_value("Patient", name):
			count = capkpi.db.sql(
	...
	def invoice_patient_registration(self):
		if capkpi.db.get_single_value("Healthcare Settings", "registration_fee"):
			...
			capkpi.db.set_value("Patient", self.name, "status", "Active")
			send_registration_sms(self)
...
def create_customer(doc):
	customer = capkpi.get_doc(
		{
			...
			or capkpi.db.get_single_value("Selling Settings", "customer_group"),
			"territory": doc.territory or capkpi.db.get_single_value("Selling Settings", "territory"),
	...
	capkpi.db.set_value("Patient", doc.name, "customer", customer.name)
	capkpi.msgprint(_("Customer {0} is created.").format(customer.name), alert=True)


def make_invoice(patient, company):
	uom = capkpi.db.exists("UOM", "Nos") or capkpi.db.get_single_value("Stock Settings", "stock_uom")
	sales_invoice = capkpi.new_doc("Sales Invoice")
	sales_invoice.customer = capkpi.db.get_value("Patient", patient, "customer")
	...
	item_line.rate = capkpi.db.get_single_value("Healthcare Settings", "registration_fee")
...
def get_patient_detail(patient):
	patient_dict = capkpi.db.sql("""select * from tabPatient where name=%s""", (patient), as_dict=1)
	if not patient_dict:
		capkpi.throw(_("Patient not found"))
	vital_sign = capkpi.db.sql(
...
def get_timeline_data(doctype, name):
	...
	customer = capkpi.db.get_value(doctype, name, "customer")
	if customer:
		from erp.accounts.party import get_timeline_data
