capkpi.provide('capkpi.patient_history');
capkpi.pages['patient_history'].on_page_load = function(wrapper) {
...
class PatientHistory {
	...
	show() {
		...
		if (capkpi.route_options && !this.patient_id) {
	...
	make_patient_profile() {
		...
		this.main_section.empty().append(capkpi.render_template('patient_history'));
		this.setup_filters();
	...
	setup_filters() {
		...
		capkpi.xcall(
			'erp.healthcare.page.patient_history.patient_history.get_patient_history_doctypes'
		).then(document_types => {
			let doctype_filter = capkpi.ui.form.make_control({
	...
	setup_documents(document_types="", selected_date_range="") {
		...
		let me = this;
		capkpi.call({
			'method': 'erp.healthcare.page.patient_history.patient_history.get_feed',
			args: filters,
			callback: function(r) {
				let data = r.message;
				if (data.length) {
	...
	add_to_records(data) {
		...
		for (i=0; i<data.length; i++) {
			if (data[i].reference_doctype) {
				...
				if (capkpi.user_info(data[i].owner).image) {
					data[i].imgsrc = capkpi.utils.get_file_link(capkpi.user_info(data[i].owner).image);
				} else {
					data[i].imgsrc = false;
				}
				...
				time_line_heading += data[i].reference_doctype + " - " +
					`<a onclick="capkpi.set_route('Form', '${data[i].reference_doctype}', '${data[i].reference_name}');">
				...
				details += `<div class='d-flex flex-column width-full'>
						...
						<div class='capkpi-card p-5 mt-3'>
	...
	add_date_separator(data) {
		let date = capkpi.datetime.str_to_obj(data.communication_date);
		let pdate = '';
		let diff = capkpi.datetime.get_day_diff(capkpi.datetime.get_today(),
			capkpi.datetime.obj_to_str(date));
		...
		if (diff < 1) {
			...
			pdate = __('on {0}', [capkpi.datetime.global_date_format(date)]);
		}
		data.date_sep = pdate;
		return data;
	}
	...
	show_patient_info() {
		this.get_patient_info().then(() => {
			$('.patient-info').empty().append(capkpi.render_template('patient_history_sidebar', {
		...
		this.page.main.on('click', '.btn-more', function() {
			...
			if (me.page.main.find('.'+docname).parent().find('.document-html').attr('data-fetched') == '1') {
				...
				if (doctype && docname) {
					...
					capkpi.call({
						method: 'erp.healthcare.utils.render_doc_as_html',
						args: {
							doctype: doctype,
							docname: docname,
	...
	show_patient_vital_charts(btn_id, scale_unit, title) {
		...
		capkpi.call({
			method: 'erp.healthcare.utils.get_patient_vitals',
			args: {
				patient: me.patient_id
			},
			callback: function(r) {
				if (r.message) {
					...
					new capkpi.Chart('.patient_vital_charts', {
						data: {
							labels: labels,
							datasets: datasets
						},
