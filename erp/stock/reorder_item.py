...
from capkpi.utils import add_days, cint, flt, nowdate
...
def reorder_item():
	...
	if not (capkpi.db.a_row_exists("Company") and capkpi.db.a_row_exists("Fiscal Year")):
		return

	if cint(capkpi.db.get_value("Stock Settings", None, "auto_indent")):
		return _reorder_item()


def _reorder_item():
	material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
	warehouse_company = capkpi._dict(
		capkpi.db.sql(
	...
	default_company = (
		erp.get_default_company() or capkpi.db.sql("""select name from tabCompany limit 1""")[0][0]
	)

	items_to_consider = capkpi.db.sql_list(
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in capkpi.db.sql(
		...
		warehouse_doc = capkpi.get_doc("Warehouse", warehouse)
		...
		while warehouse_doc.parent_warehouse:
			...
			warehouse_doc = capkpi.get_doc("Warehouse", warehouse_doc.parent_warehouse)
...
def create_material_request(material_requests):
	...
	def _log_exception():
		if capkpi.local.message_log:
			exceptions_list.extend(capkpi.local.message_log)
			capkpi.local.message_log = []
		else:
			exceptions_list.append(capkpi.get_traceback())
	...
	for request_type in material_requests:
		for company in material_requests[request_type]:
			try:
				...
				for d in items:
					d = capkpi._dict(d)
					item = capkpi.get_doc("Item", d.item_code)
					uom = item.stock_uom
					...
					must_be_whole_number = capkpi.db.get_value("UOM", uom, "must_be_whole_number", cache=True)
	...
	if mr_list:
		if getattr(capkpi.local, "reorder_email_notify", None) is None:
			capkpi.local.reorder_email_notify = cint(
				capkpi.db.get_value("Stock Settings", None, "reorder_email_notify")
			)

		if capkpi.local.reorder_email_notify:
...
def send_email_notification(mr_list):
	...
	msg = capkpi.render_template("templates/emails/reorder_item.html", {"mr_list": mr_list})

	capkpi.sendmail(recipients=email_list, subject=_("Auto Material Requests Generated"), message=msg)
