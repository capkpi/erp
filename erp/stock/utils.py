...
from capkpi.utils import cstr, flt, get_link_to_form, nowdate, nowtime
from six import string_types

import erp


class InvalidWarehouseCompany(capkpi.ValidationError):
	pass


class PendingRepostingError(capkpi.ValidationError):
	pass
...
def get_stock_value_from_bin(warehouse=None, item_code=None):
	...
	stock_value = capkpi.db.sql(query, values)
...
def get_stock_value_on(warehouse=None, posting_date=None, item_code=None):
	...
	if warehouse:
		...
		lft, rgt, is_group = capkpi.db.get_value("Warehouse", warehouse, ["lft", "rgt", "is_group"])
...
def get_stock_balance(
	...
	from erp.stock.stock_ledger import get_previous_sle
...
def get_bin(item_code, warehouse):
	bin = capkpi.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
	if not bin:
		bin_obj = _create_bin(item_code, warehouse)
	else:
		bin_obj = capkpi.get_doc("Bin", bin, for_update=True)
	bin_obj.flags.ignore_permissions = True
	return bin_obj


def get_or_make_bin(item_code: str, warehouse: str) -> str:
	bin_record = capkpi.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def _create_bin(item_code, warehouse):
	...
	try:
		capkpi.db.savepoint(bin_creation_savepoint)
		bin_obj = capkpi.get_doc(doctype="Bin", item_code=item_code, warehouse=warehouse)
		bin_obj.flags.ignore_permissions = 1
		bin_obj.insert()
	except capkpi.UniqueValidationError:
		capkpi.db.rollback(save_point=bin_creation_savepoint)  # preserve transaction in postgres
		bin_obj = capkpi.get_last_doc("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def update_bin(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	from erp.stock.doctype.bin.bin import update_stock
	...
	if is_stock_item:
		...
		capkpi.msgprint(_("Item {0} ignored since it is not a stock item").format(args.get("item_code")))


@capkpi.whitelist()
def get_incoming_rate(args, raise_error_if_no_rate=True):
	"""Get Incoming Rate based on valuation method"""
	from erp.stock.stock_ledger import get_previous_sle, get_valuation_rate
	...
	if not in_rate:
		...
		in_rate = get_valuation_rate(
			...
			currency=erp.get_company_currency(args.get("company")),
...
def get_valuation_method(item_code):
	...
	val_method = capkpi.db.get_value("Item", item_code, "valuation_method", cache=True)
	if not val_method:
		val_method = capkpi.db.get_value("Stock Settings", None, "valuation_method") or "FIFO"
	return val_method
	...
	for val in serial_nos:
		if val:
			...
			if val in valid_serial_nos:
				capkpi.throw(_("Serial number {0} entered more than once").format(val))
			else:
				valid_serial_nos.append(val)

	if qty and len(valid_serial_nos) != abs(qty):
		capkpi.throw(_("{0} valid serial nos for Item {1}").format(abs(qty), item_code))

	return valid_serial_nos


def validate_warehouse_company(warehouse, company):
	warehouse_company = capkpi.db.get_value("Warehouse", warehouse, "company", cache=True)
...
def is_group_warehouse(warehouse):
	if capkpi.db.get_value("Warehouse", warehouse, "is_group", cache=True):
		capkpi.throw(_("Group node warehouse is not allowed to select for transactions"))


def validate_disabled_warehouse(warehouse):
	if capkpi.db.get_value("Warehouse", warehouse, "disabled", cache=True):
		capkpi.throw(
		...
		if d.get("convertible"):
			...
			columns.insert(
				idx + 1,
				{
					...
					"fieldname": "{0}_{1}".format(d.get("fieldname"), capkpi.scrub(include_uom)),
	...
	for row_idx, row in enumerate(result):
		...
		for key, value in data:
			...
			if not is_dict_obj:
				...
				new_key = "{0}_{1}".format(key, capkpi.scrub(include_uom))
...
def add_additional_uom_columns(columns, result, include_uom, conversion_factors):
	...
	for col_idx in list(reversed(range(0, len(columns)))):
		...
		if isinstance(col, dict) and col.get("convertible") in ["rate", "qty"]:
			...
			convertible_column_map[col.get("fieldname")] = capkpi._dict(
...
def is_reposting_item_valuation_in_progress():
	reposting_in_progress = capkpi.db.exists(
...
def check_pending_reposting(posting_date: str, throw_error: bool = True) -> bool:
	...
	reposting_pending = capkpi.db.exists("Repost Item Valuation", filters)
	if reposting_pending and throw_error:
		...
		capkpi.msgprint(
			...
			primary_action={
				...
				"client_action": "erp.route_to_pending_reposts",
				"args": filters,
			},
		)
