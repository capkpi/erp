...
def get_warehouse_account_map(company=None):
	company_warehouse_account_map = company and capkpi.flags.setdefault(
		"warehouse_account_map", {}
	).get(company)
	warehouse_account_map = capkpi.flags.warehouse_account_map

	if not warehouse_account_map or not company_warehouse_account_map or capkpi.flags.in_test:
		warehouse_account = capkpi._dict()

		filters = {}
		if company:
			filters["company"] = company
			capkpi.flags.setdefault("warehouse_account_map", {}).setdefault(company, {})
		...
		for d in capkpi.get_all(
			...
			if d.account:
				d.account_currency = capkpi.db.get_value("Account", d.account, "account_currency", cache=True)
				warehouse_account.setdefault(d.name, d)
		if company:
			capkpi.flags.warehouse_account_map[company] = warehouse_account
		else:
			capkpi.flags.warehouse_account_map = warehouse_account

	return capkpi.flags.warehouse_account_map.get(company) or capkpi.flags.warehouse_account_map
...
def get_company_default_inventory_account(company):
	return capkpi.get_cached_value("Company", company, "default_inventory_account")
