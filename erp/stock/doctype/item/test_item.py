...
from erp.stock.doctype.item.item import (
	DataValidationError,
	InvalidBarcode,
...
def make_item(item_code=None, properties=None):
	if not item_code:
		item_code = capkpi.generate_hash(length=16)

	if capkpi.db.exists("Item", item_code):
		return capkpi.get_doc("Item", item_code)
...
class TestItem(CapKPITestCase):
	def setUp(self):
		super().setUp()
		capkpi.flags.attribute_values = None

	def get_item(self, idx):
		item_code = test_records[idx].get("item_code")
		if not capkpi.db.exists("Item", item_code):
			item = capkpi.copy_doc(test_records[idx])
			item.insert()
		else:
	...
	def test_get_item_details(self):
		...
		currency = capkpi.get_cached_value("Company", company, "default_currency")
	...
	def test_item_defaults(self):
		capkpi.delete_doc_if_exists("Item", "Test Item With Defaults", force=1)
		make_item(
	...
	def test_item_default_validations(self):
		...
		with self.assertRaises(capkpi.ValidationError) as ve:
			make_item(
				"Bad Item defaults",
				{
					"item_group": "_Test Item Group",
	...
	def test_item_attribute_change_after_variant(self):
		capkpi.delete_doc_if_exists("Item", "_Test Variant Item-L", force=1)

		variant = create_variant("_Test Variant Item", {"Test Size": "Large"})
		variant.save()

		attribute = capkpi.get_doc("Item Attribute", "Test Size")
	...
	def test_copy_fields_from_template_to_variants(self):
		capkpi.delete_doc_if_exists("Item", "_Test Variant Item-XL", force=1)
		...
		if not capkpi.db.get_value(
			...
			item_attribute = capkpi.get_doc("Item Attribute", "Test Size")
		...
		variant = capkpi.get_doc("Item", "_Test Variant Item-XL")
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in capkpi.db.get_all("Item", filters={"variant_of": "_Test Numeric Template Item"}):
			capkpi.delete_doc_if_exists("Item", d.name)

		capkpi.delete_doc_if_exists("Item", "_Test Numeric Template Item")
		capkpi.delete_doc_if_exists("Item Attribute", "Test Item Length")
	...
	def test_item_merging(self):
		old = create_item(capkpi.generate_hash(length=20)).name
		new = create_item(capkpi.generate_hash(length=20)).name
		...
		capkpi.rename_doc("Item", old, new, merge=True)

		self.assertFalse(capkpi.db.exists("Item", old))

		self.assertTrue(
			capkpi.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse - _TC"})
		)
		self.assertTrue(
			capkpi.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse 1 - _TC"})
		)
	...
	def test_item_merging_with_product_bundle(self):
		...
		with self.assertRaises(DataValidationError):
			capkpi.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		bundle1.delete()
		capkpi.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		self.assertFalse(capkpi.db.exists("Item", "Test Item Bundle Item 1"))

	def test_uom_conversion_factor(self):
		if capkpi.db.exists("Item", "Test Item UOM"):
			capkpi.delete_doc("Item", "Test Item UOM")
	...
	def test_item_variant_by_manufacturer(self):
		...
		if capkpi.db.exists("Item", "_Test Variant Mfg"):
			capkpi.delete_doc("Item", "_Test Variant Mfg")
		if capkpi.db.exists("Item", "_Test Variant Mfg-1"):
			capkpi.delete_doc("Item", "_Test Variant Mfg-1")
		if capkpi.db.exists("Manufacturer", "MSG1"):
			capkpi.delete_doc("Manufacturer", "MSG1")
		...
		manufacturer = capkpi.get_doc(dict(doctype="Manufacturer", short_name="MSG1")).insert()
	...
	def test_stock_exists_against_template_item(self):
		stock_item = capkpi.get_all("Stock Ledger Entry", fields=["item_code"], limit=1)
		if stock_item:
			item_code = stock_item[0].item_code

			item_doc = capkpi.get_doc("Item", item_code)
			item_doc.has_variants = 1
			self.assertRaises(StockExistsForTemplate, item_doc.save)

	def test_add_item_barcode(self):
		# Clean up
		capkpi.db.sql("""delete from `tabItem Barcode`""")
		item_code = "Test Item Barcode"
		if capkpi.db.exists("Item", item_code):
			capkpi.delete_doc("Item", item_code)
		...
		self.assertRaises(capkpi.UniqueValidationError, item_doc.save)
	...
	def test_check_stock_uom_with_bin(self):
		...
		item.stock_uom = "Gram"
		self.assertRaises(capkpi.ValidationError, item.save)

	def test_check_stock_uom_with_bin_no_sle(self):
		from erp.stock.stock_balance import update_bin_qty
		...
		try:
			item.save()
		except capkpi.ValidationError as e:
	...
	def test_erasure_of_old_conversions(self):
		...
		item.stock_uom = "Gram"
		item.append("uoms", capkpi._dict(uom="Box", conversion_factor=2))
		item.save()
		item.reload()
		item.stock_uom = "Nos"
		item.save()
	...
	def test_validate_stock_item(self):
		self.assertRaises(capkpi.ValidationError, validate_is_stock_item, "_Test Non Stock Item")
...
def set_item_variant_settings(fields):
	doc = capkpi.get_doc("Item Variant Settings")
	doc.set("fields", fields)
	doc.save()


def make_item_variant():
	if not capkpi.db.exists("Item", "_Test Variant Item-S"):
...
test_records = capkpi.get_test_records("Item")
