...
capkpi.provide("erp.item");

capkpi.ui.form.on("Item", {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.is_stock_item) {
			...
			frm.add_custom_button(__("Stock Projected Qty"), function() {
				...
				capkpi.set_route("query-report", "Stock Projected Qty");
			}, __("View"));
		}
		...
		if (capkpi.defaults.get_default("item_naming_by")!="Naming Series" || frm.doc.variant_of) {
		...
		if (!frm.doc.published_in_website) {
			frm.add_custom_button(__("Publish in Website"), function() {
				capkpi.call({
					method: "erp.e_commerce.doctype.website_item.website_item.make_website_item",
					args: {doc: frm.doc},
					freeze: true,
			...
			frm.add_custom_button(__("Website Item"), function() {
				capkpi.db.get_value("Website Item", {item_code: frm.doc.name}, "name", (d) => {
					if (!d.name) capkpi.throw(__("Website Item not found"));
					capkpi.set_route("Form", "Website Item", d.name);
				});
			}, __("View"));
		}

		erp.item.edit_prices_button(frm);
		erp.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			erp.item.make_dashboard(frm);
		}

		frm.add_custom_button(__('Duplicate'), function() {
			var new_item = capkpi.model.copy_doc(frm.doc);
			...
			capkpi.set_route('Form', 'Item', new_item.name);
		});
	...
	validate: function(frm){
		erp.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function(frm) {
		...
		capkpi.call({
			method: "erp.stock.doctype.item.item.get_asset_naming_series",
			callback: function(r) {
	...
	page_name: capkpi.utils.warn_page_name_change,
...
capkpi.ui.form.on('Item Reorder', {
...
capkpi.ui.form.on('Item Customer Detail', {
	customer_items_add: function(frm, cdt, cdn) {
		capkpi.model.set_value(cdt, cdn, 'customer_group', "");
	},
	...
	customer_group: function(frm, cdt, cdn) {
		if(set_customer_group(frm, cdt, cdn)){
			capkpi.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	}
});
...
var set_customer_group = function(frm, cdt, cdn) {
	...
	capkpi.model.with_doc("Customer", row.customer_name, function() {
		var customer = capkpi.model.get_doc("Customer", row.customer_name);
...
$.extend(erp.item, {
	setup_queries: function(frm) {
		...
		frm.fields_dict["item_defaults"].grid.get_field("income_account").get_query = function(doc, cdt, cdn) {
			const row = locals[cdt][cdn];
			return {
				query: "erp.controllers.queries.get_income_account",
				filters: { company: row.company }
			}
		}
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			capkpi.require('assets/js/item-dashboard.min.js', function() {
				const section = frm.dashboard.add_section('', __("Stock Levels"));
				erp.item.item_dashboard = new erp.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: 'erp.stock.dashboard.item_dashboard.get_data',
					template: 'item_dashboard_list'
				});
				erp.item.item_dashboard.refresh();
			});
		}
	},
	...
	edit_prices_button: function(frm) {
		frm.add_custom_button(__("Add / Edit Prices"), function() {
			capkpi.set_route("List", "Item Price", {"item_code": frm.doc.name});
		}, __("Actions"));
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Create'), function() {
			...
			capkpi.call({
				method: "erp.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new capkpi.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__('Create Variants'), () => {
				...
				capkpi.call({
					method: "erp.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find('.form-column'))
				.find('.capkpi-control')).css('margin-bottom', '0px');
			...
			capkpi.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						capkpi.msgprint_dialog = capkpi.msgprint(__("Item Variant {0} already exists with same attributes",
							[repl('<a href="/app/item/%(item_encoded)s" class="strong variant-click">%(item)s</a>', {
								item_encoded: encodeURIComponent(variant),
								item: variant
							})]
						));
						capkpi.msgprint_dialog.hide_on_page_refresh = true;
						capkpi.msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							d.hide();
						});
					} else {
						d.hide();
						capkpi.call({
							method: "erp.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					capkpi.call({
						method: "erp.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
capkpi.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			capkpi.call({
				method: "erp.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
				...
				callback: function(r) {
					if (!r.exc && r.message) {
						capkpi.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				}
			});
		}
