...
from erp.stock.get_item_details import get_item_details, get_price_list_rate
...
def make_packing_list(doc):
	...
	set_price_from_children = capkpi.db.get_single_value(
...
def is_product_bundle(item_code: str) -> bool:
	return bool(capkpi.db.exists("Product Bundle", {"new_item_code": item_code}))
...
def get_product_bundle_items(item_code):
	product_bundle = capkpi.qb.DocType("Product Bundle")
	product_bundle_item = capkpi.qb.DocType("Product Bundle Item")

	query = (
		capkpi.qb.from_(product_bundle_item)
		.join(product_bundle)
...
def update_packed_item_price_data(pi_row, item_data, doc):
	...
	item_doc = capkpi.get_cached_doc("Item", pi_row.item_code)
...
def on_doctype_update():
	capkpi.db.add_index("Packed Item", ["item_code", "warehouse"])
