...
from erp.stock.doctype.batch.batch import UnableToSelectBatchError, get_batch_no, get_batch_qty
...
class TestBatch(CapKPITestCase):
	def test_item_has_batch_enabled(self):
		self.assertRaises(
			ValidationError,
			capkpi.get_doc({"doctype": "Batch", "name": "_test Batch", "item": "_Test Item"}).save,
		)
	...
	def make_batch_item(cls, item_name=None):
		...
		if not capkpi.db.exists("Item", item_name):
			return make_item(item_name, dict(has_batch_no=1, create_new_batch=1, is_stock_item=1))
		else:
			return capkpi.get_doc("Item", item_name)

	def test_purchase_receipt(self, batch_qty=100):
		"""Test automated batch creation from Purchase Receipt"""
		self.make_batch_item("ITEM-BATCH-1")

		receipt = capkpi.get_doc(
			dict(
	...
	def test_batch_split(self):
		...
		from erp.stock.doctype.batch.batch import split_batch
	...
	def test_total_batch_qty(self):
		...
		existing_batch_qty = flt(capkpi.db.get_value("Batch", "B100", "batch_qty"))
		stock_entry = self.make_new_batch_and_entry("ITEM-BATCH-3", "B100", "_Test Warehouse - _TC")

		current_batch_qty = flt(capkpi.db.get_value("Batch", "B100", "batch_qty"))
	...
	def make_new_batch_and_entry(cls, item_name, batch_name, warehouse):
		...
		if not capkpi.db.exists("Batch", batch_name):
			batch = capkpi.get_doc(dict(doctype="Batch", item=item_name, batch_id=batch_name)).insert(
	...
	def test_batch_name_with_naming_series(self):
		stock_settings = capkpi.get_single("Stock Settings")
		use_naming_series = cint(stock_settings.use_naming_series)

		if not use_naming_series:
			capkpi.set_value("Stock Settings", "Stock Settings", "use_naming_series", 1)
		...
		if not use_naming_series:
			capkpi.set_value("Stock Settings", "Stock Settings", "use_naming_series", 0)

	def make_new_batch(self, item_name=None, batch_id=None, do_not_insert=0):
		batch = capkpi.new_doc("Batch")
	...
	def test_batch_wise_item_price(self):
		if not capkpi.db.get_value("Item", "_Test Batch Price Item"):
			capkpi.get_doc(
				{
					"doctype": "Item",
					"is_stock_item": 1,
...
def create_batch(item_code, rate, create_item_price_for_batch):
	...
	batch = capkpi.db.get_value("Batch", {"item": item_code, "reference_name": pi.name})
...
def make_new_batch(**args):
	...
	try:
		...
		batch = capkpi.get_doc("Batch", args.batch_id)
