...
from capkpi.model.naming import make_autoname, revert_series_if_last
from capkpi.utils import cint, flt, get_link_to_form
from capkpi.utils.data import add_days
from capkpi.utils.jinja import render_template
from six import text_type


class UnableToSelectBatchError(capkpi.ValidationError):
	pass
...
def get_name_from_hash():
	...
	temp = None
	while not temp:
		temp = capkpi.generate_hash()[:7].upper()
		if capkpi.db.exists("Batch", temp):
			temp = None
...
def batch_uses_naming_series():
	...
	:return: bool
	"""
	use_naming_series = cint(capkpi.db.get_single_value("Stock Settings", "use_naming_series"))
...
def _get_batch_prefix():
	...
	"""
	naming_series_prefix = capkpi.db.get_single_value("Stock Settings", "naming_series_prefix")
...
class Batch(Document):
	def autoname(self):
		...
		create_new_batch, batch_number_series = capkpi.db.get_value(
			"Item", self.item, ["create_new_batch", "batch_number_series"]
		)

		if not create_new_batch:
			capkpi.throw(_("Batch ID is mandatory"), capkpi.MandatoryError)
		...
		while not self.batch_id:
			...
			if capkpi.db.exists("Batch", self.batch_id):
				self.batch_id = None

		self.name = self.batch_id

	def onload(self):
		self.image = capkpi.db.get_value("Item", self.item, "image")
	...
	def item_has_batch_enabled(self):
		if capkpi.db.get_value("Item", self.item, "has_batch_no") == 0:
			capkpi.throw(_("The selected item cannot have Batch"))

	def before_save(self):
		has_expiry_date, shelf_life_in_days = capkpi.db.get_value(
		...
		if has_expiry_date and not self.expiry_date:
			capkpi.throw(
				msg=_("Please set {0} for Batched Item {1}, which is used to set {2} on Submit.").format(
					capkpi.bold("Shelf Life in Days"),
					get_link_to_form("Item", self.item),
					capkpi.bold("Batch Expiry Date"),
				),
...
def get_batches_by_oldest(item_code, warehouse):
	...
	batches_dates = [
		[batch, capkpi.get_value("Batch", batch.batch_no, "expiry_date")] for batch in batches
	]
...
def split_batch(batch_no, item_code, warehouse, qty, new_batch_id=None):
	...
	batch = capkpi.get_doc(dict(doctype="Batch", item=item_code, batch_id=new_batch_id)).insert()

	company = capkpi.db.get_value(
		"Stock Ledger Entry",
...
def set_batch_nos(doc, warehouse_field, throw=False, child_table="items"):
	...
	for d in doc.get(child_table):
		...
		if warehouse and qty > 0 and capkpi.db.get_value("Item", d.item_code, "has_batch_no"):
			if not d.batch_no:
...
def get_batches(item_code, warehouse, qty=1, throw=False, serial_no=None):
	...
	cond = ""
	if serial_no and capkpi.get_cached_value("Item", item_code, "has_batch_no"):
		serial_nos = get_serial_nos(serial_no)
		batch = capkpi.get_all(
			"Serial No",
		...
		cond = " and `tabBatch`.name = %s" % (capkpi.db.escape(batch[0].batch_no))
...
def validate_serial_no_with_batch(serial_nos, item_code):
	if capkpi.get_cached_value("Serial No", serial_nos[0], "item_code") != item_code:
		capkpi.throw(
	...
	capkpi.throw(_("There is no batch found against the {0}: {1}").format(message, serial_no_link))


def make_batch(args):
	if capkpi.db.get_value("Item", args.item, "has_batch_no"):
		args.doctype = "Batch"
		capkpi.get_doc(args).insert().name
...
def get_pos_reserved_batch_qty(filters):
	...
	p = capkpi.qb.DocType("POS Invoice").as_("p")
	item = capkpi.qb.DocType("POS Invoice Item").as_("item")
