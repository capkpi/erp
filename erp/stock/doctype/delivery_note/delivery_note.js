...
capkpi.provide("erp.stock.delivery_note");
...
capkpi.ui.form.on("Delivery Note", {
	setup: function(frm) {
		...
		erp.queries.setup_warehouse_query(frm);
		...
		frm.set_query('expense_account', 'items', function(doc, cdt, cdn) {
			if (erp.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function(frm) {
		erp.stock.delivery_note.set_print_hide(frm.doc);
	},
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_return === 1 && frm.doc.per_billed !== 100) {
			frm.add_custom_button(__('Credit Note'), function() {
				capkpi.model.open_mapped_doc({
					method: "erp.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
					frm: cur_frm,
				})
			}, __('Create'));
		...
		if (frm.doc.docstatus == 1 && !frm.doc.inter_company_reference) {
			...
			if (internal) {
				...
				me.frm.add_custom_button(button_label, function() {
					capkpi.model.open_mapped_doc({
						method: 'erp.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt',
						frm: frm,
					});
				}, __('Create'));
			}
...
capkpi.ui.form.on("Delivery Note Item", {
...
erp.stock.DeliveryNoteController = erp.selling.SellingController.extend({
	setup: function(doc) {
	...
	refresh: function(doc, dt, dn) {
		...
		if (!doc.is_return && doc.status!="Closed") {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				this.frm.add_custom_button(__('Packing Slip'), function() {
					capkpi.model.open_mapped_doc({
						method: "erp.stock.doctype.delivery_note.delivery_note.make_packing_slip",
						frm: me.frm
					}) }, __('Create'));
			}
		...
		erp.stock.delivery_note.set_print_hide(doc, dt, dn);
	...
	make_shipment: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm
		})
	},
	...
	make_sales_return: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm
		})
	},
	...
	make_delivery_trip: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm
		})
	},
	...
	update_status: function(status) {
		...
		capkpi.call({
			method:"erp.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
$.extend(cur_frm.cscript, new erp.stock.DeliveryNoteController({frm: cur_frm}));
...
capkpi.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		var aii_enabled = erp.is_perpetual_inventory_enabled(frm.doc.company)
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})


erp.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
	var dn_fields = capkpi.meta.docfield_map['Delivery Note'];
	var dn_item_fields = capkpi.meta.docfield_map['Delivery Note Item'];
