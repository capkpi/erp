...
from capkpi.utils import cstr, flt, nowdate, nowtime
...
from erp.stock.doctype.delivery_note.delivery_note import (
	make_delivery_trip,
	make_sales_invoice,
)
from erp.stock.doctype.item.test_item import make_item
from erp.stock.doctype.purchase_receipt.test_purchase_receipt import get_gl_entries
from erp.stock.doctype.serial_no.serial_no import SerialNoWarehouseError, get_serial_nos
...
class TestDeliveryNote(CapKPITestCase):
	def test_over_billing_against_dn(self):
		capkpi.db.set_value("Stock Settings", None, "allow_negative_stock", 1)

		dn = create_delivery_note(do_not_submit=True)
		self.assertRaises(capkpi.ValidationError, make_sales_invoice, dn.name)
		...
		self.assertRaises(capkpi.ValidationError, capkpi.get_doc(si).insert)

	def test_delivery_note_no_gl_entry(self):
		company = capkpi.db.get_value("Warehouse", "_Test Warehouse - _TC", "company")
	...
	def test_sales_return_for_non_bundled_items_partial(self):
		...
		returned = capkpi.get_doc("Delivery Note", dn1.name)
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = capkpi.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Delivery Note",
				"voucher_no": dn.name,
	...
	def test_closed_delivery_note(self):
		from erp.stock.doctype.delivery_note.delivery_note import update_delivery_note_status
		...
		self.assertEqual(capkpi.db.get_value("Delivery Note", dn.name, "Status"), "Closed")
	...
	def test_dn_billing_status_case4(self):
		# SO -> SI -> DN
		from erp.accounts.doctype.sales_invoice.sales_invoice import make_delivery_note
	...
	def test_internal_transfer_precision_gle(self):
		...
		self.assertFalse(
			capkpi.db.exists("GL Entry", {"voucher_no": dn.name, "voucher_type": dn.doctype})
		)


def create_delivery_note(**args):
	dn = capkpi.new_doc("Delivery Note")
