...
from capkpi.exceptions import QueryDeadlockError, QueryTimeoutError
from capkpi.model.document import Document
from capkpi.utils import cint, get_link_to_form, get_weekday, now, nowtime
from capkpi.utils.user import get_users_with_role
from rq.timeouts import JobTimeoutException

import erp
from erp.accounts.utils import get_future_stock_vouchers, repost_gle_for_stock_vouchers
from erp.stock.stock_ledger import (
...
class RepostItemValuation(Document):
	...
	def set_company(self):
		if self.based_on == "Transaction":
			self.company = capkpi.get_cached_value(self.voucher_type, self.voucher_no, "company")
		elif self.warehouse:
			self.company = capkpi.get_cached_value("Warehouse", self.warehouse, "company")
	...
	def on_submit(self):
		...
		Exceptions:
		        ...
		        2. global flag capkpi.flags.dont_execute_stock_reposts is set
		...
		if not capkpi.flags.in_test:
			return
		if self.flags.dont_run_in_test or capkpi.flags.dont_execute_stock_reposts:
			return
	...
	def check_pending_repost_against_cancelled_transaction(self):
		...
		transaction_status = capkpi.db.get_value(self.voucher_type, self.voucher_no, "docstatus")
		if transaction_status == 2:
			msg = _("Cannot cancel as processing of cancelled documents is  pending.")
			msg += "<br>" + _("Please try again in an hour.")
			capkpi.throw(msg, title=_("Pending processing"))
...
def on_doctype_update():
	capkpi.db.add_index("Repost Item Valuation", ["warehouse", "item_code"], "item_warehouse")


def repost(doc):
	try:
		if not capkpi.db.exists("Repost Item Valuation", doc.name):
			return
		...
		message = capkpi.message_log.pop() if capkpi.message_log else ""
		if traceback:
			message += "<br>" + "Traceback: <br>" + traceback
		capkpi.db.set_value(doc.doctype, doc.name, "error_log", message)
...
def repost_gl_entries(doc):
	if not cint(erp.is_perpetual_inventory_enabled(doc.company)):
		return
...
def _get_directly_dependent_vouchers(doc):
	...
	if doc.based_on == "Transaction":
		ref_doc = capkpi.get_doc(doc.voucher_type, doc.voucher_no)
...
def notify_error_to_stock_managers(doc, traceback):
	...
	capkpi.sendmail(recipients=recipients, subject=subject, message=message)
...
def repost_entries():
	...
	for row in riv_entries:
		doc = capkpi.get_doc("Repost Item Valuation", row.name)
...
def in_configured_timeslot(repost_settings=None, current_time=None):
	...
	if repost_settings is None:
		repost_settings = capkpi.get_cached_doc("Stock Reposting Settings")
...
def execute_repost_item_valuation():
	...
	capkpi.get_doc("Scheduled Job Type", "repost_item_valuation.repost_entries").enqueue(force=True)
