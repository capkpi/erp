...
capkpi.ui.form.on('Repost Item Valuation', {
	setup: function(frm) {
	...
	setup_realtime_progress: function(frm) {
		capkpi.realtime.on('item_reposting_progress', data => {
	...
	execute_reposting(frm) {
		frm.add_custom_button(__("Start Reposting"), () => {
			capkpi.call({
				method: 'erp.stock.doctype.repost_item_valuation.repost_item_valuation.execute_repost_item_valuation',
				callback: function() {
					capkpi.msgprint(__('Reposting has been started in the background.'));
				}
			});
		});
	},
