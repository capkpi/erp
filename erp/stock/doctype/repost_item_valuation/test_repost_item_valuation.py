...
from capkpi.utils.data import add_to_date, today

from erp.accounts.utils import repost_gle_for_stock_vouchers
from erp.controllers.stock_controller import create_item_wise_repost_entries
...
from erp.stock.doctype.repost_item_valuation.repost_item_valuation import (
...
from erp.stock.utils import PendingRepostingError


class TestRepostItemValuation(CapKPITestCase, StockTestMixin):
	def tearDown(self):
		capkpi.flags.dont_execute_stock_reposts = False

	def test_repost_time_slot(self):
		repost_settings = capkpi.get_doc("Stock Reposting Settings")
	...
	def test_deduplication(self):
		...
		riv2 = capkpi.get_doc(riv_args.update({"posting_date": "2021-01-03"}))
		...
		riv3 = capkpi.get_doc(riv_args.update({"posting_date": "2021-01-01"}))
		...
		riv4 = capkpi.get_doc(riv_args.update({"warehouse": "Stores - _TC"}))
	...
	def test_prevention_of_cancelled_transaction_riv(self):
		...
		riv = capkpi.get_last_doc(
			"Repost Item Valuation", {"voucher_type": old.doctype, "voucher_no": old.name}
		)
		self.assertRaises(capkpi.ValidationError, riv.cancel)
	...
	def test_queue_progress_serialization(self):
		...
		self.assertEqual(
			[["a", "b"], ["c", "d"]],
			sorted(capkpi.parse_json(capkpi.as_json(set([("a", "b"), ("c", "d")])))),
		)

	def test_gl_repost_progress(self):
		from erp.accounts import utils
		...
		doc = capkpi.new_doc("Repost Item Valuation")
