...
from capkpi.utils import add_days, flt, now_datetime, nowdate

import erp
from erp.stock.doctype.delivery_trip.delivery_trip import (
...
class TestDeliveryTrip(CapKPITestCase):
	def setUp(self):
		super().setUp()
	...
	def tearDown(self):
		capkpi.db.sql("delete from `tabDriver`")
		capkpi.db.sql("delete from `tabVehicle`")
		capkpi.db.sql("delete from `tabEmail Template`")
		capkpi.db.sql("delete from `tabDelivery Trip`")
...
def create_address(driver):
	if not capkpi.db.exists("Address", {"address_title": "_Test Address for Driver"}):
		...
		capkpi.db.set_value("Driver", driver.name, "address", address.name)

		return address

	return capkpi.get_doc("Address", {"address_title": "_Test Address for Driver"})


def create_driver():
	if not capkpi.db.exists("Driver", {"full_name": "Newton Scmander"}):
		driver = capkpi.get_doc(
			{
				"doctype": "Driver",
	...
	return capkpi.get_doc("Driver", {"full_name": "Newton Scmander"})


def create_delivery_notification():
	if not capkpi.db.exists("Email Template", "Delivery Notification"):
		dispatch_template = capkpi.get_doc(
			{
	...
	delivery_settings = capkpi.get_single("Delivery Settings")
	delivery_settings.dispatch_template = "Delivery Notification"
	delivery_settings.save()


def create_vehicle():
	if not capkpi.db.exists("Vehicle", "JB 007"):
