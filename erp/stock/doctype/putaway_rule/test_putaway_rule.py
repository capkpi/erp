...
class TestPutawayRule(CapKPITestCase):
	def setUp(self):
		if not capkpi.db.exists("Item", "_Rice"):
			make_item(
				"_Rice", {"is_stock_item": 1, "has_batch_no": 1, "create_new_batch": 1, "stock_uom": "Kg"}
			)

		if not capkpi.db.exists("Warehouse", {"warehouse_name": "Rack 1"}):
			create_warehouse("Rack 1")
		if not capkpi.db.exists("Warehouse", {"warehouse_name": "Rack 2"}):
			create_warehouse("Rack 2")

		self.warehouse_1 = capkpi.db.get_value("Warehouse", {"warehouse_name": "Rack 1"})
		self.warehouse_2 = capkpi.db.get_value("Warehouse", {"warehouse_name": "Rack 2"})

		if not capkpi.db.exists("UOM", "Bag"):
	...
	def test_putaway_rules_multi_uom(self):
		...
		item = capkpi.get_doc("Item", "_Rice")
		if not capkpi.db.get_value("UOM Conversion Detail", {"parent": "_Rice", "uom": "Bag"}):
	...
	def test_putaway_rules_multi_uom_whole_uom(self):
		...
		capkpi.db.set_value("UOM", "Bag", "must_be_whole_number", 1)
	...
	def test_validate_over_receipt_in_warehouse(self):
		...
		self.assertRaises(capkpi.ValidationError, pr.save)
	...
	def test_putaway_rule_on_stock_entry_material_transfer_batch_serial_item(self):
		...
		if not capkpi.db.exists("Item", "Water Bottle"):
			make_item(
				"Water Bottle",
				{
					"is_stock_item": 1,
...
def create_putaway_rule(**args):
	...
	putaway = capkpi.new_doc("Putaway Rule")
	...
	putaway.stock_uom = capkpi.db.get_value("Item", putaway.item_code, "stock_uom")
