...
from capkpi.utils import cint, cstr, floor, flt, nowdate
...
class PutawayRule(Document):
	...
	def validate_duplicate_rule(self):
		existing_rule = capkpi.db.exists(
		...
		if existing_rule and existing_rule != self.name:
			capkpi.throw(
				_("Putaway Rule already exists for Item {0} in Warehouse {1}.").format(
					capkpi.bold(self.item_code), capkpi.bold(self.warehouse)
				),
				title=_("Duplicate"),
			)
	...
	def validate_priority(self):
		if self.priority < 1:
			capkpi.throw(_("Priority cannot be lesser than 1."), title=_("Invalid Priority"))

	def validate_warehouse_and_company(self):
		company = capkpi.db.get_value("Warehouse", self.warehouse, "company")
		if company != self.company:
			capkpi.throw(
				_("Warehouse {0} does not belong to Company {1}.").format(
					capkpi.bold(self.warehouse), capkpi.bold(self.company)
				),
				title=_("Invalid Warehouse"),
			)

	def validate_capacity(self):
		stock_uom = capkpi.db.get_value("Item", self.item_code, "stock_uom")
		...
		if flt(self.stock_capacity) < flt(balance_qty):
			capkpi.throw(
				...
				).format(self.item_code, capkpi.bold(balance_qty), stock_uom),
				title=_("Insufficient Capacity"),
			)

		if not self.capacity:
			capkpi.throw(_("Capacity must be greater than 0"), title=_("Invalid"))
...
def get_available_putaway_capacity(rule):
	stock_capacity, item_code, warehouse = capkpi.db.get_value(
...
def apply_putaway_rule(doctype, items, company, sync=None, purpose=None):
	...
	for item in items:
		...
		uom_must_be_whole_number = capkpi.db.get_value("UOM", item.uom, "must_be_whole_number")
	...
	if updated_table and _items_changed(items, updated_table, doctype):
		items[:] = updated_table
		capkpi.msgprint(_("Applied putaway rules."), alert=True)
...
def _items_changed(old, new, doctype: str) -> bool:
	...
	old = [capkpi._dict(item) if isinstance(item, dict) else item for item in old]
...
def show_unassigned_items_message(items_not_accomodated):
	...
	for entry in items_not_accomodated:
		item_link = capkpi.utils.get_link_to_form("Item", entry[0])
		formatted_item_rows += """
			<td>{0}</td>
			<td>{1}</td>
		</tr>""".format(
			item_link, capkpi.bold(entry[1])
		)
	...
	capkpi.msgprint(msg, title=_("Insufficient Capacity"), is_minimizable=True, wide=True)
