...
capkpi.ui.form.on('Material Request', {
	setup: function(frm) {
	...
	set_from_warehouse: function(frm) {
		if (frm.doc.material_request_type == "Material Transfer"
			...
			frm.doc.items.forEach(d => {
				capkpi.model.set_value(d.doctype, d.name,
	...
	make_custom_buttons: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.status != 'Stopped') {
			let precision = capkpi.defaults.get_default("float_precision");
	...
	update_status: function(frm, stop_status) {
		capkpi.call({
			method: 'erp.stock.doctype.material_request.material_request.update_status',
		...
		frm.call({
			...
			args: {
				args: {
					...
					buying_price_list: capkpi.defaults.get_default('buying_price_list'),
					currency: capkpi.defaults.get_default('Currency'),
					name: frm.doc.name,
					qty: item.qty || 1,
					...
					callback: function(r) {
						if (!r.message) {
							...
							$.each(r.message, function(i, item) {
								var d = capkpi.model.add_child(cur_frm.doc, "Material Request Item", "items");
		...
		capkpi.prompt(
			{
				...
				get_query: () => {
					return{
						query: "erp.stock.doctype.material_request.material_request.get_default_supplier_query",
	...
	make_request_for_quotation: function(frm) {
		capkpi.model.open_mapped_doc({
			method: "erp.stock.doctype.material_request.material_request.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true
		});
	},
	...
	make_supplier_quotation: function(frm) {
		capkpi.model.open_mapped_doc({
			method: "erp.stock.doctype.material_request.material_request.make_supplier_quotation",
			frm: frm
		});
	},
	...
	raise_work_orders: function(frm) {
		capkpi.call({
			method:"erp.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
capkpi.ui.form.on("Material Request Item", {
	qty: function (frm, doctype, name) {
		var d = locals[doctype][name];
		if (flt(d.qty) < flt(d.min_order_qty)) {
			capkpi.msgprint(__("Warning: Material Requested Qty is less than Minimum Order Qty"));
		}
...
erp.buying.MaterialRequestController = erp.buying.BuyingController.extend({
	tc_name: function() {
		this.get_terms();
	},
...
$.extend(cur_frm.cscript, new erp.buying.MaterialRequestController({frm: cur_frm}));
