...
# ERP - web based ERP (http://capkpi.com)
# For license information, please see license.txt


import capkpi
from capkpi.tests.utils import CapKPITestCase, change_settings
from capkpi.utils import add_days, cstr, flt, nowdate, nowtime, random_string

from erp.accounts.utils import get_stock_and_account_balance
...
from erp.stock.stock_ledger import get_previous_sle, update_entries_after
from erp.stock.tests.test_utils import StockTestMixin
from erp.stock.utils import get_incoming_rate, get_stock_value_on, get_valuation_method


class TestStockReconciliation(CapKPITestCase, StockTestMixin):
	@classmethod
	def setUpClass(cls):
	...
	def tearDown(self):
		...
		capkpi.flags.pop("dont_execute_stock_reposts", None)
	...
	def test_stock_reco_for_serialized_item(self):
		...
		for d in to_delete_records:
			stock_doc = capkpi.get_doc("Stock Reconciliation", d)
			stock_doc.cancel()
	...
	def test_stock_reco_for_serial_and_batch_item(self):
		...
		self.assertEqual(capkpi.db.get_value("Serial No", serial_nos[0], "batch_no"), batch_no)

		sr.cancel()

		self.assertEqual(capkpi.db.get_value("Serial No", serial_nos[0], "status"), "Inactive")
	...
	def test_stock_reco_for_serial_and_batch_item_with_future_dependent_entry(self):
		...
		self.assertEqual(capkpi.db.get_value("Serial No", reco_serial_no, "batch_no"), batch_no)
		self.assertEqual(capkpi.db.get_value("Serial No", reco_serial_no, "status"), "Active")

		# Check if Serial No from Stock Entry is Unlinked and Inactive
		self.assertEqual(capkpi.db.get_value("Serial No", serial_no_2, "batch_no"), None)
		self.assertEqual(capkpi.db.get_value("Serial No", serial_no_2, "status"), "Inactive")
	...
	def test_backdated_stock_reco_qty_reposting(self):
		...
		capkpi.flags.dont_execute_stock_reposts = True

		def assertBalance(doc, qty_after_transaction):
			sle_balance = capkpi.db.get_value(
	...
	def test_backdated_stock_reco_cancellation_future_negative_stock(self):
		...
		dn_balance = capkpi.db.get_value(
		...
		repost_exists = bool(capkpi.db.exists("Repost Item Valuation", {"voucher_no": sr.name}))
	...
	def test_intermediate_sr_bin_update(self):
		...
		old_bin_qty = capkpi.db.get_value(
		...
		new_bin_qty = capkpi.db.get_value(
			"Bin", {"item_code": item_code, "warehouse": warehouse}, "actual_qty"
		)
	...
	def test_valid_batch(self):
		...
		self.assertRaises(capkpi.ValidationError, sr.submit)
...
def create_batch_item_with_batch(item_name, batch_id):
	...
	if not capkpi.db.exists("Batch", batch_id):
		b = capkpi.new_doc("Batch")
		b.item = item_name
		b.batch_id = batch_id
		b.save()
...
def create_stock_reconciliation(**args):
	...
	sr = capkpi.new_doc("Stock Reconciliation")
	...
	sr.expense_account = args.expense_account or (
		"Stock Adjustment - _TC" if capkpi.get_all("Stock Ledger Entry") else "Temporary Opening - _TC"
	)
	sr.cost_center = (
		args.cost_center
		or capkpi.get_cached_value("Company", sr.company, "cost_center")
...
def set_valuation_method(item_code, valuation_method):
	...
	capkpi.db.set_value("Item", item_code, "valuation_method", valuation_method)

	for warehouse in capkpi.get_all(
