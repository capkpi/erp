...
from capkpi.query_builder import Order
from capkpi.query_builder.functions import CombineDatetime
...
class Bin(Document):
	def before_save(self):
		if self.get("__islocal") or not self.stock_uom:
			self.stock_uom = capkpi.get_cached_value("Item", self.item_code, "stock_uom")
	...
	def update_reserved_qty_for_production(self):
		...
		in open work orders"""
		from erp.manufacturing.doctype.work_order.work_order import get_reserved_qty_for_production
	...
	def update_reserved_qty_for_sub_contracting(self):
		# reserved qty
		reserved_qty_for_sub_contract = capkpi.db.sql(
			"""
		...
		if capkpi.db.field_exists("Stock Entry", "is_return"):
...
def on_doctype_update():
	capkpi.db.add_unique("Bin", ["item_code", "warehouse"], constraint_name="unique_item_warehouse")


def update_stock(bin_name, args, allow_negative_stock=False, via_landed_cost_voucher=False):
	"""WARNING: This function is deprecated. Inline this function instead of using it."""
	from erp.stock.stock_ledger import repost_current_voucher
