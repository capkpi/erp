...
class ItemAttributeIncrementError(capkpi.ValidationError):
	pass
	...
	def validate_numeric(self):
		if self.numeric_values:
			...
			if self.from_range is None or self.to_range is None:
				capkpi.throw(_("Please specify from/to range"))

			elif flt(self.from_range) >= flt(self.to_range):
				capkpi.throw(_("From Range has to be less than To Range"))

			if not self.increment:
				capkpi.throw(_("Increment cannot be 0"), ItemAttributeIncrementError)
		else:
	...
	def validate_duplication(self):
		...
		for d in self.item_attribute_values:
			...
			if d.attribute_value in values:
				capkpi.throw(_("{0} must appear only once").format(d.attribute_value))
			values.append(d.attribute_value)

			if d.abbr in abbrs:
				capkpi.throw(_("{0} must appear only once").format(d.abbr))
			abbrs.append(d.abbr)
