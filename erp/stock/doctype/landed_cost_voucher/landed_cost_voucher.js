...
erp.stock.LandedCostVoucher = erp.stock.StockController.extend({
	setup: function() {
		var me = this;
		this.frm.fields_dict.purchase_receipts.grid.get_field('receipt_document').get_query =
			function (doc, cdt, cdn) {
				...
				if (!me.frm.doc.company) capkpi.msgprint(__("Please enter company first"));
				return {
					filters: filters
				}
			};
	...
	refresh: function() {
		...
		if (this.frm.doc.company) {
			let company_currency = capkpi.get_doc(":Company", this.frm.doc.company).default_currency;
	...
	get_items_from_purchase_receipts: function() {
		...
		if(!this.frm.doc.purchase_receipts.length) {
			capkpi.msgprint(__("Please enter Purchase Receipt first"));
		} else {
...
cur_frm.script_manager.make(erp.stock.LandedCostVoucher);
