...
from capkpi.model.mapper import map_child_doc
from capkpi.utils import cint, floor, flt, today
from capkpi.utils.nestedset import get_descendants_of

from erp.selling.doctype.sales_order.sales_order import (
	make_delivery_note as create_delivery_note_from_sales_order,
)
from erp.stock.get_item_details import get_conversion_factor
...
class PickList(Document):
	...
	def before_save(self):
		...
		for location in self.get("locations"):
			if (
				...
				and capkpi.db.get_value("Sales Order", location.sales_order, "per_picked") == 100
			):
				capkpi.throw("Row " + str(location.idx) + " has been picked already!")
	...
	def before_submit(self):
		...
		for item in self.locations:
			...
			if not item.serial_no:
				capkpi.throw(
					_("Row #{0}: {1} does not have any available serial numbers in {2}").format(
						capkpi.bold(item.idx), capkpi.bold(item.item_code), capkpi.bold(item.warehouse)
					),
			...
			capkpi.throw(
				...
				).format(capkpi.bold(item.item_code), capkpi.bold(item.idx)),
	...
	def update_sales_order_item(self, item, picked_qty, item_code):
		...
		already_picked, actual_qty = capkpi.db.get_value(
			item_table,
		...
		if self.docstatus == 1:
			if (((already_picked + picked_qty) / actual_qty) * 100) > (
				100 + flt(capkpi.db.get_single_value("Stock Settings", "over_delivery_receipt_allowance"))
			):
				capkpi.throw(
					_(
		...
		capkpi.db.set_value(item_table, item.sales_order_item, "picked_qty", already_picked + picked_qty)

	@staticmethod
	def update_sales_order_picking_status(sales_orders: Set[str]) -> None:
		for sales_order in sales_orders:
			if sales_order:
				capkpi.get_doc("Sales Order", sales_order).update_picking_status()

	@capkpi.whitelist()
	def set_item_locations(self, save=False):
		self.validate_for_qty()
		items = self.aggregate_item_qty()
		self.item_location_map = capkpi._dict()
	...
	def aggregate_item_qty(self):
		...
		for item in locations:
			if not item.item_code:
				capkpi.throw("Row #{0}: Item Code is Mandatory".format(item.idx))
	...
	def validate_for_qty(self):
		if self.purpose == "Material Transfer for Manufacture" and (
			...
			capkpi.throw(_("Qty of Finished Goods Item should be greater than 0."))
	...
	def update_bundle_picked_qty(self):
		...
		for so_row, item_code in product_bundles.items():
			...
			already_picked = capkpi.db.get_value(item_table, so_row, "picked_qty")
			capkpi.db.set_value(
				item_table,
				so_row,
				"picked_qty",
	...
	def _get_product_bundles(self) -> Dict[str, str]:
		...
		for item in self.locations:
			if not item.product_bundle_item:
				continue
			product_bundles[item.product_bundle_item] = capkpi.db.get_value(
				"Sales Order Item",
	...
	def _get_product_bundle_qty_map(self, bundles: List[str]) -> Dict[str, Dict[str, float]]:
		...
		for bundle_item_code in bundles:
			bundle = capkpi.get_last_doc("Product Bundle", {"new_item_code": bundle_item_code})
	...
	def _compute_picked_qty_for_bundle(self, bundle_row, bundle_items) -> int:
		...
		precision = capkpi.get_precision("Stock Ledger Entry", "qty_after_transaction")
...
def validate_item_locations(pick_list):
	if not pick_list.locations:
		capkpi.throw(_("Add items in the Item Locations table"))
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
	...
	while remaining_stock_qty > 0 and available_locations:
		...
		item_location = capkpi._dict(item_location)
		...
		uom_must_be_whole_number = capkpi.db.get_value("UOM", item_doc.uom, "must_be_whole_number")
...
def get_available_item_locations(
	...
	locations = []
	has_serial_no = capkpi.get_cached_value("Item", item_code, "has_serial_no")
	has_batch_no = capkpi.get_cached_value("Item", item_code, "has_batch_no")
	...
	if remaining_qty > 0 and not ignore_validation:
		capkpi.msgprint(
			_("{0} units of Item {1} is not available.").format(
				remaining_qty, capkpi.get_desk_link("Item", item_code)
			),
...
def get_available_item_locations_for_serialized_item(
	...
	filters = capkpi._dict({"item_code": item_code, "company": company, "warehouse": ["!=", ""]})
	...
	warehouse_serial_nos_map = capkpi._dict()
...
def get_available_item_locations_for_other_item(item_code, from_warehouses, required_qty, company):
	...
	warehouses = [x.get("name") for x in capkpi.get_list("Warehouse", {"company": company}, "name")]
...
def create_delivery_note(source_name, target_doc=None):
	pick_list = capkpi.get_doc("Pick List", source_name)
	...
	capkpi.msgprint(_("Delivery Note(s) created for the Pick List"))
	return delivery_note


def create_dn_wo_so(pick_list):
	delivery_note = capkpi.new_doc("Delivery Note")
...
def map_pl_locations(pick_list, item_mapper, delivery_note, sales_order=None):
	...
	for location in pick_list.locations:
		...
		if location.sales_order_item:
			sales_order_item = capkpi.get_doc("Sales Order Item", location.sales_order_item)
		else:
	...
	delivery_note.customer = capkpi.get_value("Sales Order", sales_order, "customer")
...
def add_product_bundles_to_delivery_note(
	...
	for so_row, item_code in product_bundles.items():
		sales_order_item = capkpi.get_doc("Sales Order Item", so_row)
...
def create_stock_entry(pick_list):
	pick_list = capkpi.get_doc(json.loads(pick_list))
	validate_item_locations(pick_list)

	if stock_entry_exists(pick_list.get("name")):
		return capkpi.msgprint(_("Stock Entry has been already created against this Pick List"))
...
def get_pending_work_orders(doctype, txt, searchfield, start, page_length, filters, as_dict):
	return capkpi.db.sql(
			...
			%(start)s, %(page_length)s""",
		{
			...
			"_txt": txt.replace("%", ""),
			"start": start,
			"page_length": capkpi.utils.cint(page_length),
...
def target_document_exists(pick_list_name, purpose):
	if purpose == "Delivery":
		return capkpi.db.exists("Delivery Note", {"pick_list": pick_list_name})
...
def get_item_details(item_code, uom=None):
	details = capkpi.db.get_value("Item", item_code, ["stock_uom", "name"], as_dict=1)
...
def update_delivery_note_item(source, target, delivery_note):
	cost_center = capkpi.db.get_value("Project", delivery_note.project, "cost_center")
	if not cost_center:
...
def stock_entry_exists(pick_list_name):
	return capkpi.db.exists("Stock Entry", {"pick_list": pick_list_name})


def update_stock_entry_based_on_work_order(pick_list, stock_entry):
	work_order = capkpi.get_doc("Work Order", pick_list.get("work_order"))
	...
	is_wip_warehouse_group = capkpi.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = capkpi.get_value(
