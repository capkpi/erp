...
from erp.selling.doctype.sales_order.sales_order import create_pick_list
...
from erp.stock.doctype.packed_item.test_packed_item import create_product_bundle
from erp.stock.doctype.pick_list.pick_list import create_delivery_note
...
class TestPickList(CapKPITestCase):
	...
	def test_pick_list_shows_serial_no_for_serialized_item(self):
		...
		stock_reconciliation = capkpi.get_doc(
			{
				"doctype": "Stock Reconciliation",
	...
	def test_pick_list_shows_batch_no_for_batched_item(self):
		...
		item = capkpi.db.exists("Item", {"item_name": "Batched Item"})
		if not item:
			...
			item = capkpi.get_doc("Item", {"item_name": "Batched Item"})
		...
		sales_order = capkpi.get_doc(
			{
				...
				"customer": "_Test Customer",
				"company": "_Test Company",
				"items": [{"item_code": "_Test Item", "qty": 10, "delivery_date": capkpi.utils.today()}],
			}
		)
		sales_order.submit()
				...
				"items": [
					{
						...
						"conversion_factor": 5,
						"stock_qty": 5,
						"delivery_date": capkpi.utils.today(),
					},
					{
						"item_code": "_Test Item",
						"qty": 1,
	...
	def test_multiple_dn_creation(self):
		...
		for dn in capkpi.get_all(
			"Delivery Note",
			filters={"pick_list": pick_list.name, "customer": "_Test Customer"},
			fields={"name"},
		):
			for dn_item in capkpi.get_doc("Delivery Note", dn.name).get("items"):
