// Copyright (c) 2015, CapKPI Technologies Pvt. Ltd. and Contributors // License: GNU General Public License v3. See license.txt

capkpi.provide("erp.stock");
capkpi.provide("erp.accounts.dimensions");

{% include 'erp/stock/landed_taxes_and_charges_common.js' %};

capkpi.ui.form.on('Stock Entry', {
	setup: function(frm) {
		...
		capkpi.db.get_value('Stock Settings', {name: 'Stock Settings'}, 'sample_retention_warehouse', (r) => {
		...
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			...
			if(!item.item_code) {
				capkpi.throw(__("Please enter Item Code to get Batch Number"));
			} else {
				if (in_list(["Material Transfer for Manufacture", "Manufacture", "Repack", "Send to Subcontractor"], doc.purpose)) {
					var filters = {
						'item_code': item.item_code,
						'posting_date': frm.doc.posting_date || capkpi.datetime.nowdate()
					}
				} else {
					var filters = {
						'item_code': item.item_code
		...
		capkpi.db.get_single_value('Stock Settings', 'disable_serial_no_and_batch_selector')
		.then((value) => {
			if (value) {
	...
	setup_quality_inspection: function(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(__("Quality Inspection(s)"), () => {
				let transaction_controller = new erp.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function(doc, cdt, cdn) {
			...
			return {
				query:"erp.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function(frm) {
		if(!frm.doc.docstatus) {
			...
			frm.add_custom_button(__('Material Request'), function() {
				capkpi.model.with_doctype('Material Request', function() {
					var mr = capkpi.model.get_new_doc('Material Request');
					var items = frm.get_field('items').grid.get_selected_children();
					if(!items.length) {
						items = frm.doc.items;
					}
					items.forEach(function(item) {
						var mr_item = capkpi.model.add_child(mr, 'items');
						...
						mr_item.required_date = capkpi.datetime.nowdate();
					});
					capkpi.set_route('Form', 'Material Request', mr.name);
				});
			}, __("Create"));
		}
		...
		if(frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Item'), () => {
					erp.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
		...
		if (frm.doc.docstatus === 1) {
			if (frm.doc.add_to_transit && frm.doc.purpose=='Material Transfer' && frm.doc.per_transferred < 100) {
				frm.add_custom_button('End Transit', function() {
					capkpi.model.open_mapped_doc({
						method: "erp.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm
					})
				});
			}
			...
			if (frm.doc.per_transferred > 0) {
				frm.add_custom_button(__('Received Stock Entries'), function() {
					...
					capkpi.set_route('List', 'Stock Entry');
				}, __("View"));
			}
		}
		...
		if (frm.doc.docstatus===0) {
			frm.add_custom_button(__('Purchase Invoice'), function() {
				erp.utils.map_current_doc({
					method: "erp.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(__('Material Request'), function() {
				...
				const d = erp.utils.map_current_doc({
					method: "erp.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus===0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				capkpi.call({
					method: "erp.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("items", []);
	...
	validate_purpose_consumption: function(frm) {
		capkpi.call({
			method: "erp.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
		}).then(r => {
			if (cint(r.message) == 0
				&& frm.doc.purpose=="Material Consumption for Manufacture") {
				frm.set_value("purpose", 'Manufacture');
				capkpi.throw(__('Material Consumption is not set in Manufacturing Settings.'));
			}
		});
	},
	...
	company: function(frm) {
		if(frm.doc.company) {
			var company_doc = capkpi.get_doc(":Company", frm.doc.company);
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		var d = capkpi.model.get_doc(cdt, cdn);
		...
		capkpi.call({
			method: "erp.stock.get_item_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe && r.message){
					capkpi.model.set_value(cdt, cdn, "serial_no", r.message);
				}
				if (callback) {
					callback();
				}
		...
		capkpi.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					capkpi.msgprint(__("Retention Stock Entry already created or Sample Quantity not provided"));
				}
			}
		});
	},
	...
	set_basic_rate: function(frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			capkpi.call({
				...
				callback: function(r) {
					capkpi.model.set_value(cdt, cdn, 'basic_rate', (r.message || 0.0));
	...
	get_warehouse_details: function(frm, cdt, cdn) {
		...
		if(!child.bom_no) {
			capkpi.call({
				method: "erp.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					"args": {
				...
				callback: function(r) {
					if (!r.exc) {
						["actual_qty", "basic_rate"].forEach((field) => {
							capkpi.model.set_value(cdt, cdn, field, (r.message[field] || 0.0));
						});
	...
	get_items_from_bom: function(frm) {
		...
		d.get_input("fetch").on("click", function() {
			...
			if(!frm.doc.company) capkpi.throw(__("Company field is required"));
			capkpi.call({
				method: "erp.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function(r) {
					if (!r.message) {
						capkpi.throw(__("BOM does not contain any stock item"));
					} else {
						erp.utils.remove_empty_first_row(frm, "items");
						$.each(r.message, function(i, item) {
							let d = capkpi.model.add_child(cur_frm.doc, "Stock Entry Detail", "items");
	...
	calculate_total_additional_costs: function(frm) {
		const total_additional_costs = capkpi.utils.sum(
	...
	source_warehouse_address: function(frm) {
		erp.utils.get_address_display(frm, 'source_warehouse_address', 'source_address_display', false);
	},

	target_warehouse_address: function(frm) {
		erp.utils.get_address_display(frm, 'target_warehouse_address', 'target_address_display', false);
	},
	...
	set_transit_warehouse: function(frm) {
		if(frm.doc.add_to_transit && frm.doc.purpose == 'Material Transfer' && !frm.doc.to_warehouse
			...
			capkpi.db.get_value(dt, dn, 'default_in_transit_warehouse', (r) => {
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) erp.apply_putaway_rule(frm, frm.doc.purpose);
	}
});

capkpi.ui.form.on('Stock Entry Detail', {
	...
	s_warehouse: function(frm, cdt, cdn) {
		...
		if (item.s_warehouse) {
			capkpi.model.set_value(cdt, cdn, "allow_zero_valuation_rate", 0);
		}
	},
	...
	barcode: function(doc, cdt, cdn) {
		...
		if (d.barcode) {
			capkpi.call({
				method: "erp.stock.get_item_details.get_item_code",
				args: {"barcode": d.barcode },
				callback: function(r) {
					if (!r.exe){
						capkpi.model.set_value(cdt, cdn, "item_code", r.message);
					}
				}
			});
		}
	...
	uom: function(doc, cdt, cdn) {
		...
		if(d.uom && d.item_code){
			return capkpi.call({
				method: "erp.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty
				...
				callback: function(r) {
					if(r.message) {
						...
						$.each(r.message, function(k, v) {
							if (v) {
								capkpi.model.set_value(cdt, cdn, k, v); // qty and it's subsequent fields weren't triggered
							}
						});
						refresh_field("items");
						...
						if (no_batch_serial_number_value && !capkpi.flags.hide_serial_batch_dialog) {
							erp.stock.select_batch_and_serial_no(frm, d);
						}
					}
				}
			});
	...
	expense_account: function(frm, cdt, cdn) {
		erp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},
	cost_center: function(frm, cdt, cdn) {
		erp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
...
capkpi.ui.form.on('Landed Cost Taxes and Charges', {
...
erp.stock.StockEntry = erp.stock.StockController.extend({
	setup: function() {
		var me = this;
		...
		if(me.frm.doc.company && erp.is_perpetual_inventory_enabled(me.frm.doc.company)) {
			this.frm.add_fetch("company", "stock_adjustment_account", "expense_account");
		}

		this.frm.fields_dict.items.grid.get_field('expense_account').get_query = function() {
			if (erp.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						"company": me.frm.doc.company,
						"is_group": 0
		...
		capkpi.dynamic_link = { doc: this.frm.doc, fieldname: 'supplier', doctype: 'Supplier' }
		this.frm.set_query("supplier_address", erp.queries.address_query)
	},
	...
	refresh: function() {
		...
		if (this.frm.doc.docstatus===1 && erp.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		erp.hide_company();
		erp.utils.add_item(this.frm);
	},

	scan_barcode: function() {
		let transaction_controller= new erp.TransactionController({frm:this.frm});
	...
	set_default_account: function(callback) {
		...
		if(this.frm.doc.company && erp.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "erp.accounts.utils.get_company_default",
				args: {
	...
	clean_up: function() {
		...
		if(this.frm.doc.work_order &&
			...
			capkpi.model.remove_from_locals("Work Order",
	...
	get_items: function() {
		...
		if(!this.frm.doc.fg_completed_qty || !this.frm.doc.bom_no)
			capkpi.throw(__("BOM and Manufacturing Quantity are required"));
	...
	work_order: function() {
		...
		return capkpi.call({
			method: "erp.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	add_excise_button: function() {
		if(capkpi.boot.sysdefaults.country === "India")
			this.frm.add_custom_button(__("Excise Invoice"), function() {
				var excise = capkpi.model.make_new_doc_and_get_name('Journal Entry');
				excise = locals['Journal Entry'][excise];
				excise.voucher_type = 'Excise Entry';
				capkpi.set_route('Form', 'Journal Entry', excise.name);
			}, __('Create'));
	},
	...
	set_warehouse_in_children: function(child_table, warehouse_field, warehouse) {
		let transaction_controller = new erp.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	},

	items_on_form_rendered: function(doc, grid_row) {
		erp.setup_serial_or_batch_no();
	},
	...
	supplier: function(doc) {
		erp.utils.get_party_details(this.frm, null, null, null);
	}
});

erp.stock.select_batch_and_serial_no = (frm, item) => {
...
function attach_bom_items(bom_no) {
	...
	capkpi.db.get_doc("BOM",bom_no).then(bom => {
		const {name, items} = bom
		erp.stock.bom = {name, items:{}}
		items.forEach(item => {
			erp.stock.bom.items[item.item_code] = item;
		});
	});
}
...
function check_should_not_attach_bom_items(bom_no) {
  return (
	bom_no === undefined ||
	(erp.stock.bom && erp.stock.bom.name === bom_no)
  );
}

$.extend(cur_frm.cscript, new erp.stock.StockEntry({frm: cur_frm}));
