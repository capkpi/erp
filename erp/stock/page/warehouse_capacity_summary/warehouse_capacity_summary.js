capkpi.pages['warehouse-capacity-summary'].on_page_load = function(wrapper) {
	...
	page.sort_selector = new capkpi.ui.SortSelector({
	...
	capkpi.require('assets/js/item-dashboard.min.js', function() {
		$(capkpi.render_template('warehouse_capacity_summary_header')).appendTo(page.main);

		page.capacity_dashboard = new erp.stock.ItemDashboard({
			page_name: "warehouse-capacity-summary",
			page_length: 10,
			parent: page.main,
			sort_by: 'stock_capacity',
			sort_order: 'desc',
			method: 'erp.stock.dashboard.warehouse_capacity_dashboard.get_data',
		...
		let setup_click = function(doctype) {
			page.main.on('click', 'a[data-type="'+ doctype.toLowerCase() +'"]', function() {
				...
				if (field.get_value()===name) {
					capkpi.set_route('Form', doctype, name);
				} else {
