...
from capkpi.query_builder import Field
from capkpi.query_builder.functions import Min, Timestamp
...
def execute(filters=None):
	if not erp.is_perpetual_inventory_enabled(filters.company):
		capkpi.throw(
...
def get_unsync_date(filters):
	...
	if not date:
		date = (capkpi.qb.from_("Stock Ledger Entry").select(Min(Field("posting_date")))).run()
		date = date[0][0]
