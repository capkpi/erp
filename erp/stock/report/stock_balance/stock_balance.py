...
from capkpi.utils import cint, date_diff, flt, getdate
from six import iteritems

import erp
from erp.stock.report.stock_ageing.stock_ageing import FIFOSlots, get_average_age
from erp.stock.report.stock_ledger.stock_ledger import get_item_group_condition
from erp.stock.utils import add_additional_uom_columns, is_reposting_item_valuation_in_progress
...
def execute(filters=None):
	...
	if filters.get("company"):
		...
		company_currency = capkpi.db.get_single_value("Global Defaults", "default_currency")
...
def get_conditions(filters):
	...
	if not filters.get("from_date"):
		capkpi.throw(_("'From Date' is required"))

	if filters.get("to_date"):
		conditions += " and sle.posting_date <= %s" % capkpi.db.escape(filters.get("to_date"))
	else:
		capkpi.throw(_("'To Date' is required"))

	if filters.get("company"):
		conditions += " and sle.company = %s" % capkpi.db.escape(filters.get("company"))
...
def get_stock_ledger_entries(filters, items):
	...
	if items:
		item_conditions_sql = " and sle.item_code in ({})".format(
			", ".join(capkpi.db.escape(i, percent=False) for i in items)
		)
...
def get_item_warehouse_map(filters, sle):
	...
	float_precision = cint(capkpi.db.get_default("float_precision")) or 3

	for d in sle:
		key = (d.company, d.item_code, d.warehouse)
		if key not in iwb_map:
			iwb_map[key] = capkpi._dict(
				{
					"opening_qty": 0.0,
					"opening_val": 0.0,
					"in_qty": 0.0,
		...
		if d.posting_date < from_date or (
			...
			and capkpi.db.get_value("Stock Reconciliation", d.voucher_no, "purpose") == "Opening Stock"
		):
...
def get_item_details(items, sle, filters):
	...
	if filters.get("include_uom"):
		...
		cf_join = (
			...
			% capkpi.db.escape(filters.get("include_uom"))
		)
...
def get_item_reorder_details(items):
	item_reorder_details = capkpi._dict()

	if items:
		item_reorder_details = capkpi.db.sql(
			"""
...
def get_variants_attributes():
	...
	return [i.name for i in capkpi.get_all("Item Attribute")]


def get_variant_values_for(items):
	"""Returns variant values for items."""
	attribute_map = {}
	for attr in capkpi.db.sql(
