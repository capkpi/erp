...
from capkpi.utils import cint, cstr, flt, get_datetime, get_link_to_form, getdate, now, nowdate
from six import iteritems

import erp
from erp.stock.doctype.bin.bin import update_qty as update_bin_qty
from erp.stock.utils import (
...
class NegativeStockError(capkpi.ValidationError):
	pass


class SerialNoExistsInFutureTransaction(capkpi.ValidationError):
	pass
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from erp.controllers.stock_controller import future_sle_exists
	...
	if sl_entries:
		...
		for sle in sl_entries:
			...
			is_stock_item = capkpi.get_cached_value("Item", args.get("item_code"), "is_stock_item")
			if is_stock_item:
...
def validate_serial_no(sle):
	...
	for sn in get_serial_nos(sle.serial_no):
		...
		for row in get_stock_ledger_entries(args, ">"):
			voucher_type = capkpi.bold(row.voucher_type)
			voucher_no = capkpi.bold(get_link_to_form(row.voucher_type, row.voucher_no))
			vouchers.append(f"{voucher_type} {voucher_no}")

		if vouchers:
			serial_no = capkpi.bold(sn)
			msg = (
			...
			capkpi.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransaction)


def validate_cancellation(args):
	if args[0].get("is_cancelled"):
		repost_entry = capkpi.db.get_value(
		...
		if repost_entry:
			...
			if repost_entry.status == "Queued":
				doc = capkpi.get_doc("Repost Item Valuation", repost_entry.name)
				doc.status = "Skipped"
	...
	doc.db_set(
		{
			...
			"current_index": index,
			"affected_transactions": capkpi.as_json(affected_transactions),
		}
	)
...
def get_items_to_be_repost(voucher_type=None, voucher_no=None, doc=None):
	...
	if not items_to_be_repost and voucher_type and voucher_no:
		items_to_be_repost = capkpi.db.get_all(
			"Stock Ledger Entry",
...
def get_distinct_item_warehouse(args=None, doc=None):
	...
	if doc and doc.distinct_item_and_warehouse:
		...
		distinct_item_warehouses = {
			capkpi.safe_eval(k): capkpi._dict(v) for k, v in distinct_item_warehouses.items()
		}
	else:
		for i, d in enumerate(args):
			distinct_item_warehouses.setdefault(
				(d.item_code, d.warehouse), capkpi._dict({"reposting_status": False, "sle": d, "args_idx": i})
			)
...
def get_affected_transactions(doc) -> Set[Tuple[str, str]]:
	...
	transactions = capkpi.parse_json(doc.affected_transactions)
...
class update_entries_after(object):
	...
	def __init__(
		...
		self.allow_negative_stock = allow_negative_stock or cint(
			capkpi.db.get_single_value("Stock Settings", "allow_negative_stock")
		)

		self.args = capkpi._dict(args)
		self.item_code = args.get("item_code")
		if self.args.sle_id:
			self.args["name"] = self.args.sle_id

		self.company = capkpi.get_cached_value("Warehouse", self.args.warehouse, "company")
		self.set_precision()
		self.valuation_method = get_valuation_method(self.item_code)

		self.new_items_found = False
		self.distinct_item_warehouses = args.get("distinct_item_warehouses", capkpi._dict())
		self.affected_transactions: Set[Tuple[str, str]] = set()

		self.data = capkpi._dict()
		self.initialize_previous_data(self.args)
		self.build()

	def set_precision(self):
		self.flt_precision = cint(capkpi.db.get_default("float_precision")) or 2
		self.currency_precision = get_field_precision(
			capkpi.get_meta("Stock Ledger Entry").get_field("stock_value")
		)
	...
	def initialize_previous_data(self, args):
		...
		"""
		self.data.setdefault(args.warehouse, capkpi._dict())
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or capkpi._dict(
	...
	def update_distinct_item_warehouses(self, dependant_sle):
		...
		val = capkpi._dict({"sle": dependant_sle})
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		if sle.voucher_type == "Stock Entry":
			...
			rate = capkpi.db.get_value("Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
		# Sales and Purchase Return
			...
			if capkpi.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from erp.controllers.sales_and_purchase_return import (
				...
				and capkpi.get_cached_value(sle.voucher_type, sle.voucher_no, "is_internal_supplier")
			):
				field = (
				...
				refernce_name = capkpi.get_cached_value(
					sle.voucher_type + " Item", sle.voucher_detail_no, field
				)

				if refernce_name:
					rate = capkpi.get_cached_value(
						doctype,
						refernce_name,
						"incoming_rate",
					)
				...
				# check in item table
				item_code, incoming_rate = capkpi.db.get_value(
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		capkpi.db.set_value("Stock Entry Detail", sle.voucher_detail_no, "basic_rate", outgoing_rate)
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no):
		stock_entry = capkpi.get_doc("Stock Entry", voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		item_code = capkpi.db.get_value(sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
		if item_code == sle.item_code:
			capkpi.db.set_value(
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if capkpi.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			capkpi.db.set_value(
		...
		if capkpi.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted") == "Yes":
			doc = capkpi.get_doc(sle.voucher_type, sle.voucher_no)
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if ref_item_dt:
			return capkpi.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=erp.get_company_currency(sle.company),
			company=sle.company,
		)
	...
	def get_sle_before_datetime(self, args):
		...
		sle = sle[0] if sle else capkpi._dict()
		return sle
	...
	def raise_exceptions(self):
		...
		for warehouse, exceptions in iteritems(self.exceptions):
			...
			) in capkpi.local.flags.currently_saving:

				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					abs(deficiency),
					capkpi.get_desk_link("Item", exceptions[0]["item_code"]),
					capkpi.get_desk_link("Warehouse", warehouse),
				)
			else:
				msg = _(
					...
					capkpi.get_desk_link(exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
				)
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				capkpi.throw(message, NegativeStockError, title=_("Insufficient Stock"))
			else:
	...
	def update_bin(self):
		...
		for warehouse, data in self.data.items():
			...
			capkpi.db.set_value("Bin", bin_name, updated_values)
...
def get_previous_sle_of_current_voucher(args, exclude_current_voucher=False):
	...
	return sle[0] if sle else capkpi._dict()
...
def get_stock_ledger_entries(
	...
	if check_serial_no and previous_sle.get("serial_no"):
		# conditions += " and serial_no like {}".format(capkpi.db.escape('%{0}%'.format(previous_sle.get("serial_no"))))
...
def get_valuation_rate(
	...
	if not company:
		company = capkpi.get_cached_value("Warehouse", warehouse, "company")

	# Get valuation rate from last sle for the same item and warehouse
	last_valuation_rate = capkpi.db.sql(
	...
	if not valuation_rate:
		...
		valuation_rate = capkpi.db.get_value("Item", item_code, "standard_rate")

		if not valuation_rate:
			# try in price list
			valuation_rate = capkpi.db.get_value(
	...
	if (
		...
		and cint(erp.is_perpetual_inventory_enabled(company))
	):
		...
		capkpi.throw(msg=msg, title=_("Valuation Rate Missing"))
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if is_negative_with_precision(neg_sle):
		message = _(
			...
			capkpi.get_desk_link("Item", args.item_code),
			capkpi.get_desk_link("Warehouse", args.warehouse),
			neg_sle[0]["posting_date"],
			neg_sle[0]["posting_time"],
			capkpi.get_desk_link(neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
		)
	...
	if is_negative_with_precision(neg_batch_sle, is_batch=True):
		message = _(
			...
			capkpi.get_desk_link("Batch", args.batch_no),
			capkpi.get_desk_link("Warehouse", args.warehouse),
			neg_batch_sle[0]["posting_date"],
			neg_batch_sle[0]["posting_time"],
			capkpi.get_desk_link(neg_batch_sle[0]["voucher_type"], neg_batch_sle[0]["voucher_no"]),
		)
		capkpi.throw(message, NegativeStockError, title=_("Insufficient Stock for Batch"))
