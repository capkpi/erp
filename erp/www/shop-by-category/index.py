...
def get_context(context):
	...
	settings = capkpi.get_cached_doc("E Commerce Settings")
...
def get_slideshow(slideshow):
	...
	slideshow = capkpi.get_cached_doc("Website Slideshow", slideshow)
...
def get_tabs(categories):
	...
	for index, tab in enumerate(categorical_data, start=1):
		tab_values[f"tab_{index + 1}_title"] = capkpi.unscrub(tab)
		# pre-render cards for each tab
		tab_values[f"tab_{index + 1}_content"] = capkpi.render_template(
			"erp/www/shop-by-category/category_card_section.html",
...
def get_category_records(categories):
	...
	for category in categories:
		if category == "item_group":
			categorical_data["item_group"] = capkpi.db.sql(
				"""
				Select
			...
			doctype = capkpi.unscrub(category)
			fields = ["name"]
			if capkpi.get_meta(doctype, cached=True).get_field("image"):
				fields += ["image"]

			categorical_data[category] = capkpi.db.sql(
				f"""
					Select
						{",".join(fields)}
					from
