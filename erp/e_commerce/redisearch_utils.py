...
from capkpi.utils.redis_wrapper import RedisWrapper
...
def is_redisearch_enabled():
	...
	is_redisearch_enabled = capkpi.db.get_single_value("E Commerce Settings", "is_redisearch_enabled")
...
def is_search_module_loaded():
	try:
		...
		parsed_output = " ".join(
			(" ".join([capkpi.as_unicode(s) for s in o if not isinstance(s, int)]) for o in out)
		)
...
def make_key(key):
	return "{0}|{1}".format(capkpi.conf.db_name, key).encode("utf-8")
...
def create_website_items_index():
	...
	# CREATE index
	client = Client(make_key(WEBSITE_ITEM_INDEX), conn=capkpi.cache())
	...
	idx_fields = capkpi.db.get_single_value("E Commerce Settings", "search_index_fields")
...
def insert_to_name_ac(web_name, doc_name):
	ac = AutoCompleter(make_key(WEBSITE_ITEM_NAME_AUTOCOMPLETE), conn=capkpi.cache())
...
def create_item_groups_autocomplete_dict(autocompleter):
	...
	published_item_groups = capkpi.get_all(
	...
	for item_group in published_item_groups:
		...
		autocompleter.add_suggestions(
			Suggestion(
				...
				score=capkpi.utils.flt(item_group.weightage) or 1.0,
...
def reindex_all_web_items():
	items = capkpi.get_all("Website Item", fields=get_fields_indexed(), filters={"published": True})
...
def get_cache_key(name):
	name = capkpi.scrub(name)
	return f"{WEBSITE_ITEM_KEY_PREFIX}{name}"


def get_fields_indexed():
	fields_to_index = capkpi.db.get_single_value("E Commerce Settings", "search_index_fields")
...
def raise_redisearch_error():
	...
	log = capkpi.log_error(traceback, capkpi._("Redisearch Error"))
	log_link = capkpi.utils.get_link_to_form("Error Log", log.name)
