...
class UnverifiedReviewer(capkpi.ValidationError):
	pass
...
def get_item_reviews(web_item, start=0, end=10, data=None):
	...
	if settings and settings.get("enable_reviews"):
		reviews_cache = capkpi.cache().hget("item_reviews", web_item)
...
def get_queried_reviews(web_item, start=0, end=10, data=None):
	...
	data.reviews = capkpi.db.get_all(
		"Item Review",
...
def set_reviews_in_cache(web_item, reviews_dict):
	capkpi.cache().hset("item_reviews", web_item, reviews_dict)
...
def add_item_review(web_item, title, rating, comment=None):
	...
	if capkpi.session.user == "Guest":
		...
		capkpi.throw(_("You are not verified to write a review yet."), exc=UnverifiedReviewer)

	if not capkpi.db.exists("Item Review", {"user": capkpi.session.user, "website_item": web_item}):
		doc = capkpi.get_doc(
			{
				"doctype": "Item Review",
				...
				"item": capkpi.db.get_value("Website Item", web_item, "item_code"),
				"review_title": title,
				"rating": rating,
				"comment": comment,
			}
...
def get_customer(silent=False):
	...
	if customer:
		return capkpi.db.get_value("Customer", customer)
	elif silent:
		return None
	else:
