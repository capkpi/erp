...
QUnit.test("test: request_for_quotation", function(assert) {
	...
	capkpi.run_serially([
		() => {
			...
			return capkpi.tests.make('Request for Quotation', [
				...
				{items: [
					[
						...
						{"qty": 5},
						{"schedule_date": capkpi.datetime.add_days(capkpi.datetime.now_date(),20)},
		...
		() => {
			...
			assert.ok(cur_frm.doc.items[0].warehouse == 'All Warehouses - '+capkpi.get_abbr(capkpi.defaults.get_default("Company")), "Warehouse correct");
		...
		() => capkpi.click_button('Send Supplier Emails'),
		() => capkpi.timeout(6),
		() => {
