...
capkpi.ui.form.on('Shareholder', {
	refresh: function(frm) {
		capkpi.dynamic_link = { doc: frm.doc, fieldname: 'name', doctype: 'Shareholder' };
		...
		if (frm.doc.folio_no != undefined){
			frm.add_custom_button(__("Share Balance"), function(){
				...
				capkpi.set_route("query-report", "Share Balance");
			});
			frm.add_custom_button(__("Share Ledger"), function(){
				capkpi.route_options = {
					"shareholder": frm.doc.name,
				};
				capkpi.set_route("query-report", "Share Ledger");
			});
