...
capkpi.ui.form.on("Journal Entry", {
	...
	refresh: function(frm) {
		...
		if(frm.doc.docstatus==1) {
			frm.add_custom_button(__('Reverse Journal Entry'), function() {
				return erp.journal_entry.reverse_journal_entry(frm);
			}, __('Actions'));
		}

		if (frm.doc.__islocal) {
			frm.add_custom_button(__('Quick Entry'), function() {
				return erp.journal_entry.quick_entry(frm);
			});
		}

		// hide /unhide fields based on currency
		erp.journal_entry.toggle_fields_based_on_currency(frm);
	...
	posting_date: function(frm) {
		...
		$.each(frm.doc.accounts || [], function(i, row) {
			erp.journal_entry.set_exchange_rate(frm, row.doctype, row.name);
		})
	},
...
erp.accounts.JournalEntry = capkpi.ui.form.Controller.extend({
	onload: function() {
		...
		erp.accounts.dimensions.setup_dimension_filters(this.frm, this.frm.doctype);
	},
	...
	setup_queries: function() {
		...
		me.frm.set_query("account", "accounts", function(doc, cdt, cdn) {
			return erp.journal_entry.account_query(me.frm);
		});
		...
		me.frm.set_query("reference_name", "accounts", function(doc, cdt, cdn) {
			...
			if(jvd.reference_type==="Payroll Entry") {
				return {
					query: "erp.payroll.doctype.payroll_entry.payroll_entry.get_payroll_entries_for_jv",
				};
			}
	...
	accounts_add: function(doc, cdt, cdn) {
		...
		erp.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, 'accounts');
	},

});

cur_frm.script_manager.make(erp.accounts.JournalEntry);
...
capkpi.ui.form.on("Journal Entry Account", {
	...
	cost_center: function(frm, dt, dn) {
		erp.journal_entry.set_account_balance(frm, dt, dn);
	},
	...
	debit_in_account_currency: function(frm, cdt, cdn) {
		erp.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
		...
		var dialog = new capkpi.ui.Dialog({
			...
			fields: [
				...
				{fieldtype: "Link", fieldname: "debit_account", label: __("Debit Account"), reqd: 1,
					options: "Account",
					get_query: function() {
						return erp.journal_entry.account_query(frm);
					}
				},
		...
		if(d.account) {
			...
			return capkpi.call({
				...
				callback: function(r) {
					if(r.message) {
						$.extend(d, r.message);
						erp.journal_entry.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field('accounts');
					}
				}
			});
