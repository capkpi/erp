...
erp.accounts.SalesInvoiceController = erp.selling.SellingController.extend({
	...
	onload: function() {
		...
		erp.queries.setup_warehouse_query(this.frm);
	},
	...
	quotation_btn: function() {
		...
		this.$quotation_btn = this.frm.add_custom_button(__('Quotation'),
			function() {
				erp.utils.map_current_doc({
					method: "erp.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function() {
				erp.utils.map_current_doc({
					...
					get_query: function() {
						...
						return {
							query: "erp.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters
						};
					}
				});
	...
	customer: function() {
		...
		erp.utils.get_party_details(this.frm,
			"erp.accounts.party.get_party_details", {
...
cur_frm.cscript.income_account = function(doc, cdt, cdn) {
	erp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
}

cur_frm.cscript.expense_account = function(doc, cdt, cdn) {
	erp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
}

cur_frm.cscript.cost_center = function(doc, cdt, cdn) {
	erp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
}
...
frappe.ui.form.on('Sales Invoice', {
	setup: function(frm){
		...
		frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function(doc, cdt, cdn){
			return{
				query: "erp.projects.doctype.timesheet.timesheet.get_timesheet",
...
var get_healthcare_services_to_invoice = function(frm) {
	...
	dialog.fields_dict["patient"].df.onchange = () => {
		...
		if(patient && patient!=selected_patient){
			...
			var method = "erp.healthcare.utils.get_healthcare_services_to_invoice";
...
var get_drugs_to_invoice = function(frm) {
	...
	dialog.fields_dict["encounter"].df.onchange = () => {
		...
		if(encounter && encounter!=selected_encounter){
			...
			var method = "erp.healthcare.utils.get_drugs_to_invoice";
