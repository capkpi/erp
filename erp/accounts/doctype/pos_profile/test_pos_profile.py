...
from erp.accounts.doctype.pos_profile.pos_profile import get_child_nodes
from erp.stock.get_item_details import get_pos_profile
...
class TestPOSProfile(unittest.TestCase):
	def test_pos_profile(self):
		...
		if pos_profile:
			doc = capkpi.get_doc("POS Profile", pos_profile.get("name"))
...
def make_pos_profile(**args):
	capkpi.db.sql("delete from `tabPOS Payment Method`")
	capkpi.db.sql("delete from `tabPOS Profile`")

	args = capkpi._dict(args)

	pos_profile = capkpi.get_doc(
		{
			...
			"customer_group": capkpi.db.get_value("Customer Group", {"is_group": 0}, "name"),
	...
	if not capkpi.db.get_value("Mode of Payment Account", {"company": company, "parent": "Cash"}):
	...
	if not capkpi.db.exists("POS Profile", args.name or "_Test POS Profile"):
		pos_profile.insert()
