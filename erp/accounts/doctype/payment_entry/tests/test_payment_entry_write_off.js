...
QUnit.test("test payment entry", function(assert) {
	...
	capkpi.run_serially([
		...
		() => capkpi.timeout(0.5),
		() => {
			assert.equal(capkpi.get_route()[1], 'Payment Entry', 'made payment entry');
		...
		() => capkpi.timeout(1),
		() => {
			capkpi.model.set_value("Payment Entry Reference",
		...
		() => {
			capkpi.db.set_value("Company", "For Testing", "write_off_account", "_Test Write Off - FT");
			capkpi.timeout(1);
			capkpi.db.set_value("Company", "For Testing",
