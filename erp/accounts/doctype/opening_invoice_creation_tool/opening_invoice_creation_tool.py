...
class OpeningInvoiceCreationTool(Document):
	...
	def get_opening_invoice_summary(self):
		...
		companies = capkpi.get_all("Company", fields=["name as company", "default_currency as currency"])
		if not companies:
			return None, None
	...
	def validate_company(self):
		if not self.company:
			capkpi.throw(_("Please select the Company"))
	...
	def validate_mandatory_invoice_fields(self, row):
		if not capkpi.db.exists(row.party_type, row.party):
			if self.create_missing_party:
				self.add_party(row.party_type, row.party)
			else:
				capkpi.throw(
					_("Row #{}: {} {} does not exist.").format(
						row.idx, capkpi.bold(row.party_type), capkpi.bold(row.party)
					)
				)
		...
		for d in ("Party", "Outstanding Amount", "Temporary Opening Account"):
			if not row.get(scrub(d)):
				capkpi.throw(mandatory_error_msg.format(row.idx, d, self.invoice_type))
	...
	def get_invoices(self):
		...
		for row in self.invoices:
			...
			company_details = (
				capkpi.get_cached_value(
					"Company", self.company, ["default_currency", "default_letter_head"], as_dict=1
				)
				or {}
			)

			default_currency = capkpi.db.get_value(row.party_type, row.party, "default_currency")
	...
	def add_party(self, party_type, party):
		party_doc = capkpi.new_doc(party_type)
		if party_type == "Customer":
			party_doc.customer_name = party
		else:
			supplier_group = capkpi.db.get_single_value("Buying Settings", "supplier_group")
			if not supplier_group:
				capkpi.throw(_("Please Set Supplier Group in Buying Settings."))
	...
	def get_invoice_dict(self, row=None):
		def get_item_dict():
			cost_center = row.get("cost_center") or capkpi.get_cached_value(
				"Company", self.company, "cost_center"
			)
			if not cost_center:
				capkpi.throw(
					_("Please set the Default Cost Center in {0} company.").format(capkpi.bold(self.company))
				)

			income_expense_account_field = (
				"income_account" if row.party_type == "Customer" else "expense_account"
			)
			default_uom = capkpi.db.get_single_value("Stock Settings", "stock_uom") or _("Nos")
			rate = flt(row.outstanding_amount) / flt(row.qty)

			item_dict = capkpi._dict(
				{
					"uom": default_uom,
					"rate": rate or 0.0,
					"qty": row.qty,
		...
		invoice = capkpi._dict(
			{
				...
				capkpi.scrub(row.party_type): row.party,
				"is_pos": 0,
				"doctype": "Sales Invoice" if self.invoice_type == "Sales" else "Purchase Invoice",
				"update_stock": 0,  # important: https://github.com/capkpi/erp/pull/23559
...
def start_import(invoices):
	...
	for idx, d in enumerate(invoices):
		try:
			...
			capkpi.log_error(title="Error while creating Opening Invoice", message=message)
			capkpi.db.commit()
	if errors:
		capkpi.msgprint(
...
def get_temporary_opening_account(company=None):
	...
	accounts = capkpi.get_all("Account", filters={"company": company, "account_type": "Temporary"})
	if not accounts:
		capkpi.throw(_("Please add a Temporary Opening account in Chart of Accounts"))
