...
from erp.accounts.party import get_due_date, get_party_account
...
class PurchaseInvoice(BuyingController):
	...
	def get_gl_entries(self, warehouse_account=None):
		self.auto_accounting_for_stock = erp.is_perpetual_inventory_enabled(self.company)
		...
		for item in self.get("items"):
			if item.is_fixed_asset:
				...
				if not self.update_stock:
					...
					if item.item_tax_amount and not cint(erp.is_perpetual_inventory_enabled(self.company)):
						asset_eiiav_currency = get_account_currency(eiiav_account)
						gl_entries.append(
							self.get_gl_dict(
								{
