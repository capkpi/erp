...
def register_marketplace(company, company_description):
	...
	settings = capkpi.get_single("Marketplace Settings")
	...
	if message.get("hub_seller_name"):
		...
		settings.add_hub_user(capkpi.session.user)
...
def validate_registerer():
	if current_user == "Administrator":
		capkpi.throw(_("Please login as another user to register on Marketplace"))

	valid_roles = ["System Manager", "Item Manager"]

	if not capkpi.utils.is_subset(valid_roles, capkpi.get_roles()):
		capkpi.throw(
...
def map_fields(items):
	...
	table_fields = [d.fieldname for d in capkpi.get_meta("Item").get_table_fields()]

	hub_seller_name = capkpi.db.get_value(
...
def update_item(ref_doc, data):
	...
	try:
		...
		capkpi.log_error(message=e, title="Hub Sync Error")
...
def publish_selected_items(items_to_publish):
	...
	if not len(items_to_publish):
		capkpi.throw(_("No items to publish"))

	for item in items_to_publish:
		item_code = item.get("item_code")
		capkpi.db.set_value("Item", item_code, "publish_in_hub", 1)
		...
		capkpi.get_doc(hub_dict).insert(ignore_if_duplicate=True)
...
def unpublish_item(item_code, hub_item_name):
	...
	if response:
		capkpi.db.set_value("Item", item_code, "publish_in_hub", 0)
		capkpi.delete_doc("Hub Tracked Item", item_code)
	else:
		capkpi.throw(_("Unable to update remote activity"))
...
def get_unregistered_users():
	...
	all_users = [user.name for user in capkpi.db.get_all("User", filters={"enabled": 1})]
...
def item_sync_preprocess(intended_item_publish_count):
	...
	if response:
		capkpi.db.set_value("Marketplace Settings", "Marketplace Settings", "sync_in_progress", 1)
		return response
	else:
		capkpi.throw(_("Unable to update remote activity"))
...
def item_sync_postprocess():
	...
	if response:
		capkpi.db.set_value(
			"Marketplace Settings", "Marketplace Settings", "last_sync_datetime", capkpi.utils.now()
		)
	else:
		capkpi.throw(_("Unable to update remote activity"))

	capkpi.db.set_value("Marketplace Settings", "Marketplace Settings", "sync_in_progress", 0)
...
def convert_relative_image_urls_to_absolute(items):
	...
	for item in items:
		...
		if file_path.startswith("/files/"):
			item["image"] = urljoin(capkpi.utils.get_url(), file_path)


def get_hub_connection():
	settings = capkpi.get_single("Marketplace Settings")
	marketplace_url = settings.marketplace_url
	hub_user = settings.get_hub_user(capkpi.session.user)

	if hub_user:
		password = hub_user.get_password()
		hub_connection = CapKPIClient(marketplace_url, hub_user.user, password)
		return hub_connection
	else:
		read_only_hub_connection = CapKPIClient(marketplace_url)
