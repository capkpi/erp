...
QUnit.test('Make fixtures', assert => {
	...
	let tasks = [];
	Object.keys(capkpi.test_data).forEach(function(doctype) {
		tasks.push(function() {
			return capkpi.tests.setup_doctype(doctype, capkpi.test_data[doctype]);
		});
	});
	capkpi.run_serially(tasks).then(() => done());
});
