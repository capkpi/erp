...
def setup_taxes_and_charges(company_name: str, country: str):
	if not capkpi.db.exists("Company", company_name):
		capkpi.throw(_("Company {} does not exist yet. Taxes setup aborted.").format(company_name))
...
def from_detailed_data(company_name, data):
	...
	coa_name = capkpi.db.get_value("Company", company_name, "chart_of_accounts")
...
def update_regional_tax_settings(country, company):
	...
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = "erp.regional.{0}.setup.update_regional_tax_settings".format(
				capkpi.scrub(country)
			)
			capkpi.get_attr(module_name)(country, company)
...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	if capkpi.db.exists(doctype, {"title": template.get("title"), "company": company_name}):
		return
	...
	for tax_row in template.get("taxes"):
		...
		tax_row_defaults = {
			...
			"cost_center": capkpi.db.get_value("Company", company_name, "cost_center"),
		}
...
def make_tax_category(tax_category):
	...
	if not capkpi.db.exists(doctype, tax_category["title"]):
		tax_category["doctype"] = doctype
		doc = capkpi.get_doc(tax_category)
		doc.flags.ignore_links = True
		doc.flags.ignore_validate = True
		doc.insert(ignore_permissions=True)
...
def get_or_create_account(company_name, account):
	...
	existing_accounts = capkpi.get_all(
		"Account",
	...
	if existing_accounts:
		return capkpi.get_doc("Account", existing_accounts[0].name)
...
def get_or_create_tax_group(company_name, root_type):
	...
	tax_group_name = capkpi.db.get_value(
		"Account",
	...
	tax_group_account = capkpi.get_doc(
		{
			"doctype": "Account",
			"company": company_name,
			"is_group": 1,
