...
from capkpi.desk.page.setup_wizard.setup_wizard import make_records
from capkpi.utils import cstr, getdate
from capkpi.utils.nestedset import rebuild_tree

from erp.accounts.doctype.account.account import RootNotEditable
from erp.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	from erp.setup.setup_wizard.data.industry_type import get_industry_types
	...
	from erp.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def update_selling_defaults():
	selling_settings = capkpi.get_doc("Selling Settings")
...
def update_buying_defaults():
	buying_settings = capkpi.get_doc("Buying Settings")
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(capkpi.get_app_path("erp", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
		if not capkpi.db.exists("UOM", _(d.get("uom_name"))):
			uom_doc = capkpi.get_doc(
				{
					"doctype": "UOM",
	...
	uom_conversions = json.loads(
		open(
			capkpi.get_app_path("erp", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
	for d in uom_conversions:
		if not capkpi.db.exists("UOM Category", _(d.get("category"))):
			capkpi.get_doc({"doctype": "UOM Category", "category_name": _(d.get("category"))}).db_insert()

		if not capkpi.db.exists(
			"UOM Conversion Factor", {"from_uom": _(d.get("from_uom")), "to_uom": _(d.get("to_uom"))}
		):
			uom_conversion = capkpi.get_doc(
				{
...
def add_sale_stages():
	...
	for sales_stage in records:
		capkpi.get_doc(sales_stage).db_insert()
...
def install_defaults(args=None):
	...
	capkpi.db.set_value("Currency", args.get("currency"), "enabled", 1)
	capkpi.db.set_value("Stock Settings", None, "email_footer_address", args.get("company_name"))
...
def set_global_defaults(args):
	global_defaults = capkpi.get_doc("Global Defaults", "Global Defaults")
	current_fiscal_year = capkpi.get_all("Fiscal Year")[0]
...
def set_active_domains(args):
	capkpi.get_single("Domain Settings").set_active_domains(args.get("domains"))
...
def update_stock_settings():
	...
	stock_settings.default_warehouse = capkpi.db.get_value(
...
def create_bank_account(args):
	...
	if bank_account_group:
		...
		try:
			...
			capkpi.throw(_("Bank account cannot be named as {0}").format(args.get("bank_account")))
...
def update_shopping_cart_settings(args):
	shopping_cart = capkpi.get_doc("E Commerce Settings")
	shopping_cart.update(
		{
			"enabled": 1,
			"company": args.company_name,
			"price_list": capkpi.db.get_value("Price List", {"selling": 1}),
