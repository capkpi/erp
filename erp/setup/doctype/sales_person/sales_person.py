...
class SalesPerson(NestedSet):
	...
	def load_dashboard_info(self):
		...
		allocated_amount = capkpi.db.sql(
			"""
	...
	def get_email_id(self):
		if self.employee:
			user = capkpi.db.get_value("Employee", self.employee, "user_id")
			if not user:
				capkpi.throw(_("User ID not set for Employee {0}").format(self.employee))
			else:
				return capkpi.db.get_value("User", user, "email") or user

	def validate_employee_id(self):
		if self.employee:
			sales_person = capkpi.db.get_value("Sales Person", {"employee": self.employee})
...
def on_doctype_update():
	capkpi.db.add_index("Sales Person", ["lft", "rgt"])
