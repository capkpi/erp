...
capkpi.provide("capkpi.company");

capkpi.ui.form.on("Company", {
	onload: function(frm) {
		if (frm.doc.__islocal && frm.doc.parent_company) {
			capkpi.db.get_value('Company', frm.doc.parent_company, 'is_group', (r) => {
				if (!r.is_group) {
	...
	setup: function(frm) {
		capkpi.company.setup_queries(frm);
	...
	date_of_commencement: function(frm) {
		...
		{
			capkpi.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function(frm) {
		...
		if (!frm.is_new()) {
			...
			capkpi.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Company'}

			if (capkpi.perm.has_perm("Cost Center", 0, 'read')) {
				frm.add_custom_button(__('Cost Centers'), function() {
					capkpi.set_route('Tree', 'Cost Center', {'company': frm.doc.name});
				}, __("View"));
			}

			if (capkpi.perm.has_perm("Account", 0, 'read')) {
				frm.add_custom_button(__('Chart of Accounts'), function() {
					capkpi.set_route('Tree', 'Account', {'company': frm.doc.name});
				}, __("View"));
			}

			if (capkpi.perm.has_perm("Sales Taxes and Charges Template", 0, 'read')) {
				frm.add_custom_button(__('Sales Tax Template'), function() {
					capkpi.set_route('List', 'Sales Taxes and Charges Template', {'company': frm.doc.name});
				}, __("View"));
			}

			if (capkpi.perm.has_perm("Purchase Taxes and Charges Template", 0, 'read')) {
				frm.add_custom_button(__('Purchase Tax Template'), function() {
					capkpi.set_route('List', 'Purchase Taxes and Charges Template', {'company': frm.doc.name});
				}, __("View"));
			}
		...
		capkpi.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	make_default_tax_template: function(frm) {
		frm.call({
			...
			freeze: true,
			callback: function() {
				capkpi.msgprint(__("Default tax templates for sales, purchase and items are created."));
			}
		})
	},
	...
	delete_company_transactions: function(frm) {
		capkpi.verify_password(function() {
			var d = capkpi.prompt({
				fieldtype:"Data",
			...
			function(data) {
				if(data.company_name !== frm.doc.name) {
					capkpi.msgprint(__("Company name not same"));
					return;
				}
				capkpi.call({
					method: "erp.setup.doctype.company.company.create_transaction_deletion_request",
					args: {
					...
					callback: function(r, rt) {
						if(!r.exc)
							capkpi.msgprint(__("Successfully deleted all transactions related to this company!"));
					},
					onerror: function() {
						capkpi.msgprint(__("Wrong Password"));
					}
				});
			},
...
capkpi.company.set_chart_of_accounts_options = function(doc) {
	var selected_value = doc.chart_of_accounts;
	if(doc.country) {
		return capkpi.call({
			method: "erp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				"country": doc.country,
				"with_standard": true
			},
...
capkpi.company.setup_queries = function(frm) {
	$.each([
		...
		capkpi.company.set_custom_query(frm, v);
	});
...
capkpi.company.set_custom_query = function(frm, v) {
	var filters = {
