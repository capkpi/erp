...
test_records = capkpi.get_test_records("Currency Exchange")
...
def save_new_records(test_records):
	for record in test_records:
		...
		try:
			capkpi.set_value(**kwargs)
		except capkpi.DoesNotExistError:
			curr_exchange = capkpi.new_doc(record.get("doctype"))
...
class TestCurrencyExchange(unittest.TestCase):
	...
	def tearDown(self):
		capkpi.db.set_value("Accounts Settings", None, "allow_stale", 1)
		self.clear_cache()
	...
	def test_exchange_rate_strict(self):
		...
		capkpi.db.set_value("Accounts Settings", None, "allow_stale", 0)
		capkpi.db.set_value("Accounts Settings", None, "stale_days", 1)
