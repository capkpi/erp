...
def execute():
	capkpi.reload_doc("accounts", "doctype", "loyalty_program")
	capkpi.reload_doc("accounts", "doctype", "sales_invoice_item")
	...
	for si_ref_doc in sales_invoice_referenced_doc:
		if capkpi.db.exists("DocType", si_ref_doc):
			capkpi.reload_doc(get_doctype_module(si_ref_doc), "doctype", scrub(si_ref_doc))

			if capkpi.db.has_column(
				si_ref_doc, sales_invoice_referenced_doc[si_ref_doc]
			) and capkpi.db.has_column(si_ref_doc, "invoiced"):
				# Set Reference DocType and Reference Docname
				doc_list = capkpi.db.sql(
					"""
				...
				if doc_list:
					capkpi.reload_doc(get_doctype_module("Sales Invoice"), "doctype", "sales_invoice")
					for doc_id in doc_list:
						invoice_id = capkpi.db.get_value(
							si_ref_doc, doc_id[0], sales_invoice_referenced_doc[si_ref_doc]
						)
						if capkpi.db.exists("Sales Invoice", invoice_id):
							if si_ref_doc == "Lab Test":
								template = capkpi.db.get_value("Lab Test", doc_id[0], "template")
								if template:
									item = capkpi.db.get_value("Lab Test Template", template, "item")
									if item:
										capkpi.db.sql(
								...
								invoice = capkpi.get_doc("Sales Invoice", invoice_id)
