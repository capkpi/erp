...
def execute():
	for dt, field_list in field_rename_map.items():
		if capkpi.db.exists("DocType", dt):
			capkpi.reload_doc(get_doctype_module(dt), "doctype", scrub(dt))
			for field in field_list:
				if capkpi.db.has_column(dt, field[0]):
					rename_field(dt, field[0], field[1])

	if capkpi.db.exists("DocType", "Lab Prescription"):
		if capkpi.db.has_column("Lab Prescription", "parentfield"):
			capkpi.db.sql(
				"""
	...
	if capkpi.db.exists("DocType", "Lab Test Groups"):
		if capkpi.db.has_column("Lab Test Groups", "parentfield"):
			capkpi.db.sql(
				"""
