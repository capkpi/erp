...
def execute():
	if not capkpi.db.table_exists("Daily Work Summary Group"):
		capkpi.reload_doc("hr", "doctype", "daily_work_summary_group")
		capkpi.reload_doc("hr", "doctype", "daily_work_summary_group_user")

		# check if Daily Work Summary Settings Company table exists
		try:
			capkpi.db.sql("DESC `tabDaily Work Summary Settings Company`")
		except Exception:
			return
	...
	capkpi.delete_doc("DocType", "Daily Work Summary Settings")
	capkpi.delete_doc("DocType", "Daily Work Summary Settings Company")


def get_previous_setting():
	obj = {}
	setting_data = capkpi.db.sql(
...
def get_setting_companies():
	return capkpi.db.sql("select * from `tabDaily Work Summary Settings Company`", as_dict=True)
