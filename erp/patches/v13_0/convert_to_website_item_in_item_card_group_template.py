...
def execute():
	...
	"""
	capkpi.reload_doc("e_commerce", "web_template", "item_card_group")

	blocks = capkpi.db.get_all(
		"Web Page Block",
	...
	for block in blocks:
		...
		for field in fields:
			...
			if capkpi.db.exists("Website Item", {"item_code": item}):
				website_item = capkpi.db.get_value("Website Item", {"item_code": item})
			else:
