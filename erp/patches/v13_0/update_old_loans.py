...
def execute():
	...
	capkpi.reload_doc("loan_management", "doctype", "repayment_schedule")
	capkpi.reload_doc("loan_management", "doctype", "process_loan_interest_accrual")
	capkpi.reload_doc("loan_management", "doctype", "loan_repayment")
	capkpi.reload_doc("loan_management", "doctype", "loan_repayment_detail")
	capkpi.reload_doc("loan_management", "doctype", "loan_interest_accrual")
	capkpi.reload_doc("accounts", "doctype", "gl_entry")
	capkpi.reload_doc("accounts", "doctype", "journal_entry_account")

	updated_loan_types = []
	loans_to_close = []

	# Update old loan status as closed
	if capkpi.db.has_column("Repayment Schedule", "paid"):
		loans_list = capkpi.db.sql(
	...
	for loan in loans:
		...
		loan_type_company = capkpi.db.get_value("Loan Type", loan.loan_type, "company")
		loan_type = loan.loan_type

		group_income_account = capkpi.get_value(
			"Account",
			{
		...
		if loan_type_company and loan_type_company != loan.company:
			...
			loan_type_name = capkpi.get_value(
				"Loan Type",
				{
					"company": loan.company,
			...
			loan_type_doc = capkpi.get_doc("Loan Type", loan.loan_type)
		...
		if loan_type in updated_loan_types:
			...
			if capkpi.db.has_column("Repayment Schedule", "paid"):
				total_principal, total_interest = capkpi.db.get_value(
					"Repayment Schedule",
...
def create_loan_type(loan, loan_type_name, penalty_account):
	loan_type_doc = capkpi.new_doc("Loan Type")
