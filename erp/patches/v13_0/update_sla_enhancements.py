...
def execute():
	...
	if capkpi.db.exists("DocType", "Service Level Agreement"):
		sla_details = capkpi.db.get_all("Service Level Agreement", fields=["name", "service_level"])
		priorities = capkpi.db.get_all(
		...
		capkpi.reload_doc("support", "doctype", "pause_sla_on_status")
		capkpi.reload_doc("support", "doctype", "service_level_priority")
		capkpi.reload_doc("support", "doctype", "service_day")

		for entry in sla_details:
			values = capkpi.db.get_value(
		...
		if capkpi.db.exists("DocType", "Service Level"):
			service_levels = capkpi.db.get_all(
				"Service Level", filters={"service_level": ("not in", sl)}, fields=["*"]
			)
			for entry in service_levels:
				sla = capkpi.new_doc("Service Level Agreement")
				...
				if priority_list:
					sla = capkpi.get_doc("Service Level Agreement", sla.name)
	...
	capkpi.delete_doc_if_exists("DocType", "Service Level")
