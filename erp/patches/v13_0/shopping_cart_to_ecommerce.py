...
def execute():
	...
	capkpi.delete_doc("DocType", "Shopping Cart Settings", ignore_missing=True)
	capkpi.delete_doc("DocType", "Products Settings", ignore_missing=True)
	capkpi.delete_doc("DocType", "Supplier Item Group", ignore_missing=True)

	if capkpi.db.get_single_value("E Commerce Settings", "enabled"):
		notify_users()
...
def notify_users():
	...
	click.secho(
		...
		"https://docs.capkpi.com/docs/v13/user/manual/en/e_commerce/set_up_e_commerce",
		fg="yellow",
	)

	note = capkpi.new_doc("Note")
