...
def execute():
	if not (
		capkpi.db.table_exists("Payroll Period") and capkpi.db.table_exists("Taxable Salary Slab")
	):
		return
	...
	standard_tax_exemption_amount_exists = capkpi.db.has_column(
	...
	for company in capkpi.get_all("Company"):
		payroll_periods = capkpi.db.sql(
			"""
			SELECT
				{0}
			FROM
	...
	if not capkpi.db.table_exists("Employee Tax Exemption Proof Submission"):
		return

	migrated = []
	proofs = capkpi.get_all(
	...
	for proof in proofs:
		if proof.income_from_other_sources:
			employee_other_income = capkpi.new_doc("Employee Other Income")
	...
	if not capkpi.db.table_exists("Employee Tax Exemption Declaration"):
		return

	declerations = capkpi.get_all(
