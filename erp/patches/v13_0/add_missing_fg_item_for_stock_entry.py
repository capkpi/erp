...
from erp.stock.stock_ledger import make_sl_entries


def execute():
	if not capkpi.db.has_column("Work Order", "has_batch_no"):
		return
	...
	work_orders = [d.name for d in capkpi.get_all("Work Order", filters=filters, fields=fields)]
	...
	stock_entries = capkpi.db.sql_list(
		"""
		SELECT
			se.name
		FROM
	...
	for stock_entry in stock_entries:
		doc = capkpi.get_doc("Stock Entry", stock_entry)
...
def set_expense_account(doc):
	for row in doc.items:
		if row.is_finished_item and not row.expense_account:
			row.expense_account = capkpi.get_cached_value(
