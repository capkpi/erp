...
def execute():
	...
	if not capkpi.db.exists("DocType", "Party Specific Item"):
		return
	...
	for doctype in ("Purchase Invoice", "Sales Invoice"):
		...
		invoices_to_update = capkpi.get_all(
			doctype,
			fields=fields,
			filters={
				"docstatus": 1,
		...
		payment_schedule_items = capkpi.get_all(
			"Payment Schedule",
		...
		for status, docs in status_map.items():
			capkpi.db.set_value(doctype, {"name": ("in", docs)}, "status", status, update_modified=False)
