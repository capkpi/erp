...
from erp.controllers.status_updater import OverAllowanceError


def execute():
	capkpi.reload_doc("stock", "doctype", "purchase_receipt")
	capkpi.reload_doc("stock", "doctype", "purchase_receipt_item")
	capkpi.reload_doc("stock", "doctype", "delivery_note")
	capkpi.reload_doc("stock", "doctype", "delivery_note_item")
	capkpi.reload_doc("stock", "doctype", "stock_settings")

	def update_from_return_docs(doctype):
		for return_doc in capkpi.get_all(
			doctype, filters={"is_return": 1, "docstatus": 1, "return_against": ("!=", "")}
		):
			# Update original receipt/delivery document from return
			return_doc = capkpi.get_cached_doc(doctype, return_doc.name)
			try:
			...
			return_against = capkpi.get_doc(doctype, return_doc.return_against)
