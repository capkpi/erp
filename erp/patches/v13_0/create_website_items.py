...
def execute():
	capkpi.reload_doc("e_commerce", "doctype", "website_item")
	capkpi.reload_doc("e_commerce", "doctype", "website_item_tabbed_section")
	capkpi.reload_doc("e_commerce", "doctype", "website_offer")
	capkpi.reload_doc("e_commerce", "doctype", "recommended_items")
	capkpi.reload_doc("e_commerce", "doctype", "e_commerce_settings")
	...
	item_table_fields = capkpi.db.sql("desc `tabItem`", as_dict=1)
	...
	items = capkpi.db.get_all("Item", fields=item_fields, or_filters=or_filters)
	total_count = len(items)

	for count, item in enumerate(items, start=1):
		if capkpi.db.exists("Website Item", {"item_code": item.item_code}):
			continue
		...
		capkpi.utils.update_progress_bar("Creating Website Items", count, total_count)
