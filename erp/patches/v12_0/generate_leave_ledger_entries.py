...
def execute():
	...
	capkpi.reload_doc("HR", "doctype", "Leave Type")

	if not capkpi.get_meta("Leave Allocation").has_field("unused_leaves"):
...
def generate_allocation_ledger_entries():
	...
	for allocation in allocation_list:
		if not capkpi.db.exists(
			...
			allocation_obj = capkpi.get_doc("Leave Allocation", allocation)
...
def generate_application_leave_ledger_entries():
	...
	for application in leave_applications:
		if not capkpi.db.exists(
			...
			capkpi.get_doc("Leave Application", application.name).create_leave_ledger_entry()
...
def generate_encashment_leave_ledger_entries():
	...
	for encashment in leave_encashments:
		if not capkpi.db.exists(
			...
			capkpi.get_doc("Leave Encashment", encashment).create_leave_ledger_entry()


def generate_expiry_allocation_ledger_entries():
	"""fix ledger entries for missing leave allocation transaction"""
	from erp.hr.doctype.leave_ledger_entry.leave_ledger_entry import expire_allocation
...
def get_leaves_application_records():
	return capkpi.get_all("Leave Application", filters={"docstatus": 1}, fields=["name"])


def get_leave_encashment_records():
	return capkpi.get_all("Leave Encashment", filters={"docstatus": 1}, fields=["name"])
