...
capkpi.ui.form.on('Hotel Room Reservation', {
	...
	recalculate_rates: function(frm) {
		...
		capkpi.call({
			"method": "erp.hotels.doctype.hotel_room_reservation.hotel_room_reservation.get_room_rate",
	...
	make_invoice: function(frm) {
		capkpi.model.with_doc("Hotel Settings", "Hotel Settings", ()=>{
			capkpi.model.with_doctype("Sales Invoice", ()=>{
				let hotel_settings = capkpi.get_doc("Hotel Settings", "Hotel Settings");
				let invoice = capkpi.model.get_new_doc("Sales Invoice");
				...
				for (let d of frm.doc.items){
					let invoice_item = capkpi.model.add_child(invoice, "items")
				...
				capkpi.set_route("Form", invoice.doctype, invoice.name);
			});
		});
	}
});

capkpi.ui.form.on('Hotel Room Reservation Item', {
