...
import capkpi
from capkpi import _, qb, query_builder
from capkpi.query_builder import Criterion, functions
...
def get_conditions(filters):
	...
	"""
	filters = capkpi._dict(filters) if filters else capkpi._dict({})
	conditions = capkpi._dict({})

	conditions.company = filters.company or capkpi.defaults.get_user_default("company")
	conditions.end_date = filters.period_end_date or capkpi.utils.today()
	conditions.start_date = filters.period_start_date or capkpi.utils.add_months(
...
def set_payment_terms_statuses(sales_orders, invoices, filters):
	...
	for so in sales_orders:
		so.currency = capkpi.get_cached_value("Company", filters.get("company"), "default_currency")
		so.invoices = ""
