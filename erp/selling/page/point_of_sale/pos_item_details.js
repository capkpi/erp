erp.PointOfSale.ItemDetails = class {
	...
	async toggle_item_details_section(item) {
		...
		if (item && current_item_changed) {
			...
			this.item_meta = capkpi.get_meta(this.doctype);
			this.name = item.name;
			this.item_row = item;
	...
	validate_serial_batch_item() {
		...
		if ((serialized && no_serial_selected) || (batched && no_batch_selected) ||
			...
			capkpi.utils.play_sound("cancel");
		...
		if (!this.hide_images && image) {
			this.$item_image.html(
				`<img
					...
					alt="${capkpi.get_abbr(item_name)}"
					style="object-fit: cover;">`
			);
		} else {
			this.$item_image.html(`<div class="item-abbr">${capkpi.get_abbr(item_name)}</div>`);
		}
	...
	render_form(item) {
		...
		fields_to_display.forEach((fieldname, idx) => {
			...
			this[`${fieldname}_control`] = capkpi.ui.form.make_control({
				df: {
					...field_meta,
					onchange: function() {
	...
	bind_custom_control_change_event() {
			...
			this.rate_control.df.onchange = function() {
				if (this.value || flt(this.value) === 0) {
					me.events.form_updated(me.current_item, 'rate', this.value).then(() => {
						const item_row = capkpi.get_doc(me.doctype, me.name);
		...
		capkpi.model.on("POS Invoice Item", "*", (fieldname, value, item_row) => {
	...
	async auto_update_batch_no() {
		if (this.serial_no_control && this.batch_no_control) {
			...
			const serials_with_batch_no = await capkpi.db.get_list("Serial No", {
