...
from erp.accounts.doctype.pos_invoice.pos_invoice import get_stock_availability
from erp.accounts.doctype.pos_profile.pos_profile import get_child_nodes, get_item_groups
...
def search_by_term(search_term, warehouse, price_list):
	...
	if result:
		...
		price_list_rate, currency = capkpi.db.get_value(
			"Item Price",
...
def get_items(start, page_length, price_list, item_group, pos_profile, search_term=""):
	warehouse, hide_unavailable_items = capkpi.db.get_value(
	...
	if not capkpi.db.exists("Item Group", item_group):
		item_group = get_root_of("Item Group")

	condition = get_conditions(search_term)
	condition += get_item_group_condition(pos_profile)

	lft, rgt = capkpi.db.get_value("Item Group", item_group, ["lft", "rgt"])
	...
	if items_data:
		...
		item_prices_data = capkpi.get_all(
			"Item Price",
...
def search_for_serial_or_batch_or_barcode_number(search_value):
	# search barcode no
	barcode_data = capkpi.db.get_value(
	...
	# search serial no
	serial_no_data = capkpi.db.get_value(
	...
	# search batch no
	batch_no_data = capkpi.db.get_value(
...
def get_conditions(search_term):
	...
	condition += """item.name like {search_term}
		...
		search_term=capkpi.db.escape("%" + search_term + "%")
	)
...
def add_search_fields_condition(search_term):
	condition = ""
	search_fields = capkpi.get_all("POS Search Fields", fields=["fieldname"])
	if search_fields:
		for field in search_fields:
			condition += " or item.`{0}` like {1}".format(
				field["fieldname"], capkpi.db.escape("%" + search_term + "%")
			)
	return condition
...
def check_opening_entry(user):
	open_vouchers = capkpi.db.get_all(
		"POS Opening Entry",
...
def create_opening_voucher(pos_profile, company, balance_details):
	...
	new_pos_opening = capkpi.get_doc(
		{
			"doctype": "POS Opening Entry",
			"period_start_date": capkpi.utils.get_datetime(),
			"posting_date": capkpi.utils.getdate(),
...
def get_past_order_list(search_term, status, limit=20):
	...
	if search_term and status:
		invoices_by_customer = capkpi.db.get_all(
			"POS Invoice",
			filters={"customer": ["like", "%{}%".format(search_term)], "status": status},
			fields=fields,
			page_length=limit,
		)
		invoices_by_name = capkpi.db.get_all(
			"POS Invoice",
		...
		invoice_list = capkpi.db.get_all(
...
def set_customer_info(fieldname, customer, value=""):
	if fieldname == "loyalty_program":
		capkpi.db.set_value("Customer", customer, "loyalty_program", value)

	contact = capkpi.get_cached_value("Customer", customer, "customer_primary_contact")
	if not contact:
		contact = capkpi.db.sql(
			"""
	...
	if not contact:
		...
		capkpi.db.set_value("Customer", customer, "customer_primary_contact", contact)

	contact_doc = capkpi.get_doc("Contact", contact)
	if fieldname == "email_id":
		contact_doc.set("email_ids", [{"email_id": value, "is_primary": 1}])
		capkpi.db.set_value("Customer", customer, "email_id", value)
	elif fieldname == "mobile_no":
		contact_doc.set("phone_nos", [{"phone": value, "is_primary_mobile_no": 1}])
		capkpi.db.set_value("Customer", customer, "mobile_no", value)
	contact_doc.save()


@capkpi.whitelist()
def get_pos_profile_data(pos_profile):
	pos_profile = capkpi.get_doc("POS Profile", pos_profile)
