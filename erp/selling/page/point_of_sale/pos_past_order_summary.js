erp.PointOfSale.PastOrderSummary = class {
	...
	init_email_print_dialog() {
		const email_dialog = new capkpi.ui.Dialog({
			title: 'Email Receipt',
			fields: [
		...
		const print_dialog = new capkpi.ui.Dialog({
			title: 'Print Receipt',
			fields: [
	...
	print_receipt() {
		...
		capkpi.utils.print(
			...
			this.doc.language || capkpi.boot.lang
		);
	}
	...
	send_email() {
		...
		capkpi.call({
			method: "capkpi.core.doctype.communication.email.make",
			args: {
				...
				send_email: 1,
				print_format,
				sender_full_name: capkpi.user.full_name(),
				_lang: doc.language
			},
			callback: r => {
				if (!r.exc) {
					capkpi.utils.play_sound("email");
					if (r.message["emails_not_sent_to"]) {
						capkpi.msgprint(__(
							"Email not sent to {0} (unsubscribed / disabled)",
							[ capkpi.utils.escape_html(r.message["emails_not_sent_to"]) ]
						));
					} else {
						capkpi.show_alert({
					...
					capkpi.msgprint(__("There were errors while sending email. Please try again."));
				}
			}
		});
	}
	...
	attach_document_info(doc) {
		capkpi.db.get_value('Customer', this.doc.customer, 'email_id').then(({ message }) => {
