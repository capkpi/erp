...
capkpi.pages['sales-funnel'].on_page_load = function(wrapper) {
	...
	wrapper.sales_funnel = new erp.SalesFunnel(wrapper);

	capkpi.breadcrumbs.add("Selling");
}

erp.SalesFunnel = class SalesFunnel {
	constructor(wrapper) {
		var me = this;
	...
	setup(wrapper) {
		...
		this.company_field = wrapper.page.add_field({"fieldtype": "Link", "fieldname": "company", "options": "Company",
			"label": __("Company"), "reqd": 1, "default": capkpi.defaults.get_user_default('company'),
			change: function() {
				me.company = this.value || capkpi.defaults.get_user_default('company');
				me.get_data();
			}
		}),
		...
		this.company = capkpi.defaults.get_user_default('company');
		this.options = {
			from_date: capkpi.datetime.add_months(capkpi.datetime.get_today(), -1),
			to_date: capkpi.datetime.get_today(),
			chart: 'sales_funnel'
		};
		...
		$.each(this.options, function(k, v) {
			if (['from_date', 'to_date'].includes(k)) {
				me.elements[k].val(capkpi.datetime.str_to_user(v));
			} else {
				me.elements[k].val(v);
			}
			...
			me.elements[k].on("change", function() {
				if (['from_date', 'to_date'].includes(k)) {
					me.options[k] = capkpi.datetime.user_to_str($(this).val()) != 'Invalid date' ? capkpi.datetime.user_to_str($(this).val()) : capkpi.datetime.get_today();
				} else {
	...
	get_data(btn) {
		var me = this;
		if (!this.company) {
			capkpi.throw(__("Please Select a Company."));
		}

		const method_map = {
			"sales_funnel": "erp.selling.page.sales_funnel.sales_funnel.get_funnel_data",
			"opp_by_lead_source": "erp.selling.page.sales_funnel.sales_funnel.get_opp_by_lead_source",
			"sales_pipeline": "erp.selling.page.sales_funnel.sales_funnel.get_pipeline_data"
		};
		capkpi.call({
