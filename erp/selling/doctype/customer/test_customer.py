...
from erp.exceptions import PartyDisabled, PartyFrozen
from erp.selling.doctype.customer.customer import get_credit_limit, get_customer_outstanding
from erp.tests.utils import create_test_contact_and_address

test_ignore = ["Price List"]
test_dependencies = ["Payment Term", "Payment Terms Template"]
test_records = capkpi.get_test_records("Customer")

from six import iteritems


class TestCustomer(CapKPITestCase):
	def setUp(self):
		if not capkpi.get_value("Item", "_Test Item"):
	...
	def test_get_customer_group_details(self):
		doc = capkpi.new_doc("Customer Group")
	...
	def test_party_details_tax_category(self):
		...
		capkpi.delete_doc_if_exists("Address", "_Test Address With Tax Category-Shipping")

		# Tax Category without Address
		details = get_party_details("_Test Customer With Tax Category")
		self.assertEqual(details.tax_category, "_Test Tax Category 1")

		billing_address = capkpi.get_doc(
			dict(
				doctype="Address",
		...
		shipping_address = capkpi.get_doc(
			dict(
				doctype="Address",
				address_title="_Test Address With Tax Category",
		...
		settings = capkpi.get_single("Accounts Settings")
	...
	def test_rename(self):
		...
		# add comments
		comment = capkpi.get_doc("Customer", "_Test Customer 1").add_comment(
			"Comment", "Test Comment for Rename"
		)

		# rename
		capkpi.rename_doc("Customer", "_Test Customer 1", new_name)

		# check if customer renamed
		self.assertTrue(capkpi.db.exists("Customer", new_name))
		self.assertFalse(capkpi.db.exists("Customer", "_Test Customer 1"))
		...
		capkpi.rename_doc("Customer", new_name, "_Test Customer 1")
	...
	def test_freezed_customer(self):
		...
		capkpi.db.set_value("Customer", "_Test Customer", "is_frozen", 1)
		...
		capkpi.db.set_value("Customer", "_Test Customer", "is_frozen", 0)

		so.save()

	def test_delete_customer_contact(self):
		customer = capkpi.get_doc(get_customer_dict("_Test Customer for delete")).insert(
		...
		capkpi.delete_doc("Customer", customer.name)

	def test_disabled_customer(self):
		make_test_records("Item")

		capkpi.db.set_value("Customer", "_Test Customer", "disabled", 1)
		...
		capkpi.db.set_value("Customer", "_Test Customer", "disabled", 0)

		so.save()

	def test_duplicate_customer(self):
		capkpi.db.sql("delete from `tabCustomer` where customer_name='_Test Customer 1'")

		if not capkpi.db.get_value("Customer", "_Test Customer 1"):
			test_customer_1 = capkpi.get_doc(get_customer_dict("_Test Customer 1")).insert(
				ignore_permissions=True
			)
		else:
			test_customer_1 = capkpi.get_doc("Customer", "_Test Customer 1")

		duplicate_customer = capkpi.get_doc(get_customer_dict("_Test Customer 1")).insert(
			ignore_permissions=True
		)
	...
	def test_customer_credit_limit(self):
		...
		self.assertRaises(capkpi.ValidationError, so.submit)
		...
		self.assertRaises(capkpi.ValidationError, make_sales_order)

	def test_customer_credit_limit_on_change(self):
		outstanding_amt = self.get_customer_outstanding_amount()
		customer = capkpi.get_doc("Customer", "_Test Customer")
		customer.append(
		...
		self.assertRaises(capkpi.ValidationError, customer.save)
	...
	def test_customer_payment_terms(self):
		...
		capkpi.db.set_value("Customer", "_Test Customer With Template", "payment_terms", "")
...
def create_internal_customer(
	...
	exisiting_representative = capkpi.db.get_value(
	...
	if not capkpi.db.exists("Customer", customer_name):
		...
		customer_name = capkpi.db.get_value("Customer", customer_name)
