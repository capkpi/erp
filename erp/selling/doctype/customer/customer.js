...
capkpi.ui.form.on("Customer", {
	setup: function(frm) {

		frm.make_methods = {
			'Quotation': () => capkpi.model.open_mapped_doc({
				method: "erp.selling.doctype.customer.customer.make_quotation",
				frm: cur_frm
			}),
			'Opportunity': () => capkpi.model.open_mapped_doc({
				method: "erp.selling.doctype.customer.customer.make_opportunity",
				frm: cur_frm
			})
		}
		...
		frm.set_query('customer_primary_contact', function(doc) {
			return {
				query: "erp.selling.doctype.customer.customer.get_customer_primary_contact",
				filters: {
					'customer': doc.name
				}
			}
	...
	refresh: function(frm) {
		if(capkpi.defaults.get_default("cust_master_name")!="Naming Series") {
			frm.toggle_display("naming_series", false);
		} else {
			erp.toggle_naming_series();
		}

		capkpi.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Customer'}
		...
		if(!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Accounts Receivable'), function () {
				capkpi.set_route('query-report', 'Accounts Receivable', {customer:frm.doc.name});
			}, __('View'));
			...
			frm.add_custom_button(__('Pricing Rule'), function () {
				erp.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name);
			}, __('Create'));
	...
	validate: function(frm) {
		if(frm.doc.lead_name) capkpi.model.clear_doc("Lead", frm.doc.lead_name);
