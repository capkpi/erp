...
capkpi.ui.form.on("Sales Order", {
	...
	onload: function(frm) {
		...
		frm.set_query('project', function(doc, cdt, cdn) {
			return {
				query: "erp.controllers.queries.get_project_name",
				filters: {
		...
		frm.set_query('warehouse', 'items', function(doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "erp.controllers.queries.warehouse_query";
...
capkpi.ui.form.on("Sales Order Item", {
	...
	delivery_date: function(frm, cdt, cdn) {
		if(!frm.doc.delivery_date) {
			erp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	}
});

erp.selling.SalesOrderController = erp.selling.SellingController.extend({
			...
			this.frm.add_custom_button(__('Quotation'),
				function() {
					erp.utils.map_current_doc({
						method: "erp.selling.doctype.quotation.quotation.make_sales_order",
	...
	make_material_request: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm
		})
	},
	...
	make_raw_material_request_dialog: function(r) {
		...
		var d = new capkpi.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'erp.selling.doctype.sales_order.sales_order.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
					...
					callback: function(r) {
						if(r.message) {
							capkpi.msgprint(__('Material Request {0} submitted.',
					...
					${delivery_dates.map(date => `
						<div class="list-item">
							<div class="list-item__content list-item__content--flex-2">
								...
								${capkpi.datetime.str_to_user(date)}
								</label>
							</div>
						</div>
					`).join("")}
	...
	make_delivery_note: function(delivery_dates) {
		capkpi.model.open_mapped_doc({
			method: "erp.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates
			}
	...
	make_sales_invoice: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.selling.doctype.sales_order.sales_order.make_sales_invoice",
			frm: this.frm
		})
	},
	...
	make_maintenance_schedule: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm
		})
	},
	...
	make_project: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm
		})
	},
	...
	make_inter_company_order: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm
		});
	},
	...
	make_maintenance_visit: function() {
		capkpi.model.open_mapped_doc({
			method: "erp.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm
		})
	},
	...
	make_purchase_order: function(){
		...
		if(!pending_items){
			capkpi.throw({message: __("Purchase Order already created for all Sales Order items"), title: __("Note")});
		}
		...
		var dialog = new capkpi.ui.Dialog({
			...
			primary_action (args) {
				...
				if(selected_items.length == 0) {
					capkpi.throw({message: 'Please select Items from the Table', title: __('Items Required'), indicator:'blue'})
				}
				...
				return capkpi.call({
					method: "erp.selling.doctype.sales_order.sales_order." + method,
					freeze: true,
					...
					callback: function(r) {
						if(!r.exc) {
							if (!args.against_default_supplier) {
								...
								capkpi.set_route("List", "Purchase Order");
							}
						}
					}
				})
	...
	update_status: function(label, status){
		...
		capkpi.call({
			method: "erp.selling.doctype.sales_order.sales_order.update_status",
...
$.extend(cur_frm.cscript, new erp.selling.SalesOrderController({frm: cur_frm}));
